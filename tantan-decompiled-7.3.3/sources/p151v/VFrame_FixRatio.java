        ...
        if (mode != 1073741824 || size <= 0 || size2 <= 0) {
            if (mode != 1073741824 || size <= 0) {
                if (mode2 != 1073741824 || size2 <= 0) {
                    float f = size;
                    float f2 = size2;
                    float f3 = this.f210016a;
                    if (f >= f2 * f3) {
                        size = (int) (f2 * f3);
                    }
                } else {
                    fMin = size2 * this.f210016a;
                }
            }
            super.onMeasure(View.MeasureSpec.makeMeasureSpec(size, 1073741824), View.MeasureSpec.makeMeasureSpec((int) (size / this.f210016a), 1073741824));
        }
        fMin = Math.min(size, size2 * this.f210016a);
        size = (int) fMin;
        super.onMeasure(View.MeasureSpec.makeMeasureSpec(size, 1073741824), View.MeasureSpec.makeMeasureSpec((int) (size / this.f210016a), 1073741824));
    }
