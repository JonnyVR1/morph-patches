...
public class LZ4BlockOutputStream extends FilterOutputStream {
    ...
    private static int compressionLevel(int i) {
        if (i < 64) {
            za50.m219101a("blockSize must be >= 64, got ", i);
            return 0;
        }
        if (i <= MAX_BLOCK_SIZE) {
            return Math.max(0, 22 - Integer.numberOfLeadingZeros(i - 1));
        }
        za50.m219101a("blockSize must be <= 33554432, got ", i);
        return 0;
    }

    private void ensureNotFinished() {
        if (this.finished) {
            wtq0.m207906a("This stream is already closed");
        }
    }
    ...
    private void flushBufferedData() throws IOException {
        ...
        writeIntLE(this.f206764o, this.compressedBuffer, i5 + 5);
    ...
    public void write(int i) throws IOException {
        ensureNotFinished();
        if (this.f206764o == this.blockSize) {
            flushBufferedData();
        }
        byte[] bArr = this.buffer;
        int i2 = this.f206764o;
    ...
    public LZ4BlockOutputStream(OutputStream outputStream, int i, LZ4Compressor lZ4Compressor) {
        emq0.m121374b().m121381g(DEFAULT_SEED);
        throw null;
    }
