...
public class LZ4FrameInputStream extends FilterInputStream {
    ...
    public LZ4FrameInputStream(InputStream inputStream, LZ4SafeDecompressor lZ4SafeDecompressor, cmq0 cmq0Var, boolean z) throws IOException {
        super(inputStream);
    ...
    private void readBlock() throws IOException {
        ...
        if (iDecompress == 0) {
            if (this.frameInfo.isEnabled(LZ4FrameOutputStream.FLG.Bits.CONTENT_CHECKSUM) && readInt(((FilterInputStream) this).in) != this.frameInfo.currentStreamHash()) {
                zpg0.m220844a("Content checksum mismatch");
                return;
                ...
                zpg0.m220844a("Size check mismatch");
                return;
            }
        }
        byte[] bArr = z ? this.compressedBuffer : this.rawBuffer;
        if (iDecompress > this.maxBlockSize) {
            zpg0.m220844a(String.format(Locale.ROOT, "Block size %s exceeded max: %s", Integer.valueOf(iDecompress), Integer.valueOf(this.maxBlockSize)));
            return;
        }
        int i2 = 0;
        while (i2 < iDecompress) {
            ...
            if (i3 < 0) {
                zpg0.m220844a(PREMATURE_EOS);
                return;
            }
            i2 += i3;
        }
        if (this.frameInfo.isEnabled(LZ4FrameOutputStream.FLG.Bits.BLOCK_CHECKSUM) && readInt(((FilterInputStream) this).in) != this.checksum.m111313a(bArr, 0, iDecompress, 0)) {
            zpg0.m220844a(BLOCK_HASH_MISMATCH);
            return;
        }
        if (z) {
            try {
    ...
    private void readHeader() throws IOException {
        ...
        byte b2 = (byte) (i2 & 255);
        LZ4FrameOutputStream.C21950BD c21950bdFromByte = LZ4FrameOutputStream.C21950BD.fromByte(b2);
        this.headerBuffer.put(b2);
        this.frameInfo = new LZ4FrameOutputStream.FrameInfo(flgFromByte, c21950bdFromByte);
        ...
        this.totalContentSize = 0L;
        byte bM111313a = (byte) ((this.checksum.m111313a(this.headerArray, 0, this.headerBuffer.position(), 0) >> 8) & 255);
        int i3 = ((FilterInputStream) this).in.read();
        if (i3 < 0) {
            zpg0.m220844a(PREMATURE_EOS);
            return;
        }
        if (bM111313a != ((byte) (i3 & 255))) {
            zpg0.m220844a(DESCRIPTOR_HASH_MISMATCH);
            return;
        }
    ...
    public long getExpectedContentSize() {
        ...
        pr3.m173429a("Operation not permitted when multiple frames can be read");
        return 0L;
    }
    ...
    public LZ4FrameInputStream(InputStream inputStream, boolean z) throws IOException {
        this(inputStream, LZ4Factory.fastestInstance().safeDecompressor(), emq0.m121374b().m121380d(), z);
    }

    public LZ4FrameInputStream(InputStream inputStream, LZ4SafeDecompressor lZ4SafeDecompressor, cmq0 cmq0Var) throws IOException {
        this(inputStream, lZ4SafeDecompressor, cmq0Var, false);
    }

    public LZ4FrameInputStream(InputStream inputStream) throws IOException {
        this(inputStream, LZ4Factory.fastestInstance().safeDecompressor(), emq0.m121374b().m121380d());
    }
