...
public abstract class FiniteFields {
    ...
    public static PolynomialExtensionField getBinaryExtensionField(int[] iArr) {
        if (iArr[0] != 0) {
            wg3.m206174a("Irreducible polynomials in GF(2) must have constant term");
            return null;
        }
        for (int i = 1; i < iArr.length; i++) {
            if (iArr[i] <= iArr[i - 1]) {
                wg3.m206174a("Polynomial exponents must be montonically increasing");
                return null;
            }
        }
    ...
    public static FiniteField getPrimeField(BigInteger bigInteger) {
        ...
        if (bigInteger.signum() <= 0 || iBitLength < 2) {
            wg3.m206174a("'characteristic' must be >= 2");
            return null;
        }
        if (iBitLength < 3) {
