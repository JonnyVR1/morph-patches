...
public class BERConstructedOctetString extends BEROctetString {
    ...
    private static byte[] toByteArray(ASN1Primitive aSN1Primitive) {
        try {
            ...
            wg3.m206174a("Unable to encode object");
            return null;
        }
    }
    ...
    private static byte[] toBytes(Vector vector) {
        ...
        for (int i = 0; i != vector.size(); i++) {
            try {
                ...
                C21225x1.m208893a("exception converting octets ", e);
                return null;
            } catch (ClassCastException unused) {
                wg3.m206174a(vector.elementAt(i).getClass().getName().concat(" found in input should only contain DEROctetString"));
                return null;
            }
        }
