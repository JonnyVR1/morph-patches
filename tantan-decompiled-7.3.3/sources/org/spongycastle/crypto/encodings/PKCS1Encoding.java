...
public class PKCS1Encoding implements AsymmetricBlockCipher {
    ...
    private byte[] decodeBlock(byte[] bArr, int i, int i2) throws InvalidCipherTextException {
        ...
        if (bArrProcessBlock.length < getOutputBlockSize()) {
            psl.m173659a("block truncated");
            return null;
        }
        byte b2 = bArrProcessBlock[0];
        if (this.forPrivateKey) {
            if (b2 != 2) {
                psl.m173659a("unknown block type");
                return null;
            }
        } else if (b2 != 1) {
            psl.m173659a("unknown block type");
        ...
        if (this.useStrictLength && bArrProcessBlock.length != this.engine.getOutputBlockSize()) {
            psl.m173659a("block incorrect size");
            return null;
        }
        int i3 = 1;
        while (i3 != bArrProcessBlock.length && (b = bArrProcessBlock[i3]) != 0) {
            if (b2 == 1 && b != -1) {
                psl.m173659a("block padding incorrect");
                return null;
            }
            i3++;
        }
        ...
        if (i4 > bArrProcessBlock.length || i4 < 10) {
            psl.m173659a("no data in block");
            return null;
        }
        int length = bArrProcessBlock.length - i4;
        byte[] bArr2 = new byte[length];
        System.arraycopy(bArrProcessBlock, i4, bArr2, 0, length);
        return bArr2;
    }

    private byte[] decodeBlockOrRandom(byte[] bArr, int i, int i2) throws InvalidCipherTextException {
        if (!this.forPrivateKey) {
            psl.m173659a("sorry, this method is only for decryption, not for signing");
            return null;
        }
        byte[] bArrProcessBlock = this.engine.processBlock(bArr, i, i2);
    ...
    private byte[] encodeBlock(byte[] bArr, int i, int i2) throws InvalidCipherTextException {
        if (i2 > getInputBlockSize()) {
            wg3.m206174a("input data too large");
            return null;
        }
