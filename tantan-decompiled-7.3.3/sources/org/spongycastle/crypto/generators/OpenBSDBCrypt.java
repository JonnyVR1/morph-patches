...
public class OpenBSDBCrypt {
    ...
    public static boolean checkPassword(String str, char[] cArr) {
        if (str.length() != 60) {
            gx50.m132745a("Bcrypt String length: ", str.length(), ", 60 required.");
            return false;
        }
        if (str.charAt(0) != '$' || str.charAt(3) != '$' || str.charAt(6) != '$') {
            wg3.m206174a("Invalid Bcrypt String format.");
            return false;
        }
        if (!str.substring(1, 3).equals(version)) {
            wg3.m206174a("Wrong Bcrypt version, 2a expected.");
            return false;
        }
        try {
            int i = Integer.parseInt(str.substring(4, 6));
            if (i < 4 || i > 31) {
                i2r0.m138234a("Invalid cost factor: ", i, ", 4 < cost < 31 expected.");
                return false;
            }
            if (cArr != null) {
                return str.equals(generate(cArr, decodeSaltString(str.substring(str.lastIndexOf(36) + 1, str.length() - 31)), i));
            }
            wg3.m206174a("Missing password.");
            return false;
        } catch (NumberFormatException unused) {
            wg3.m206174a("Invalid cost factor:".concat(str.substring(4, 6)));
            return false;
        }
    }
    ...
    private static byte[] decodeSaltString(String str) {
        ...
        if (charArray.length != 22) {
            gx50.m132745a("Invalid base64 salt length: ", charArray.length, " , 22 required.");
            return null;
        }
        for (char c : charArray) {
            if (c > 'z' || c < '.' || (c > '9' && c < 'A')) {
                za50.m219101a("Salt string contains invalid character: ", c);
                return null;
            }
        }
    ...
    private static String encodeData(byte[] bArr) {
        ...
        if (bArr.length != 24 && bArr.length != 16) {
            gx50.m132745a("Invalid length: ", bArr.length, ", 24 for key or 16 for salt expected");
            return null;
        }
        if (bArr.length == 16) {
    ...
    public static String generate(char[] cArr, byte[] bArr, int i) {
        if (cArr == null) {
            wg3.m206174a("Password required.");
            return null;
        }
        if (bArr == null) {
            wg3.m206174a("Salt required.");
            return null;
        }
        if (bArr.length != 16) {
        ...
        if (i < 4 || i > 31) {
            wg3.m206174a("Invalid cost factor.");
            return null;
        }
