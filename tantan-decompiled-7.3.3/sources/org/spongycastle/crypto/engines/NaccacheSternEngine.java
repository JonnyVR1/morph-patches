...
public class NaccacheSternEngine implements AsymmetricBlockCipher {
    ...
    public byte[] addCryptedBlocks(byte[] bArr, byte[] bArr2) throws InvalidCipherTextException {
        if (this.forEncryption) {
            if (bArr.length > getOutputBlockSize() || bArr2.length > getOutputBlockSize()) {
                psl.m173659a("BlockLength too large for simple addition.\n");
                return null;
            }
    ...
    public int getInputBlockSize() {
        ...
        return z ? ((naccacheSternKeyParameters.getLowerSigmaBound() + 7) / 8) - 1 : naccacheSternKeyParameters.getModulus().toByteArray().length;
    }
    ...
    public int getOutputBlockSize() {
        boolean z = this.forEncryption;
        NaccacheSternKeyParameters naccacheSternKeyParameters = this.key;
        return z ? naccacheSternKeyParameters.getModulus().toByteArray().length : ((naccacheSternKeyParameters.getLowerSigmaBound() + 7) / 8) - 1;
    }
    ...
    public byte[] processBlock(byte[] bArr, int i, int i2) throws InvalidCipherTextException {
        if (this.key == null) {
            wtq0.m207906a("NaccacheStern engine not initialised");
            return null;
        }
        if (i2 > getInputBlockSize() + 1) {
            tyk0.m193596a("input too large for Naccache-Stern cipher.\n");
            return null;
        }
        if (!this.forEncryption && i2 < getInputBlockSize()) {
            psl.m173659a("BlockLength does not match modulus for Naccache-Stern cipher.\n");
            return null;
        }
    ...
    public byte[] processData(byte[] bArr) throws InvalidCipherTextException {
        ...
        while (length < bArr.length) {
            ...
            if (bArrProcessBlock == null) {
                ...
                psl.m173659a("cipher returned null");
                return null;
            }
