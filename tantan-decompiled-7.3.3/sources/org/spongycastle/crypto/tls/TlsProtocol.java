...
public abstract class TlsProtocol {
    ...
    private void processAlert() throws IOException {
        while (this.alertQueue.available() >= 2) {
            ...
            if (s == 2) {
                ...
                zpg0.m220844a(TLS_ERROR_MESSAGE);
                return;
            }
            if (s2 == 0) {
                handleClose(false);
    ...
    public int getAvailableInputBytes() {
        ...
        wtq0.m207906a("Cannot use getAvailableInputBytes() in blocking mode! Use getInputStream().available() instead.");
        return 0;
    }
    ...
    public int getAvailableOutputBytes() {
        ...
        wtq0.m207906a("Cannot use getAvailableOutputBytes() in blocking mode! Use getOutputStream() instead.");
        return 0;
    }
    ...
    public InputStream getInputStream() {
        ...
        wtq0.m207906a("Cannot use InputStream in non-blocking mode! Use offerInput() instead.");
        return null;
    }
    ...
    public OutputStream getOutputStream() {
        ...
        wtq0.m207906a("Cannot use OutputStream in non-blocking mode! Use offerOutput() instead.");
        return null;
    }
    ...
    public void offerInput(byte[] bArr) throws IOException {
        if (this.blocking) {
            wtq0.m207906a("Cannot use offerInput() in blocking mode! Use getInputStream() instead.");
            return;
        }
        if (this.closed) {
            zpg0.m220844a("Connection is closed, cannot accept any more input");
            return;
        }
    ...
    public void offerOutput(byte[] bArr, int i, int i2) throws IOException {
        if (this.blocking) {
            wtq0.m207906a("Cannot use offerOutput() in blocking mode! Use getOutputStream() instead.");
        } else if (this.appDataReady) {
            writeData(bArr, i, i2);
        } else {
            zpg0.m220844a("Application data cannot be sent until the handshake is complete!");
        }
    }
    ...
    public int readInput(byte[] bArr, int i, int i2) {
        if (this.blocking) {
            wtq0.m207906a("Cannot use readInput() in blocking mode! Use getInputStream() instead.");
            return 0;
        }
        try {
    ...
    public int readOutput(byte[] bArr, int i, int i2) {
        if (this.blocking) {
            wtq0.m207906a("Cannot use readOutput() in blocking mode! Use getOutputStream() instead.");
            return 0;
        }
    ...
    public void writeData(byte[] bArr, int i, int i2) throws IOException {
        if (this.closed) {
            if (this.failedWithError) {
                ...
                zpg0.m220844a("Sorry, connection has been closed, you cannot write more data");
                return;
            }
        }
        while (i2 > 0) {
