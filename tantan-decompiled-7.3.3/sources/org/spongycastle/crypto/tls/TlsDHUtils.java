...
public class TlsDHUtils {
    ...
    private static DHParameters fromSafeP(String str) {
        BigInteger bigIntegerFromHex = fromHex(str);
        return new DHParameters(bigIntegerFromHex, TWO, bigIntegerFromHex.shiftRight(1));
    }
    ...
    public static DHPublicKeyParameters validateDHPublicKey(DHPublicKeyParameters dHPublicKeyParameters) throws IOException {
        DHParameters dHParametersValidateDHParameters = validateDHParameters(dHPublicKeyParameters.getParameters());
        BigInteger y = dHPublicKeyParameters.getY();
        BigInteger bigInteger = TWO;
        if (y.compareTo(bigInteger) >= 0 && y.compareTo(dHParametersValidateDHParameters.getP().subtract(bigInteger)) <= 0) {
