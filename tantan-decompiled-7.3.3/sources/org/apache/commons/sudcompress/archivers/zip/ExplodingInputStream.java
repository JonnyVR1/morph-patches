...
class ExplodingInputStream extends InputStream {
    ...
    private final InputStream f206790in;
    ...
    public ExplodingInputStream(int i, int i2, InputStream inputStream) {
        if (i != 4096 && i != 8192) {
            wg3.m206174a("The dictionary size must be 4096 or 8192");
            throw null;
        }
        if (i2 != 2 && i2 != 3) {
            wg3.m206174a("The number of trees must be 2 or 3");
            throw null;
        }
        this.dictionarySize = i;
        this.numberOfTrees = i2;
        this.minimumMatchLength = i2;
        this.f206790in = inputStream;
    }
    ...
    private void init() throws IOException {
        if (this.bits == null) {
            ezg0 ezg0Var = new ezg0(new hwg0(this.f206790in));
            try {
                if (this.numberOfTrees == 3) {
                    this.literalTree = BinaryTree.decode(ezg0Var, 256);
                }
                this.lengthTree = BinaryTree.decode(ezg0Var, 64);
                this.distanceTree = BinaryTree.decode(ezg0Var, 64);
                this.treeSizes += ezg0Var.getBytesRead();
                ezg0Var.close();
                this.bits = new BitStream(this.f206790in);
            } catch (Throwable th) {
                try {
                    ezg0Var.close();
                } catch (Throwable th2) {
