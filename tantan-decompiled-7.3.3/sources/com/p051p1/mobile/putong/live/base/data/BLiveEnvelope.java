...
public class BLiveEnvelope extends ValueObject implements Cloneable, Serializable {
    ...
    public BLiveEnvelope mo225055clone() {
        BLiveEnvelope bLiveEnvelope = new BLiveEnvelope();
        Meta meta = this.meta;
        if (meta != null) {
            bLiveEnvelope.meta = meta.mo225055clone();
        }
        BLiveOriginCounter bLiveOriginCounter = this.counters;
        if (bLiveOriginCounter != null) {
            bLiveEnvelope.counters = bLiveOriginCounter.mo225055clone();
        }
        BLiveCounter bLiveCounter = this.liveCounters;
        if (bLiveCounter != null) {
            bLiveEnvelope.liveCounters = bLiveCounter.mo225055clone();
        }
        BLiveData bLiveData = this.data;
        if (bLiveData != null) {
            bLiveEnvelope.data = bLiveData.mo225055clone();
        }
        BLiveMagic bLiveMagic = this.liveMagics;
        if (bLiveMagic != null) {
            bLiveEnvelope.liveMagics = bLiveMagic.mo225055clone();
        }
        Pagination pagination = this.pagination;
        if (pagination != null) {
            bLiveEnvelope.pagination = pagination.mo225055clone();
        }
        return bLiveEnvelope;
    }
    ...
    public void onApiParseComplete() {
        ...
        for (Relationship relationship : this.data.relationships) {
            map.put(relationship.f39654id, relationship);
        }
        ...
        for (User user : this.data.users) {
            ...
            if (map2.size() != 0) {
                user.localFollowship = (Followship) map2.get(user.f56859id);
            }
        }
    }
