...
public class ProfileEditInfo extends ValueObject implements Cloneable, Serializable {
    ...
    public static ProtobufAdapter<ProfileEditInfo> PROTOBUF_ADAPTER = new MessageNanoAdapter<ProfileEditInfo>() { // from class: com.p1.mobile.putong.core.data.ProfileEditInfo.1
        ...
        public int computeAndCacheSize(ProfileEditInfo profileEditInfo) {
            int iM17275b = CodedOutputByteBufferNano.m17275b(1, profileEditInfo.isHit);
            User user = profileEditInfo.user;
            if (user != null) {
                iM17275b += CodedOutputByteBufferNano.m17285l(2, user, User.PROTOBUF_ADAPTER);
            }
            List<DetectText> list = profileEditInfo.details;
            if (list != null) {
                iM17275b += CodedOutputByteBufferNano.m17285l(3, list, DetectText.PROTOBUF_ADAPTER.ARRAY_ADAPTER());
            }
            profileEditInfo.cachedSize = iM17275b;
            return iM17275b;
        }

        @Override // com.tantanapp.common.data.ProtobufAdapter
        public ProfileEditInfo parse(nc5 nc5Var) throws IOException {
            ...
            while (true) {
                ...
                if (iM162497u == 8) {
                    profileEditInfo.isHit = nc5Var.m162483g();
                } else if (iM162497u == 18) {
                    profileEditInfo.user = (User) nc5Var.m162488l(User.PROTOBUF_ADAPTER);
                } else {
                    if (iM162497u != 26) {
                        if (profileEditInfo.user != null) {
                            break;
                    ...
                    profileEditInfo.details = (List) nc5Var.m162488l(DetectText.PROTOBUF_ADAPTER.ARRAY_ADAPTER());
                }
            }
            return profileEditInfo;
        }
        ...
        public void serialize(ProfileEditInfo profileEditInfo, CodedOutputByteBufferNano codedOutputByteBufferNano) throws IOException {
            codedOutputByteBufferNano.m17299A(1, profileEditInfo.isHit);
            User user = profileEditInfo.user;
            if (user != null) {
            ...
            List<DetectText> list = profileEditInfo.details;
            if (list != null) {
                codedOutputByteBufferNano.m17309K(3, list, DetectText.PROTOBUF_ADAPTER.ARRAY_ADAPTER());
            }
        }
    };
    ...
    public ProfileEditInfo mo225055clone() {
        ProfileEditInfo profileEditInfo = new ProfileEditInfo();
        profileEditInfo.isHit = this.isHit;
        User user = this.user;
        if (user != null) {
            profileEditInfo.user = user.mo225055clone();
        }
        List<DetectText> list = this.details;
        if (list != null) {
            profileEditInfo.details = ValueObject.util_map(list, new qcj() { // from class: l.ur90
                @Override // p153l.qcj
