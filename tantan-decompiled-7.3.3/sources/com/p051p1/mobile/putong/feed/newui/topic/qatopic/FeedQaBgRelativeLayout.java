package com.p051p1.mobile.putong.feed.newui.topic.qatopic;

import android.content.Context;
import android.util.AttributeSet;
import android.widget.RelativeLayout;
import p153l.qcj;

/* JADX INFO: loaded from: classes13.dex */
public class FeedQaBgRelativeLayout extends RelativeLayout {

    /* JADX INFO: renamed from: a */
    public qcj<Integer, Boolean> f43916a;

    public FeedQaBgRelativeLayout(Context context) {
        super(context);
    }

    @Override // android.view.ViewGroup
    public int getChildDrawingOrder(int i, int i2) {
        qcj<Integer, Boolean> qcjVar = this.f43916a;
        if (qcjVar != null) {
            if (i2 == 0) {
                if (qcjVar.call(0).booleanValue()) {
                    return i - 1;
                }
                if (this.f43916a.call(1).booleanValue()) {
                    return 0;
                }
            } else if (i2 == 1) {
                if (qcjVar.call(1).booleanValue()) {
                    return i - 1;
                }
                if (this.f43916a.call(0).booleanValue()) {
                    return 0;
                }
            }
        }
        return i2;
    }

    @Override // android.view.ViewGroup
    public boolean isChildrenDrawingOrderEnabled() {
        return true;
    }

    public void setIsViewIsSelect(qcj<Integer, Boolean> qcjVar) {
        this.f43916a = qcjVar;
    }

    public FeedQaBgRelativeLayout(Context context, AttributeSet attributeSet) {
        super(context, attributeSet);
    }

    public FeedQaBgRelativeLayout(Context context, AttributeSet attributeSet, int i) {
        super(context, attributeSet, i);
    }

    public FeedQaBgRelativeLayout(Context context, AttributeSet attributeSet, int i, int i2) {
        super(context, attributeSet, i, i2);
    }
}
