...
class WarmUpDelegate {
    ...
    public static final class LocalWarmUpInvoker implements WarmUpInvoker {
        ...
        @Override // com.p051p1.mobile.backtrace.backtrace.WarmUpInvoker
    ...
    public static final class RemoteWarmUpInvoker implements WarmUpInvoker, WarmUpService.RemoteConnection {
        ...
        public boolean warmUp(String str, int i) {
            ...
            MatrixLog.m22185i(WarmUpDelegate.TAG, "Warm-up %s:%s - retCode %s", str, Integer.valueOf(i), Integer.valueOf(i2));
            return z;
        }
    }
            ...
            while (true) {
                if (runnable != null) {
                    try {
                        ...
                        MatrixLog.m22185i(WarmUpDelegate.TAG, "Before '%s' task execution..", strPoll);
                        runnable.run();
                        MatrixLog.m22185i(WarmUpDelegate.TAG, "After '%s' task execution..", strPoll);
    ...
    public static final class WarmedUpReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            MatrixLog.m22185i(WarmUpDelegate.TAG, "Warm-up received.", new Object[0]);
    ...
    public void broadcastWarmedUp(Context context) {
        ...
        MatrixLog.m22185i(TAG, "Broadcast warmed up message to other processes.", new Object[0]);
    ...
    public boolean warmUpBlocked(String str, int i) {
        ...
        boolean z = !zCheck;
        if (!zCheck) {
            MatrixLog.m22187w(TAG, "Elf file %s:%s has blocked and will not do warm-up.", str, Integer.valueOf(i));
        }
        return z;
    }
    ...
    public void cleaningUp(final CancellationSignal cancellationSignal) {
        this.mThreadTaskExecutor.arrangeTask(new Runnable() { // from class: com.p1.mobile.backtrace.backtrace.WarmUpDelegate.2
            ...
            public void run() {
                ...
                MatrixLog.m22185i(WarmUpDelegate.TAG, "Going to clean up saving path(%s)..", file.getAbsoluteFile());
                if (!file.isDirectory()) {
                ...
                try {
                        ...
                        public boolean accept(File file2) {
                            try {
                                ...
                                if (!name.contains("_malformed_") && !name.contains("_temp_")) {
                                    ...
                                    MatrixLog.m22185i(WarmUpDelegate.TAG, "File(%s) last access time %s", absolutePath, Long.valueOf(jMax));
                                    if (System.currentTimeMillis() - jMax > 5184000000L) {
                                        file2.delete();
                                        MatrixLog.m22185i(WarmUpDelegate.TAG, "Delete long time no access file(%s)", absolutePath);
                                        ...
                                        if (pair == null) {
                                            ...
                                            if (System.currentTimeMillis() - ((Long) pair.second).longValue() >= 259200000) {
                                                ...
                                                MatrixLog.m22185i(WarmUpDelegate.TAG, "Delete file(%s) cause %s is newer(%s vs %s).", ((File) pair.first).getName(), name, pair.second, Long.valueOf(jMax));
                                            }
                                            map.put(strSubstring, new Pair(file2, Long.valueOf(jMax)));
                                        } else if (System.currentTimeMillis() - jMax >= 259200000) {
                                            file2.delete();
                                            MatrixLog.m22185i(WarmUpDelegate.TAG, "Delete file(%s) cause %s is newer(%s vs %s).", name, ((File) pair.first).getName(), Long.valueOf(jMax), pair.second);
                                        }
                                    } else if (jMax - System.currentTimeMillis() >= 2592000000L) {
                                        file2.delete();
                                        MatrixLog.m22185i(WarmUpDelegate.TAG, "Delete future file(%s)", absolutePath);
                                    }
                                } else if (System.currentTimeMillis() - file2.lastModified() >= 259200000) {
                                    MatrixLog.m22185i(WarmUpDelegate.TAG, "Delete malformed and temp file %s", absolutePath);
                                    file2.delete();
                                }
                            } catch (Throwable th) {
                    ...
                    MatrixLog.m22185i(WarmUpDelegate.TAG, "Clean up saving path(%s) cancelled.", file.getAbsoluteFile());
                    return;
                } catch (Throwable th) {
                    MatrixLog.printErrStackTrace(WarmUpDelegate.TAG, th, "", new Object[0]);
                }
                ...
                WarmUpDelegate.this.mWarmUpScheduler.taskFinished(WarmUpScheduler.TaskType.CleanUp);
                MatrixLog.m22185i(WarmUpDelegate.TAG, "Clean up saving path(%s) done.", file.getAbsoluteFile());
                WarmUpReporter warmUpReporter = WarmUpDelegate.sReporter;
                if (warmUpReporter != null) {
        ...
        this.mThreadTaskExecutor.arrangeTask(new Runnable() { // from class: com.p1.mobile.backtrace.backtrace.WarmUpDelegate.4
            ...
            public void run() {
                ...
                try {
                    ...
                    MatrixLog.m22185i(WarmUpDelegate.TAG, "Compute disk usage, file count(%s), disk usage(%s)", Long.valueOf(jArr[0]), Long.valueOf(jArr[1]));
                    WarmUpReporter warmUpReporter = WarmUpDelegate.sReporter;
                    if (warmUpReporter != null) {
    ...
    public void consumingRequestedQut(final CancellationSignal cancellationSignal) {
        this.mThreadTaskExecutor.arrangeTask(new Runnable() { // from class: com.p1.mobile.backtrace.backtrace.WarmUpDelegate.3
            ...
            public void run() {
                ...
                int iIntValue;
                boolean zWarmUp;
                MatrixLog.m22185i(WarmUpDelegate.TAG, "Going to consume requested QUT.", new Object[0]);
                ...
                if (warmUpInvokerAcquireWarmUpInvoker == null) {
                    ...
                    MatrixLog.m22187w(WarmUpDelegate.TAG, "Failed to acquire warm-up invoker.", new Object[0]);
                    return;
                }
                try {
                    for (String str : strArrConsumeRequestedQut) {
                        ...
                        MatrixLog.m22185i(WarmUpDelegate.TAG, "Consumed requested QUT -> %s, ret = %s.", str, Boolean.valueOf(zWarmUp));
                        CancellationSignal cancellationSignal2 = cancellationSignal;
                        if (cancellationSignal2 != null && cancellationSignal2.isCanceled()) {
                            MatrixLog.m22185i(WarmUpDelegate.TAG, "Consume requested QUT canceled.", new Object[0]);
                            break;
                        }
                    }
                    MatrixLog.m22185i(WarmUpDelegate.TAG, "Consume requested QUT done.", new Object[0]);
                } finally {
    ...
    public synchronized void registerWarmedUpReceiver(WeChatBacktrace.Configuration configuration, WeChatBacktrace.Mode mode) {
        ...
        if (this.mWarmedUpReceiver == null) {
            ...
            MatrixLog.m22185i(TAG, "Register warm-up receiver.", new Object[0]);
    ...
    public void warmingUp(final CancellationSignal cancellationSignal) {
        this.mThreadTaskExecutor.arrangeTask(new Runnable() { // from class: com.p1.mobile.backtrace.backtrace.WarmUpDelegate.1
            @Override // java.lang.Runnable
            public void run() {
                MatrixLog.m22185i(WarmUpDelegate.TAG, "Going to warm up.", new Object[0]);
                ...
                try {
                    if (!new File(WarmUpUtility.validateSavingPath(WarmUpDelegate.this.mConfiguration)).isDirectory()) {
                        MatrixLog.m22187w(WarmUpDelegate.TAG, "Saving path is not a directory.", new Object[0]);
                        ...
                        WarmUpUtility.iterateTargetDirectory(new File(it.next()), cancellationSignal, new FileFilter() { // from class: com.p1.mobile.backtrace.backtrace.WarmUpDelegate.1.1
                            ...
                            public boolean accept(File file) {
                                ...
                                if (file.exists() && !WarmUpDelegate.this.warmUpBlocked(absolutePath, 0) && (absolutePath.endsWith(".so") || absolutePath.endsWith(ShareConstants.ODEX_SUFFIX) || absolutePath.endsWith(".oat") || absolutePath.endsWith(ShareConstants.DEX_SUFFIX))) {
                                    MatrixLog.m22185i(WarmUpDelegate.TAG, "Warming up so %s", absolutePath);
                    ...
                    MatrixLog.m22185i(WarmUpDelegate.TAG, "Warm-up done.", new Object[0]);
                } catch (OperationCanceledException unused) {
                    if (0 != 0) {
                        WarmUpDelegate.this.releaseWarmUpInvoker(null);
                    }
                    MatrixLog.m22185i(WarmUpDelegate.TAG, "Warm-up cancelled.", new Object[0]);
                } catch (Throwable th) {
                    try {
                        MatrixLog.printErrStackTrace(WarmUpDelegate.TAG, th, "", new Object[0]);
                        if (0 != 0) {
