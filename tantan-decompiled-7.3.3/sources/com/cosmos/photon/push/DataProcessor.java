...
public class DataProcessor {
    ...
    private static boolean onMsgArrived(MoMessage moMessage) {
        MDLog.m7450i(LogTag.NOTIFY, "onMsgArrived %s", moMessage);
    ...
    public static void onMsgArrivedInner(MoMessage moMessage) {
        ...
        MDLog.m7450i(LogTag.NOTIFY, "onMsgArrivedInner %s", moMessage);
    }

    private static boolean onPushArrived(MoNotify moNotify) {
        MDLog.m7450i(LogTag.NOTIFY, "onPushArrived %s", moNotify);
        ...
        if (bundleExecuteAction == null || !bundleExecuteAction.getBoolean(ChannelConstant.Keys.KEY_RESULT)) {
            MDLog.m7445e(LogTag.NOTIFY, "onPushArrived contentUri failed");
            if (needAwake) {
                try {
    ...
    public static void onPushArrivedInner(final MoNotify moNotify) {
        ...
        handler.post(new Runnable() { // from class: com.cosmos.photon.push.DataProcessor.1
            ...
            public void run() {
                String pushSource;
                MDLog.m7450i(LogTag.NOTIFY, "onPushArrivedInner %s", moNotify);
                MoNotify moNotify2 = moNotify;
                if (moNotify2.multi == 1) {
                    boolean zExists = IDUtils.exists(moNotify2.f5847id);
                    MoNotify moNotify3 = moNotify;
                    if (zExists) {
                        MDLog.m7450i(LogTag.NOTIFY, "onPushArrivedInner duplicate id : %s ", moNotify3.f5847id);
                        EventLogBody.Builder builder = new EventLogBody.Builder();
                        try {
                    ...
                    IDUtils.saveId(moNotify3.f5847id);
                }
                try {
    ...
    public static boolean onPushClick(MoNotify moNotify) {
        ...
        if (TextUtils.equals(moNotify.toPkg, AppContext.getPackageName())) {
            MDLog.m7450i(LogTag.NOTIFY, "onPushClick %s", moNotify);
            return PhotonPushManager.messageReceiver.onNotificationMessageClicked(moNotify);
        }
        MDLog.m7446e(LogTag.NOTIFY, "onPushClick 代其他APP产生的通知被点击了", moNotify);
        return false;
    }

    public static void onPushClickFromBusiness(MoNotify moNotify) {
        int intExtra;
        MDLog.m7446e(LogTag.NOTIFY, "click push from business", moNotify);
        if (moNotify == null) {
            return;
        }
        Intent uri = null;
    ...
    public static void onPushShowFromBusiness(MoNotify moNotify, boolean z) {
        MDLog.m7446e(LogTag.NOTIFY, "onPushShowFromBusiness", moNotify);
        if (moNotify == null) {
            return;
        }
        ...
        if (!c050.m107319e(AppContext.getContext()).m107322a()) {
    ...
    private static void scheduleNotify(MoNotify moNotify) {
        MDLog.m7450i(LogTag.NOTIFY, "scheduler notify(%s) at %s", moNotify.f5847id, new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date(moNotify.showTime)));
        try {
