                        ...
                        ThreadUtils.execute(new Runnable() { // from class: com.cosmos.photon.push.service.SchedulerPushService.1
                            ...
                            public void run() {
                                ...
                                if (jCurrentTimeMillis > moNotifyFromJson.showExpire) {
                                    MDLog.m7446e(LogTag.NOTIFY, "notify(%s) expired currentTime(%s) > expireTime(%s)", moNotifyFromJson.f5847id, new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date(jCurrentTimeMillis)), new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date(moNotifyFromJson.showExpire)));
                                    return;
                                }
                                if (!AppContext.getMessageReceiver().onNotificationShow(moNotifyFromJson)) {
                                    MDLog.m7450i(LogTag.NOTIFY, "show scheduler notify : %s", moNotifyFromJson.toJson());
