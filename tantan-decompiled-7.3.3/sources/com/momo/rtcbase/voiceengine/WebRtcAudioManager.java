...
public class WebRtcAudioManager {
    ...
    public static class VolumeLogger {
        ...
        public class LogVolumeTask extends TimerTask {
            ...
            public void run() {
                ...
                if (mode == 1) {
                    Logging.m21196d(WebRtcAudioManager.TAG, "STREAM_RING stream volume: " + VolumeLogger.this.audioManager.getStreamVolume(2) + " (max=" + this.maxRingVolume + ")");
                    return;
                }
                if (mode == 3) {
                    Logging.m21196d(WebRtcAudioManager.TAG, "VOICE_CALL stream volume: " + VolumeLogger.this.audioManager.getStreamVolume(0) + " (max=" + this.maxVoiceCallVolume + ")");
                }
            }
        }
    ...
    private void dispose() {
        Logging.m21196d(TAG, "dispose" + WebRtcAudioUtils.getThreadInfo());
        if (this.initialized) {
    ...
    private int getNativeOutputSampleRate() {
        ...
        if (WebRtcAudioUtils.isDefaultSampleRateOverridden()) {
            Logging.m21196d(TAG, "Default sample rate is overriden to " + WebRtcAudioUtils.getDefaultSampleRateHz() + " Hz");
    ...
    private boolean init() {
        Logging.m21196d(TAG, "init" + WebRtcAudioUtils.getThreadInfo());
        if (this.initialized) {
            return true;
        }
        Logging.m21196d(TAG, "audio mode is: " + WebRtcAudioUtils.modeToString(this.audioManager.getMode()));
    ...
    private boolean isAAudioSupported() {
        Logging.m21200w(TAG, "AAudio support is currently disabled on all devices!");
        return false;
    }
    ...
    private boolean isDeviceBlacklistedForOpenSLESUsage() {
        ...
        if (zDeviceIsBlacklistedForOpenSLESUsage) {
            Logging.m21196d(TAG, Build.MODEL + " is blacklisted for OpenSL ES usage!");
        }
    ...
    public static synchronized void setStereoInput(boolean z) {
        Logging.m21200w(TAG, "Overriding default input behavior: setStereoInput(" + z + ')');
        useStereoInput = z;
    }

    public static synchronized void setStereoOutput(boolean z) {
        Logging.m21200w(TAG, "Overriding default output behavior: setStereoOutput(" + z + ')');
