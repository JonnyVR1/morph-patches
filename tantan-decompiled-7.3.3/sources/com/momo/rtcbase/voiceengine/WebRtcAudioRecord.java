...
public class WebRtcAudioRecord {
    ...
    public class AudioRecordThread extends Thread {
        ...
        public void run() {
            ...
            Logging.m21196d(WebRtcAudioRecord.TAG, "AudioRecordThread" + WebRtcAudioUtils.getThreadInfo());
            ...
            while (this.keepAlive) {
                ...
                if (WebRtcAudioRecord.this.audioRecord == null) {
                    Logging.m21197e(WebRtcAudioRecord.TAG, "AudioRecord creat failed:");
                    break;
                }
                ...
                if (i == WebRtcAudioRecord.this.byteBuffer.capacity()) {
                    ...
                    Logging.m21197e(WebRtcAudioRecord.TAG, str);
                    if (i == -3) {
            ...
            try {
                ...
                Logging.m21197e(WebRtcAudioRecord.TAG, "AudioRecord.stop failed: " + e.getMessage());
            }
        }

        public void stopThread() {
            Logging.m21196d(WebRtcAudioRecord.TAG, "stopThread");
            this.keepAlive = false;
        }
    }
    ...
    public WebRtcAudioRecord(long j) {
        Logging.m21196d(TAG, "WebRtcAudioRecord Created:" + WebRtcAudioUtils.getThreadInfo());
    ...
    private boolean enableBuiltInAEC(boolean z) {
        Logging.m21196d(TAG, "enableBuiltInAEC(" + z + ')');
        WebRtcAudioEffects webRtcAudioEffects = this.effects;
        if (webRtcAudioEffects != null) {
            return webRtcAudioEffects.setAEC(z);
        }
        Logging.m21197e(TAG, "Built-in AEC is not supported on this platform");
        return false;
    }

    private boolean enableBuiltInNS(boolean z) {
        Logging.m21196d(TAG, "enableBuiltInNS(" + z + ')');
        WebRtcAudioEffects webRtcAudioEffects = this.effects;
        if (webRtcAudioEffects != null) {
            return webRtcAudioEffects.setNS(z);
        }
        Logging.m21197e(TAG, "Built-in NS is not supported on this platform");
        return false;
    }
    ...
    private int initRecording(int i, int i2) {
        Logging.m21196d(TAG, "initRecording(sampleRate=" + i + ", channels=" + i2 + ")");
        ...
        Logging.m21196d(TAG, "byteBuffer.capacity: " + this.byteBuffer.capacity());
        ...
        Logging.m21196d(TAG, "AudioRecord.getMinBufferSize: " + minBufferSize);
        int iMax = Math.max(minBufferSize * 2, this.byteBuffer.capacity());
        Logging.m21196d(TAG, "bufferSizeInBytes: " + iMax);
        try {
    ...
    private void logMainParameters() {
        Logging.m21196d(TAG, "AudioRecord: session ID: " + this.audioRecord.getAudioSessionId() + ", channels: " + this.audioRecord.getChannelCount() + ", sample rate: " + this.audioRecord.getSampleRate());
    }

    private void logMainParametersExtended() {
        Logging.m21196d(TAG, "AudioRecord: buffer size in frames: " + this.audioRecord.getBufferSizeInFrames());
    }
    ...
    public void reportWebRtcAudioRecordError(String str) {
        Logging.m21197e(TAG, "Run-time recording error: " + str);
    ...
    private void reportWebRtcAudioRecordInitError(String str) {
        Logging.m21197e(TAG, "Init recording error: " + str);
        WebRtcAudioUtils.logAudioState(TAG);
        WebRtcAudioRecordErrorCallback webRtcAudioRecordErrorCallback = errorCallback;
        if (webRtcAudioRecordErrorCallback != null) {
    ...
    private void reportWebRtcAudioRecordStartError(AudioRecordStartErrorCode audioRecordStartErrorCode, String str) {
        Logging.m21197e(TAG, "Start recording error: " + audioRecordStartErrorCode + MinimalPrettyPrinter.DEFAULT_ROOT_VALUE_SEPARATOR + str);
        WebRtcAudioUtils.logAudioState(TAG);
        WebRtcAudioRecordErrorCallback webRtcAudioRecordErrorCallback = errorCallback;
        if (webRtcAudioRecordErrorCallback != null) {
    ...
    public static synchronized void setAudioSource(int i) {
        Logging.m21200w(TAG, "Audio source is changed from: " + audioSource + " to " + i);
        audioSource = i;
    }
    ...
    public static void setMicrophoneMute(boolean z) {
        Logging.m21200w(TAG, "setMicrophoneMute(" + z + ")");
        microphoneMute = z;
    }
    ...
    private boolean startRecording() {
        Logging.m21196d(TAG, "startRecording");
    ...
    private boolean stopRecording() {
        Logging.m21196d(TAG, "stopRecording");
        assertTrue(this.audioThread != null);
        this.audioThread.stopThread();
        if (!ThreadUtils.joinUninterruptibly(this.audioThread, 2000L)) {
            Logging.m21197e(TAG, "Join of AudioRecordJavaThread timed out");
            WebRtcAudioUtils.logAudioState(TAG);
        }
