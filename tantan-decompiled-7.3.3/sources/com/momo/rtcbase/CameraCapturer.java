...
abstract class CameraCapturer implements CameraVideoCapturer {
    ...
    private final CameraSession.CreateSessionCallback createSessionCallback = new CameraSession.CreateSessionCallback() { // from class: com.momo.rtcbase.CameraCapturer.1
        ...
        public void onDone(CameraSession cameraSession) {
            ...
            Logging.m21196d(CameraCapturer.TAG, "Create session done. Switch state: " + CameraCapturer.this.switchState);
            ...
            synchronized (CameraCapturer.this.stateLock) {
                try {
                    ...
                    if (CameraCapturer.this.openAttemptsRemaining <= 0) {
                        Logging.m21200w(CameraCapturer.TAG, "Opening camera failed, passing: " + str);
                        CameraCapturer.this.sessionOpening = false;
                        CameraCapturer.this.stateLock.notifyAll();
                        SwitchState switchState = CameraCapturer.this.switchState;
                        ...
                        Logging.m21200w(CameraCapturer.TAG, "Opening camera failed, retry: " + str);
    ...
    private final CameraSession.Events cameraSessionEventsHandler = new CameraSession.Events() { // from class: com.momo.rtcbase.CameraCapturer.2
            ...
            synchronized (CameraCapturer.this.stateLock) {
                try {
                    if (cameraSession == CameraCapturer.this.currentSession || CameraCapturer.this.currentSession == null) {
                        ...
                        Logging.m21196d(CameraCapturer.TAG, "onCameraClosed from another session.");
                    }
                } catch (Throwable th) {
                    throw th;
                }
            ...
            synchronized (CameraCapturer.this.stateLock) {
                try {
                    if (cameraSession != CameraCapturer.this.currentSession) {
                        Logging.m21200w(CameraCapturer.TAG, "onCameraDisconnected from another session.");
                    } else {
            ...
            synchronized (CameraCapturer.this.stateLock) {
                try {
                    if (cameraSession == CameraCapturer.this.currentSession) {
                        ...
                        Logging.m21200w(CameraCapturer.TAG, "onCameraError from another session: " + str);
                    }
                } catch (Throwable th) {
                    throw th;
                }
            ...
            synchronized (CameraCapturer.this.stateLock) {
                try {
                    if (CameraCapturer.this.currentSession != null) {
                        Logging.m21200w(CameraCapturer.TAG, "onCameraOpening while session was open.");
                    } else {
        ...
        public void onFrameCaptured(CameraSession cameraSession, VideoFrame videoFrame) {
            ...
            synchronized (CameraCapturer.this.stateLock) {
                try {
                    if (cameraSession != CameraCapturer.this.currentSession) {
                        Logging.m21200w(CameraCapturer.TAG, "onFrameCaptured from another session.");
                        return;
                    }
    ...
    public void checkIsOnCameraThread() {
        ...
        Logging.m21197e(TAG, "Check is on camera thread failed.");
    ...
    public void switchCameraInternal(CameraVideoCapturer.CameraSwitchHandler cameraSwitchHandler) {
        Logging.m21196d(TAG, "switchCamera internal");
        ...
        synchronized (this.stateLock) {
            try {
                ...
                Logging.m21196d(TAG, "switchCamera: Stopping session");
                ...
                Logging.m21196d(TAG, "switchCamera done");
            } catch (Throwable th) {
                throw th;
            }
        }
    ...
    public void changeCaptureFormat(int i, int i2, int i3) {
        Logging.m21196d(TAG, "changeCaptureFormat: " + i + BaseSei.f14624X + i2 + "@" + i3);
        synchronized (this.stateLock) {
            stopCapture();
    ...
    public void printStackTrace() {
        ...
        if (thread != null) {
            ...
            if (stackTrace.length > 0) {
                Logging.m21196d(TAG, "CameraCapturer stack trace:");
    ...
    public void startCapture(int i, int i2, int i3) {
        Logging.m21196d(TAG, "startCapture: " + i + BaseSei.f14624X + i2 + "@" + i3);
        ...
        synchronized (this.stateLock) {
            try {
                ...
                Logging.m21200w(TAG, "Session already open");
            } catch (Throwable th) {
                throw th;
            }
        }
    ...
    public void stopCapture() {
        Logging.m21196d(TAG, "Stop capture");
        synchronized (this.stateLock) {
            while (this.sessionOpening) {
                Logging.m21196d(TAG, "Stop capture: Waiting for session to open");
                try {
                    this.stateLock.wait();
                } catch (InterruptedException unused) {
                    Logging.m21200w(TAG, "Stop capture interrupted while waiting for the session to open.");
                    Thread.currentThread().interrupt();
                    return;
                }
            }
            if (this.currentSession != null) {
                Logging.m21196d(TAG, "Stop capture: Nulling session");
                this.cameraStatistics.release();
                this.cameraStatistics = null;
                final CameraSession cameraSession = this.currentSession;
                ...
                Logging.m21196d(TAG, "Stop capture: No session open");
            }
        }
        Logging.m21196d(TAG, "Stop capture done");
    }

    @Override // com.momo.rtcbase.CameraVideoCapturer
    public void switchCamera(final CameraVideoCapturer.CameraSwitchHandler cameraSwitchHandler) {
        Logging.m21196d(TAG, BLiveButtonType.switchCamera);
