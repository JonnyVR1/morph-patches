...
public class MotionCameraFilterAdapter implements MotionCameraFilterJob.IMotionFilterJobListener {
    ...
    private Handler handler = new Handler(Looper.getMainLooper()) { // from class: com.momo.mcamera.mask.motioncamera.MotionCameraFilterAdapter.1
        ...
        public void handleMessage(Message message) {
            ...
            if (message.what == MotionCameraFilterAdapter.this.START_MOVE_WHAT) {
                MDLog.m7443d(MotionCameraFilterAdapter.MotionFilterTAG, "移动move Job , 处理消息, isJobMoveRunning=" + MotionCameraFilterAdapter.this.isJobMoveRunning());
                if (MotionCameraFilterAdapter.this.mmcvInfo == null || MotionCameraFilterAdapter.this.mmcvInfo.m168299n() <= 0 || MotionCameraFilterAdapter.this.isJobMoveRunning() || MotionCameraFilterAdapter.this.currentMoveCount > MotionCameraFilterAdapter.this.moCam_Cnt) {
                    return;
                }
                MDLog.m7443d(MotionCameraFilterAdapter.MotionFilterTAG, "移动move Job , currentMoveCount=" + MotionCameraFilterAdapter.this.currentMoveCount + ", moCam_Cnt=" + MotionCameraFilterAdapter.this.moCam_Cnt);
    ...
    /* JADX INFO: Access modifiers changed from: private */
    public void move(omw omwVar) {
        MDLog.m7443d(MotionFilterTAG, "move,   currentScale=" + this.currentScale);
        float[] fArrM164777e = omwVar.m168295j(0).m164777e();
        float f = (fArrM164777e[2] + fArrM164777e[0]) * 0.5f;
        float f2 = (fArrM164777e[3] + fArrM164777e[1]) * 0.5f;
        float f3 = this.width;
    ...
    public void onScaleChanged(float f) {
        float f2;
        float f3;
        MDLog.m7443d(MotionFilterTAG, "onScaleChanged , 镜头收缩开始的时候回调， scale=" + f + ", width=" + this.width + ", height=" + this.height);
        this.currentScale = f;
        if (this.width == 0 || this.height == 0) {
            return;
        }
        omw omwVar = this.scaleMMCVInfo;
        if (omwVar == null || omwVar.m168299n() <= 0) {
            PointF pointF = this.lastCropCenterPoint;
            f2 = pointF.x;
            f3 = pointF.y;
        } else {
            float[] fArrM164777e = this.scaleMMCVInfo.m168295j(0).m164777e();
            f2 = (fArrM164777e[2] + fArrM164777e[0]) * 0.5f;
            f3 = (fArrM164777e[3] + fArrM164777e[1]) * 0.5f;
            PointF pointF2 = this.lastCropCenterPoint;
            pointF2.x = f2;
            pointF2.y = f3;
        }
    ...
    public void startTimer() {
        ...
        TimerTask timerTask = new TimerTask() { // from class: com.momo.mcamera.mask.motioncamera.MotionCameraFilterAdapter.2
            ...
            public void run() {
                MDLog.m7443d(MotionCameraFilterAdapter.MotionFilterTAG, "大周期时间到了，状态重置，开启运动. moveLoopInterval=" + MotionCameraFilterAdapter.this.moveLoopInterval + ", moCam_moveT=" + MotionCameraFilterAdapter.this.moCam_moveT);
                ...
                if (MotionCameraFilterAdapter.this.moveLoopInterval < MotionCameraFilterAdapter.this.moCam_moveT) {
                    ...
                    MotionCameraFilterAdapter.this.canMoveTimer.schedule(new TimerTask() { // from class: com.momo.mcamera.mask.motioncamera.MotionCameraFilterAdapter.2.1
                        @Override // java.util.TimerTask, java.lang.Runnable
                        public void run() {
                            MDLog.m7443d(MotionCameraFilterAdapter.MotionFilterTAG, "运动上线时间到了，关闭运动.");
                            ...
                            if (MotionCameraFilterAdapter.this.currentScale <= 0.99f) {
                                MDLog.m7443d(MotionCameraFilterAdapter.MotionFilterTAG, "运动上线时间到了，关闭运动.强制做一个收镜动作");
                                MotionCameraFilterAdapter motionCameraFilterAdapter = MotionCameraFilterAdapter.this;
    ...
    public void updateMMCVInfo(omw omwVar, int i, int i2) {
        this.mmcvInfo = omwVar;
        this.width = i;
        this.height = i2;
        if (!this.canMove) {
            MDLog.m7443d(MotionFilterTAG, "被禁止运动了.return.");
            return;
        }
        ...
        if (iM168299n > 0) {
            if (!z && this.jobQueue.isEmpty()) {
                MDLog.m7443d(MotionFilterTAG, "addJob ,有人脸，且之前也没人脸（人脸出现时）");
                this.hasFace = addJob(new MotionCameraFilterJob(this.moCamPushDuration, 1.0f, this.moCamScale));
            }
        } else if (z && this.jobQueue.isEmpty()) {
            MDLog.m7443d(MotionFilterTAG, "addJob ,没人脸，且之前有人脸（人脸消失时）");
            this.hasFace = !addJob(new MotionCameraFilterJob(this.moCamPullDuration, this.moCamScale, 1.0f));
        }
        MDLog.m7443d(MotionFilterTAG, "允许运动, jobQueue.isEmpty()=" + this.jobQueue.isEmpty() + ", isJobMoveRunning()=" + isJobMoveRunning() + ", hasMessages=" + this.handler.hasMessages(this.START_MOVE_WHAT));
        if (!this.jobQueue.isEmpty() && !isJobMoveRunning() && this.currentMoveCount <= this.moCam_Cnt) {
            this.scaleMMCVInfo = omwVar;
            ...
            MDLog.m7443d(MotionFilterTAG, "执行伸缩镜头Job , currentMoveCount=" + this.currentMoveCount + ", limitMoveCount=" + this.moCam_Cnt);
