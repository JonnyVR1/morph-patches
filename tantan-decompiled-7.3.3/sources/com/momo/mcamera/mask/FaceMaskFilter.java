...
public class FaceMaskFilter extends FaceDetectFilter {
    ...
    public FaceMaskFilter(Sticker sticker) {
        ...
        umw umwVar = new umw();
        this.curMMCVImage = umwVar;
        ImageUtils.decodeMMCVImage(umwVar, sticker.getMask().getTexturePath());
        this.model.data = this.curMMCVImage.m196795a();
    ...
    public void setMMCVInfo(omw omwVar) {
        List<Sticker> list;
        if (omwVar == null || this.texture_in == 0) {
            return;
        }
        if (omwVar.m168299n() <= 0 && this.mLastHasFace && (list = this.mStickersList) != null && list.size() > 0) {
            int i = this.mCount + 1;
            this.mCount = i;
