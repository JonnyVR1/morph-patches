...
public class PottImageMovieFilterManager extends jgj0 {
    ...
    private boolean initAllEffectGroup() {
        if (this.allParameterInfo == null) {
            MDLog.m7445e(MDLogTag.FILTER_TAG, "Initall Effect Error , Parameter is null");
            return false;
        }
        ...
        MDLog.m7445e(MDLogTag.FILTER_TAG, " 传入参数错误，未传入用户图片列表，或配置文件有问下周");
        return false;
    }
        ...
        for (int i = 0; i < this.allParameterInfo.imageList.size(); i++) {
            ...
            if (this.templateMap.size() > 0) {
                ...
                if (list == null || list.size() <= 0) {
                    MDLog.m7445e(MDLogTag.FILTER_TAG, "Get template failed , no template is perfect !!");
                    return false;
                }
                ...
                MDLog.m7443d(MDLogTag.FILTER_TAG, "image width = " + i2 + " height=" + i3 + " Index=" + num);
                StringBuilder sb = new StringBuilder("choos template type=");
                sb.append(template.getType());
                sb.append(" name=");
                sb.append(template.getSubFolder());
                MDLog.m7443d(MDLogTag.FILTER_TAG, sb.toString());
    ...
    private void initTailerCfg(List<Template> list, String str, String str2) {
        ...
        for (Template template : list) {
            ...
            if (template.getType().equalsIgnoreCase("map")) {
                ...
                MDLog.m7443d(MDLogTag.FILTER_TAG, "Init map of tailer Template");
                ...
                MDLog.m7443d(MDLogTag.FILTER_TAG, "Init end  of tailer Template");
    ...
    private void initTemplateCfg(List<Template> list, String str) {
        ...
        MDLog.m7443d(MDLogTag.FILTER_TAG, "Init normale Template + " + this.templateMap.toString());
    }
    ...
    private void initTitleCfg(List<Template> list, String str, String str2) {
        if (list.size() > 0) {
            ...
            MDLog.m7443d(MDLogTag.FILTER_TAG, "Init Title Template");
        }
    }
    ...
    public void setProcessErrorListener(jk90 jk90Var) {
        super.setProcessErrorListener(jk90Var);
        Iterator<EsTemplateProgramGroup> it = this.groupList.iterator();
        while (it.hasNext()) {
            it.next().setErrorListener(jk90Var);
        }
    }
