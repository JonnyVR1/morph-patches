    ...
    public class NetworkMonitoCallback extends ConnectivityManager.NetworkCallback {
        ...
        public void onAvailable(Network network) {
            synchronized (NetworkMonitor.this) {
                try {
                    RXLogging.m5746w(NetworkMonitor.TAG, "on available, network: " + network.toString());
        ...
        public void onBlockedStatusChanged(Network network, boolean z) {
            synchronized (NetworkMonitor.this) {
                try {
                    RXLogging.m5746w(NetworkMonitor.TAG, "on network block status changed, network: " + network.toString() + ", blocked: " + z);
        ...
        public void onCapabilitiesChanged(Network network, NetworkCapabilities networkCapabilities) {
            synchronized (NetworkMonitor.this) {
                RXLogging.m5746w(NetworkMonitor.TAG, "on capabilities changed, network: " + network.toString() + ", capabilities: " + networkCapabilities.toString());
            }
        }
        ...
        public void onLinkPropertiesChanged(Network network, LinkProperties linkProperties) {
            synchronized (NetworkMonitor.this) {
                try {
                    RXLogging.m5746w(NetworkMonitor.TAG, "on link properties changed, network: " + network.toString() + ", capabilities: " + linkProperties.toString());
        ...
        public void onLost(Network network) {
            synchronized (NetworkMonitor.this) {
                try {
                    RXLogging.m5746w(NetworkMonitor.TAG, "on lost, network: " + network.toString());
                    if (network.equals(NetworkMonitor.this.cellularNetwork)) {
        ...
        public void onUnavailable() {
            synchronized (NetworkMonitor.this) {
                try {
                    RXLogging.m5746w(NetworkMonitor.TAG, "on unavailable");
    ...
    public int bindSocketToCellularNetwork(int i, String str) {
        ...
        synchronized (this) {
            if (this.cellularNetwork == null || !str.equals(this.cellularIfName)) {
                ...
                RXLogging.m5746w(TAG, "bind socket to network, fd: " + i);
                try {
                    ...
                    RXLogging.m5746w(TAG, "bind socket to network exception: " + e.toString());
                    e.printStackTrace();
                    i2 = -1;
                }
            }
        ...
        synchronized (this) {
            if (this.cellularNetworkRequested.booleanValue()) {
                try {
                    ...
                    RXLogging.m5746w(TAG, "Cancel request cellular network exception: " + e.toString());
                }
            }
        }
    }
        ...
        synchronized (this) {
            if (!this.cellularNetworkRequested.booleanValue()) {
                try {
                    ...
                    RXLogging.m5746w(TAG, "Request cellular network exception: " + e.toString());
                }
            }
        }
    }
