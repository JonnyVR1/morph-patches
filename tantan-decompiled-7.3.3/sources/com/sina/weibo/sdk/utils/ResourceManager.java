...
public class ResourceManager {
    ...
    private static String getAppropriatePathOfDrawable(Context context, String str) {
        ...
        if (TextUtils.isEmpty(str)) {
            LogUtil.m81662e(TAG, "id is NOT correct!");
            return null;
        }
        String currentDpiFolder = getCurrentDpiFolder(context);
        LogUtil.m81661d(TAG, "find Appropriate path...");
        int i = -1;
        int i2 = 0;
        int i3 = -1;
        int i4 = -1;
        ...
        if (i4 <= 0 || i2 <= 0) {
            if (i4 > 0 && i2 < 0) {
                ...
                LogUtil.m81662e(TAG, "Not find the appropriate path for drawable");
            } else {
                i = i2;
            }
    ...
    private static boolean isFileExisted(Context context, String str) {
        ...
        try {
            try {
                inputStreamOpen = context.getAssets().open(str);
                LogUtil.m81661d(TAG, "file [" + str + "] existed");
                if (inputStreamOpen != null) {
                    try {
                        inputStreamOpen.close();
                        return true;
                ...
                LogUtil.m81661d(TAG, "file [" + str + "] NOT existed");
                if (inputStreamOpen != null) {
                    try {
                        inputStreamOpen.close();
                    } catch (IOException e2) {
