...
public class TXCMultAudioTrackPlayer {
    ...
    public synchronized void setAudioMode(Context context, int i) {
        try {
            ...
            if (this.mAudioTrackStarted) {
                TXCLog.m84156w(TAG, "mult-track-player setAudioRoute~");
            } else {
                TXCLog.m84156w(TAG, "mult-track-player do'not setAudioRoute~");
            }
        } catch (Throwable th) {
            throw th;
        }
    ...
    public void start() {
        String str = TAG;
        TXCLog.m84156w(str, "mult-track-player start!");
        if (this.mIsStarted) {
            TXCLog.m84149e(str, "mult-track-player can not start because of has started!");
            return;
        }
        if (this.mSampleRate == 0 || this.mChannel == 0) {
            TXCLog.m84149e(str, "strat mult-track-player failed with invalid audio info , samplerate:" + this.mSampleRate + ", channels:" + this.mChannel);
            return;
        }
            ...
            AudioTrackThread audioTrackThread = new AudioTrackThread("AUDIO_TRACK") { // from class: com.tencent.liteav.audio.impl.Play.TXCMultAudioTrackPlayer.1
                ...
                public void run() {
                    try {
                        ...
                        TXCLog.m84152i(TXCMultAudioTrackPlayer.TAG, "create audio track, samplerate:" + TXCMultAudioTrackPlayer.this.mSampleRate + ", channels:" + TXCMultAudioTrackPlayer.this.mChannel + ", bits:" + TXCMultAudioTrackPlayer.this.mBits + " mMinBufferLength:" + minBufferSize);
                        try {
                            audioTrack.play();
                            ...
                            try {
                                ...
                                TXCLog.m84150e(TXCMultAudioTrackPlayer.TAG, "stop AudioTrack failed.", e);
                            }
                            TXCLog.m84149e(TXCMultAudioTrackPlayer.TAG, "mult-player thread stop finish!");
                        } catch (Exception e2) {
                            TXCLog.m84150e(TXCMultAudioTrackPlayer.TAG, "start play failed.", e2);
                        }
                    } catch (Exception e3) {
                        TXCLog.m84150e(TXCMultAudioTrackPlayer.TAG, "create AudioTrack failed.", e3);
                    }
                }
            };
        ...
        TXCLog.m84156w(str, "mult-track-player thread start finish!");
    }

    public void stop() {
        String str = TAG;
        TXCLog.m84156w(str, "mult-track-player stop!");
        if (!this.mIsStarted) {
            TXCLog.m84156w(str, "mult-track-player can not stop because of not started yet!");
            return;
        }
        ...
        this.mAudioTrackStarted = false;
        this.mIsStarted = false;
        TXCLog.m84156w(str, "mult-track-player stop finish!");
    }
}
