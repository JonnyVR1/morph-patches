...
public class TXCFLVDownloader extends TXIStreamDownloader {
    ...
    public TXCFLVDownloader(Context context, TXCFLVDownloader tXCFLVDownloader) {
        ...
        TXCLog.m84152i("network.TXCFLVDownloader", "new multi flv download " + this);
    }
    ...
    private void connect() throws Exception {
        ...
        TXCLog.m84152i("network.TXCFLVDownloader", "receive flvSessionKey ".concat(headerField));
    ...
    private void onRecvFirstAudioData() {
        ...
        TXCLog.m84152i("network.TXCFLVDownloader", "onRecvData: receive first audio with ts " + this.mStats.firstAudioTS);
    }
    ...
    private void onRecvFirstVideoData() {
        ...
        TXCLog.m84152i("network.TXCFLVDownloader", "onRecvData: receive first video with ts " + this.mStats.firstVideoTS);
    }
    ...
    public void processMsgConnect() {
        try {
            ...
            TXCLog.m84149e("network.TXCFLVDownloader", "file not found, reconnect");
            e.printStackTrace();
            postReconnectMsg();
        } catch (Error e2) {
            TXCLog.m84149e("network.TXCFLVDownloader", "error, reconnect");
            e2.printStackTrace();
            postReconnectMsg();
        } catch (SocketTimeoutException unused) {
            TXCLog.m84149e("network.TXCFLVDownloader", "socket timeout, reconnect");
            postReconnectMsg();
        } catch (Exception e3) {
            TXCLog.m84149e("network.TXCFLVDownloader", "exception, reconnect");
            e3.printStackTrace();
            postReconnectMsg();
        }
    }
    ...
    public void processMsgDisConnect() {
        try {
            ...
            TXCLog.m84150e("network.TXCFLVDownloader", "disconnect failed.", e);
        }
    ...
    public void processMsgReconnect() {
        ...
        TXCLog.m84152i("network.TXCFLVDownloader", "ignore processMsgReconnect when start multi stream switch" + this);
        TXIStreamDownloader.InterfaceC14298a interfaceC14298a = this.mRestartListener;
        if (interfaceC14298a != null) {
            interfaceC14298a.onOldStreamStop();
        }
    }
        ...
        if (inputStream != null) {
            try {
                ...
                if (i > 0) {
                    ...
                    if (!this.mRecvData) {
                        TXCLog.m84156w("network.TXCFLVDownloader", "flv play receive first data " + this);
                        this.mRecvData = true;
                    }
                    ...
                    if (iNativeParseData > 1048576) {
                        TXCLog.m84149e("network.TXCFLVDownloader", "flv play parse frame: " + iNativeParseData + " > 1048576,sart reconnect");
                        postReconnectMsg();
                        return;
                    }
                } else if (i < 0) {
                    TXCLog.m84156w("network.TXCFLVDownloader", "http read: " + i + " < 0, start reconnect");
                    postReconnectMsg();
                    return;
                }
                Handler handler = this.mFlvHandler;
                ...
                TXCLog.m84156w("network.TXCFLVDownloader", "eof exception start reconnect");
                postReconnectMsg();
            } catch (Error e) {
                TXCLog.m84149e("network.TXCFLVDownloader", "error");
                e.printStackTrace();
                this.mInputStream = null;
                this.mConnection = null;
            } catch (SocketException unused2) {
                TXCLog.m84156w("network.TXCFLVDownloader", "socket exception start reconnect");
                postReconnectMsg();
            } catch (SocketTimeoutException unused3) {
                TXCLog.m84156w("network.TXCFLVDownloader", "socket timeout start reconnect");
                postReconnectMsg();
            } catch (SSLException unused4) {
                TXCLog.m84156w("network.TXCFLVDownloader", "ssl exception start reconnect");
                postReconnectMsg();
            } catch (Exception e2) {
                TXCLog.m84149e("network.TXCFLVDownloader", "exception");
                e2.printStackTrace();
                this.mInputStream = null;
                this.mConnection = null;
            }
    ...
    private void reconnect() {
        ...
        if (i >= this.connectRetryLimit) {
            TXCLog.m84149e("network.TXCFLVDownloader", "reconnect all times retried, send failed event ");
            sendNotifyEvent(-2301);
            return;
        }
        this.connectRetryTimes = i + 1;
        TXCLog.m84152i("network.TXCFLVDownloader", "reconnect retry time:" + this.connectRetryTimes + ", limit:" + this.connectRetryLimit);
        processMsgConnect();
    ...
    public void onRecvAudioData(byte[] bArr, int i, int i2, int i3) {
        if (!this.mHasReceivedFirstAudio) {
            this.mHasReceivedFirstAudio = true;
            this.mStats.firstAudioTS = TXCTimeUtil.getTimeTick();
            TXCLog.m84152i("network.TXCFLVDownloader", "receive first audio with ts " + this.mStats.firstAudioTS);
        }
    ...
    public void onRecvVideoData(byte[] bArr, int i, long j, long j2, int i2) {
        if (!this.mHasReceivedFirstVideo) {
            this.mHasReceivedFirstVideo = true;
            this.mStats.firstVideoTS = TXCTimeUtil.getTimeTick();
            TXCLog.m84152i("network.TXCFLVDownloader", "receive first video with ts " + this.mStats.firstVideoTS);
        }
    ...
    public void startDownload(Vector<C14312e> vector, boolean z, boolean z2, boolean z3, boolean z4) {
        ...
        this.mPlayUrl = vector.get(0).f60378a;
        TXCLog.m84152i("network.TXCFLVDownloader", "start pull with url " + this.mPlayUrl);
        startInternal();
    }
    ...
    public void stopDownload() {
        if (this.mIsRunning) {
            ...
            TXCLog.m84152i("network.TXCFLVDownloader", "stop pull");
            try {
                Handler handler = this.mFlvHandler;
                if (handler != null) {
                ...
                TXCLog.m84150e("network.TXCFLVDownloader", "stop download failed.", e);
            }
        }
    }
    ...
    public TXCFLVDownloader(Context context) {
        ...
        downloadStats.dnsTS = 0L;
        downloadStats.startTS = TXCTimeUtil.getTimeTick();
        TXCLog.m84152i("network.TXCFLVDownloader", "new flv download " + this);
    }
}
