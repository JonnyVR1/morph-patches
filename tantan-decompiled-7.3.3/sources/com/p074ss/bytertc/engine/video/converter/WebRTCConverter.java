package com.p074ss.bytertc.engine.video.converter;
...
public class WebRTCConverter {
    ...
    public static VideoFrame convertByteI420Frame2WebrtcI420Frame(IVideoFrame iVideoFrame) {
        ...
        iVideoFrame.addRef();
        try {
            return new VideoFrame(JavaI420Buffer.wrap(iVideoFrame.width(), iVideoFrame.height(), iVideoFrame.planeData(0), iVideoFrame.planeStride(0), iVideoFrame.planeData(1), iVideoFrame.planeStride(1), iVideoFrame.planeData(2), iVideoFrame.planeStride(2), new xrp0(iVideoFrame)), iVideoFrame.rotation().value(), iVideoFrame.timestampUs());
        } catch (IllegalArgumentException e) {
            iVideoFrame.releaseRef();
            LogUtil.m82084e(TAG, "Frame convert failed: " + e.getMessage());
            return null;
        }
    }
    ...
    public static VideoFrame convertByteRGBAFrame2WebrtcI420Frame(IVideoFrame iVideoFrame) {
        ...
        return new VideoFrame(JavaI420Buffer.wrap(iWidth, iHeight, byteBufferAllocateDirect, i, byteBufferAllocateDirect2, i2, byteBufferAllocateDirect3, i2, new xrp0(iVideoFrame)), iVideoFrame.rotation().value(), iVideoFrame.timestampUs());
    }
    ...
    public static VideoFrame convertByteTexFrame2WebrtcTexFrame(IVideoFrame iVideoFrame, Looper looper) {
        ...
        iVideoFrame.addRef();
        return new VideoFrame(new TextureBufferImpl(iVideoFrame.width(), iVideoFrame.height(), type, iVideoFrame.textureId(), matrixConvertMatrixToAndroidGraphicsMatrix, new Handler(looper), new YuvConverter(), new xrp0(iVideoFrame)), iVideoFrame.rotation().value(), iVideoFrame.timestampUs());
    }
}
