...
public class NativeRangeAudio implements IRangeAudio {
    ...
    @Override // com.p074ss.bytertc.engine.audio.IRangeAudio
    public void enableRangeAudio(boolean z) {
        this.mJniReadLock.lock();
        try {
            long j = this.mNaiveInstance;
            if (j == 0) {
                LogUtil.m82084e(TAG, "native RangeAudio is invalid, enableRangeAudio failed.");
            } else {
    ...
    public int setAttenuationModel(AttenuationType attenuationType, float f) {
        ...
        try {
            ...
            LogUtil.m82084e(TAG, "native range audio is invalid, setAttenuationModel failed.");
            return -1;
        } finally {
            this.mJniReadLock.unlock();
        }
    ...
    public void setNoAttenuationFlags(List<String> list) {
        ...
        try {
            if (this.mNaiveInstance == 0) {
                LogUtil.m82084e(TAG, "native range audio is invalid, setNoAttenuationFlags failed.");
            } else {
    ...
    public int updatePosition(Position position) {
        ...
        try {
            long j = this.mNaiveInstance;
            if (j != 0) {
                return nativeUpdatePosition(j, position.f56469x, position.f56470y, position.f56471z);
            }
            LogUtil.m82084e(TAG, "native RangeAudio is invalid, updatePosition failed.");
            return -1;
        } finally {
            this.mJniReadLock.unlock();
        }
    ...
    public int updateReceiveRange(ReceiveRange receiveRange) {
        ...
        try {
            ...
            LogUtil.m82084e(TAG, "native RangeAudio is invalid, updateReceiveRange failed.");
            return -1;
        } finally {
            this.mJniReadLock.unlock();
        }
