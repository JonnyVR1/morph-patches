...
public class TextureEglRenderer extends EglRenderer implements TextureView.SurfaceTextureListener {
    ...
    public static /* synthetic */ void m82074i(TextureEglRenderer textureEglRenderer, TextureView textureView) {
    ...
    public static /* synthetic */ void m82075j(TextureEglRenderer textureEglRenderer, VideoFrame videoFrame, CountDownLatchI420Buffer countDownLatchI420Buffer) {
        super.onFrame(videoFrame);
        countDownLatchI420Buffer.release();
    }

    /* JADX INFO: renamed from: k */
    public static /* synthetic */ void m82076k(Runnable runnable, CountDownLatch countDownLatch) {
        runnable.run();
        try {
    ...
    private boolean postOnMainThreadAndLock(final Runnable runnable, final CountDownLatch countDownLatch) {
        return this.mainHandler.post(new Runnable() { // from class: l.hti0
            @Override // java.lang.Runnable
            public final void run() {
                TextureEglRenderer.m82076k(runnable, countDownLatch);
            }
        });
    }
    ...
    public void bind(final TextureView textureView, VideoFrameRender.SurfaceLifecycleCallback surfaceLifecycleCallback) {
        ...
        runOnUIThread(new Runnable() { // from class: l.jti0
            @Override // java.lang.Runnable
            public final void run() {
                TextureEglRenderer.m82074i(this.f122593a, textureView);
            }
        });
    }

    @Override // com.p074ss.bytertc.base.media.EglRenderer
    public void init(EglBase.Context context, int[] iArr, RendererCommon.GlDrawer glDrawer) {
        if (!this.mHasInitAtomic.compareAndSet(false, true)) {
            wtq0.m207906a("TextureEglRenderer has already init!!!!!!");
        } else {
    ...
    @Override // com.p074ss.bytertc.base.media.EglRenderer, com.p074ss.bytertc.base.media.VideoSink
    public void onFrame(final VideoFrame videoFrame) {
        if (this.mHasInitAtomic.get() && this.mHasBindAtomic.get() && this.mTextureViewRef != null) {
            ...
            if (postOnMainThreadAndLock(new Runnable() { // from class: l.iti0
                @Override // java.lang.Runnable
                public final void run() {
                    TextureEglRenderer.m82075j(this.f116845a, videoFrame, countDownLatchI420Buffer);
                }
    ...
    public void onSurfaceTextureAvailable(SurfaceTexture surfaceTexture, int i, int i2) {
        ...
        LogUtil.m82083d(TAG, "onSurfaceTextureAvailable");
    ...
    public boolean onSurfaceTextureDestroyed(SurfaceTexture surfaceTexture) {
        ThreadUtils.checkIsOnMainThread();
        LogUtil.m82083d(TAG, "onSurfaceTextureDestroyed");
        CountDownLatch countDownLatch = new CountDownLatch(1);
        releaseEglSurface(new bah0(countDownLatch));
    ...
    public void onSurfaceTextureSizeChanged(SurfaceTexture surfaceTexture, int i, int i2) {
        LogUtil.m82083d(TAG, "onSurfaceTextureSizeChanged");
        synchronized (this.mSurfaceTextureListenerLock) {
            try {
                TextureView.SurfaceTextureListener surfaceTextureListener = this.mSurfaceTextureListener;
                if (surfaceTextureListener != null) {
