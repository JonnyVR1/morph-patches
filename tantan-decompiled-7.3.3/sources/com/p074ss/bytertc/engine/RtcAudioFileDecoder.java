...
class RtcAudioFileDecoder {
    ...
    private HttpURLConnection f56460oc;
    private int mSampleRate = 0;
    private int mChannels = 0;

    @CalledByNative
    public RtcAudioFileDecoder() {
        RXLogging.m5742e(TAG, "AudioMix RtcAudioFileDecoder");
    }
    ...
    private boolean checkInfoChange() {
        try {
            ...
            e.printStackTrace();
            RXLogging.m5742e(TAG, "Error when checking file's new format");
            return false;
        }
    }
    ...
    private boolean checkUrlEncoded(String str) {
        try {
            ...
            e.printStackTrace();
            RXLogging.m5742e(TAG, "Error when releasing audio file stream");
            return false;
        }
    }

    private String encodeUrl(String str) {
        RXLogging.m5742e(TAG, "encodedUrl");
        try {
            URL url = new URL(str);
    ...
    private boolean isAvailableOnlineURL(String str) {
        boolean z;
        RXLogging.m5742e(TAG, "isAvailableOnlineURL");
        this.f56460oc = null;
        try {
            HttpURLConnection httpURLConnection = (HttpURLConnection) new URL(str).openConnection();
            this.f56460oc = httpURLConnection;
            httpURLConnection.setUseCaches(false);
            this.f56460oc.setConnectTimeout(TXLiteAVCode.WARNING_START_CAPTURE_IGNORED);
            RXLogging.m5742e(TAG, "connect done....");
            int responseCode = this.f56460oc.getResponseCode();
            if (200 == responseCode) {
                z = true;
            } else {
                RXLogging.m5742e(TAG, "url is not available, error:" + responseCode);
                z = false;
            }
            InputStream inputStream = this.f56460oc.getInputStream();
            if (inputStream != null) {
                inputStream.close();
            }
            return z;
            ...
            e.printStackTrace();
            RXLogging.m5742e(TAG, "++Error when test online url: " + e.getMessage());
            return false;
        } finally {
            HttpURLConnection httpURLConnection2 = this.f56460oc;
    ...
    public void disConnectURL() {
        RXLogging.m5742e(TAG, "disConnectURL");
        HttpURLConnection httpURLConnection = this.f56460oc;
    ...
    public long getCurrentFilePosition() {
        try {
            ...
            e.printStackTrace();
            RXLogging.m5742e(TAG, "Error when getCurrentFilePosition");
            return 0L;
        }
    }
    ...
    public boolean init(String str, int i) {
        try {
            RXLogging.m5744i(TAG, "Try to decode audio file : " + str);
            ...
            if (i + 1 > trackCount) {
                RXLogging.m5742e(TAG, "useTrack > trackCount");
                return false;
            }
            this.mChannels = 0;
            int i3 = 0;
            ...
            if (mediaCodec == null) {
                RXLogging.m5742e(TAG, "mMediaCodec is null");
                return false;
            }
            mediaCodec.start();
            ...
            e.printStackTrace();
            RXLogging.m5742e(TAG, "Error when creating audio file decode, error:" + e.getMessage());
            RXLogging.m5742e(TAG, "stack track: " + Log.getStackTraceString(e));
            return false;
        }
    }

    /* JADX WARN: Code duplicated, block: B:60:0x011d A[Catch: Exception -> 0x002f, TryCatch #0 {Exception -> 0x002f, blocks: (B:3:0x0008, B:5:0x000c, B:8:0x0012, B:10:0x001a, B:12:0x0020, B:14:0x0028, B:19:0x0035, B:21:0x003a, B:23:0x004a, B:24:0x004c, B:25:0x0059, B:27:0x005d, B:33:0x0074, B:35:0x0078, B:37:0x007e, B:38:0x0080, B:42:0x0095, B:43:0x009f, B:45:0x00a4, B:46:0x00c9, B:49:0x00e1, B:48:0x00cf, B:50:0x00e8, B:52:0x00f1, B:54:0x00fd, B:56:0x0109, B:58:0x0113, B:60:0x011d, B:61:0x014d, B:62:0x0157, B:63:0x015c), top: B:68:0x0008 }] */
    /* JADX WARN: Instruction removed from duplicated block: B:60:0x011d, please report this as an issue */
    @CalledByNative
        ...
        try {
            ...
            if (mediaCodec != null && this.mExtractor != null) {
                if (!this.eoInputStream && (iDequeueInputBuffer = mediaCodec.dequeueInputBuffer(0L)) >= 0) {
                    ...
                    int sampleData = inputBuffer != null ? this.mExtractor.readSampleData(inputBuffer, 0) : 0;
                    if (sampleData <= 0) {
                    ...
                    if (iDequeueOutputBuffer == -3 || iDequeueOutputBuffer == -2) {
                        ...
                        if (i6 >= 100) {
                            ...
                            if (str.toLowerCase().contains("meizu")) {
                                RXLogging.m5742e(TAG, "EAGAIN count=" + this.mRetryCount + " presentationTimeUs=" + bufferInfo.presentationTimeUs + " totalUs=" + this.mFileLength + " Force EOS");
                                this.eoOutputStream = true;
                                this.mRetryCount = 0;
                            } else {
        ...
        try {
            ...
            if (mediaExtractor != null && this.mMediaCodec != null) {
                ...
                if (this.mSampleRate != trackFormat.getInteger("sample-rate")) {
                    RXLogging.m5742e(TAG, "mSampleRate = " + this.mSampleRate + ", used_SampleRate = " + trackFormat.getInteger("sample-rate"));
                    return false;
                }
                long sampleTime = this.mExtractor.getSampleTime();
                RXLogging.m5742e(TAG, "++current_postion = " + sampleTime);
            ...
            RXLogging.m5742e(TAG, "mExtractor or mMediaCodec is null, mExtractor = " + this.mExtractor + ", mMediaCodec = " + this.mMediaCodec);
            return false;
        } catch (Exception e) {
            e.printStackTrace();
            RXLogging.m5742e(TAG, "Error when selectTrack");
            return false;
        }
    }
    ...
    public void setCurrentFilePosition(long j) {
        if (this.eoOutputStream) {
            try {
                ...
                e.printStackTrace();
                RXLogging.m5742e(TAG, "Error when setCurrentFilePosition, mMediaCodec.flush");
            }
        }
        try {
            ...
            e2.printStackTrace();
            RXLogging.m5742e(TAG, "Error when setCurrentFilePosition, mExtractor.seekTo");
        }
    }
    ...
    public void uninit() {
        try {
            ...
            e.printStackTrace();
            RXLogging.m5742e(TAG, "Error when releasing audio file stream" + e.getMessage());
        }
        this.eoOutputStream = false;
        this.eoInputStream = false;
    }
