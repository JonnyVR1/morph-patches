...
public final class WebRtcAudioUtils {
    ...
    public static boolean isPermissionGranted() {
        return j26.m143188a(ContextUtils.getApplicationContext(), "android.permission.RECORD_AUDIO") == 0;
    }
    ...
    private static void logAudioDeviceInfo(String str, AudioManager audioManager) {
        ...
        RXLogging.m5744i(str, "Audio Devices: ");
        for (AudioDeviceInfo audioDeviceInfo : devices) {
            ...
            RXLogging.m5744i(str, sb.toString());
        }
    }
    ...
    private static void logAudioStateBasic(String str, AudioManager audioManager) {
        RXLogging.m5744i(str, "Audio State: audio mode: " + modeToString(audioManager.getMode()) + ", has mic: " + hasMicrophone() + ", mic muted: " + audioManager.isMicrophoneMute() + ", music active: " + audioManager.isMusicActive() + ", speakerphone: " + audioManager.isSpeakerphoneOn() + ", BT SCO: " + audioManager.isBluetoothScoOn());
    }

    private static void logAudioStateVolume(String str, AudioManager audioManager) {
        int[] iArr = {0, 3, 2, 4, 5, 1};
        RXLogging.m5744i(str, "Audio State: ");
        boolean zIsVolumeFixed = isVolumeFixed(audioManager);
        RXLogging.m5744i(str, "  fixed volume=" + zIsVolumeFixed);
        if (zIsVolumeFixed) {
            return;
        }
        for (int i = 0; i < 6; i++) {
    ...
    public static synchronized void setWebRtcBasedAutomaticGainControl(boolean z) {
        RXLogging.m5746w(TAG, "setWebRtcBasedAutomaticGainControl() is deprecated");
    }
    ...
    public static synchronized boolean useWebRtcBasedAcousticEchoCanceler() {
        try {
            if (useWebRtcBasedAcousticEchoCanceler) {
                RXLogging.m5746w(TAG, "Overriding default behavior; now using WebRTC AEC!");
            }
        } catch (Throwable th) {
            throw th;
        }
    ...
    public static synchronized boolean useWebRtcBasedNoiseSuppressor() {
        try {
            if (useWebRtcBasedNoiseSuppressor) {
                RXLogging.m5746w(TAG, "Overriding default behavior; now using WebRTC NS!");
            }
        } catch (Throwable th) {
            throw th;
        }
