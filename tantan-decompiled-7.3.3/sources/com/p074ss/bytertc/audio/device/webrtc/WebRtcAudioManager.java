...
import com.p074ss.bytertc.audio.device.webrtc.observer.MicMuteChangedObserver;
import com.p074ss.bytertc.audio.device.webrtc.observer.VolumeChangedObserver;
...
public class WebRtcAudioManager implements VolumeChangedObserver.OnVolumeChangedListener, MicMuteChangedObserver.OnMicMuteChangedListener {
        ...
        public void onCallStateChanged(int i, String str) {
            synchronized (WebRtcAudioManager.this) {
                try {
                    ...
                    this.callState = i;
                    RXLogging.m5744i(WebRtcAudioManager.TAG, "onCallStateChanged, from: " + i2 + ", to: " + i);
                    if (i != 0) {
                        if ((i == 1 || i == 2) && i2 == 0) {
                            if (WebRtcAudioManager.this.nativeAudioManager != 0) {
                                RXLogging.m5744i(WebRtcAudioManager.TAG, "startPhoneCall");
                                ...
                                RXLogging.m5742e(WebRtcAudioManager.TAG, "nativeAudioManager is null");
                            }
                        }
                    } else if (i2 == 1 || i2 == 2) {
                        if (WebRtcAudioManager.this.nativeAudioManager != 0) {
                            RXLogging.m5744i(WebRtcAudioManager.TAG, "endPhoneCall");
    ...
    public WebRtcAudioManager(long j) {
        synchronized (this) {
            RXLogging.m5744i(TAG, "ctor(" + this + ")" + WebRtcAudioUtils.getThreadInfo());
    ...
    private int SetAudioMode(int i) {
        RXLogging.m5744i(TAG, "SetAudioMode enter, modeInvalid: " + this.modeInvalid + ", curMode: " + this.curMode + ", setMode: " + i);
        try {
            if (i != this.curMode || this.modeInvalid) {
                this.audioManager.setMode(i);
                RXLogging.m5746w(TAG, "set mode from: " + this.curMode + ", to: " + i + ", modeInvalid is: " + this.modeInvalid);
            }
            int mode = getMode();
            this.curMode = mode;
            this.modeInvalid = mode != i;
            RXLogging.m5744i(TAG, "SetAudioMode leave, modeInvalid: " + this.modeInvalid + ", curMode: " + this.curMode + ", setMode: " + i);
            return 0;
        } catch (Exception e) {
            e.printStackTrace();
            return -1;
    ...
    private String getAudioDeviceInfo() {
        try {
            ...
            RXLogging.m5742e(TAG, stringWriter.toString());
    ...
    private int getNativeOutputSampleRate() {
        if (WebRtcAudioUtils.runningOnEmulator()) {
            RXLogging.m5744i(TAG, "Running emulator, overriding sample rate to 8 kHz.");
            return TXRecordCommon.AUDIO_SAMPLERATE_8000;
        }
        if (WebRtcAudioUtils.isDefaultSampleRateOverridden()) {
            RXLogging.m5744i(TAG, "Default sample rate is overriden to " + WebRtcAudioUtils.getDefaultSampleRateHz() + " Hz");
            return WebRtcAudioUtils.getDefaultSampleRateHz();
        }
        int defaultSampleRateHz = WebRtcAudioUtils.getDefaultSampleRateHz();
        RXLogging.m5744i(TAG, "Sample rate is set to " + defaultSampleRateHz + " Hz");
    ...
    private boolean init() {
        synchronized (this) {
            try {
                RXLogging.m5744i(TAG, "init(" + this + ")" + WebRtcAudioUtils.getThreadInfo());
                if (this.initialized) {
                    return true;
                }
                RXLogging.m5744i(TAG, "audio mode is: " + WebRtcAudioUtils.modeToString(this.audioManager.getMode()));
                ...
                try {
                    ...
                    RXLogging.m5742e(TAG, stringWriter2.toString());
                }
    ...
    private boolean isDeviceBlacklistedForOpenSLESUsage() {
        ...
        if (zDeviceIsBlacklistedForOpenSLESUsage) {
            RXLogging.m5744i(TAG, Build.MODEL + " is blacklisted for OpenSL ES usage!");
        }
        ...
        synchronized (this) {
            try {
                if (this.nativeAudioManager != 0) {
                    RXLogging.m5744i(TAG, "dixing: notifyPhoneStateListenerRegistered: " + z + ", " + this.nativeAudioManager);
        ...
        new Handler(Looper.getMainLooper()).post(new Runnable() { // from class: com.ss.bytertc.audio.device.webrtc.WebRtcAudioManager.1
            ...
            public void run() {
                try {
                    ...
                    RXLogging.m5744i(WebRtcAudioManager.TAG, "registerPhoneStateListener success with event: " + i);
                } catch (Throwable th) {
                    th.printStackTrace();
                    RXLogging.m5742e(WebRtcAudioManager.TAG, "registerPhoneStateListener (" + i + "), error: " + th.getCause() + ", " + th.getMessage());
                }
            }
        });
    }
    ...
    public static synchronized void setStereoInput(boolean z) {
        RXLogging.m5746w(TAG, "Overriding default input behavior: setStereoInput(" + z + ')');
        useStereoInput = z;
    }

    public static synchronized void setStereoOutput(boolean z) {
        RXLogging.m5746w(TAG, "Overriding default output behavior: setStereoOutput(" + z + ')');
        useStereoOutput = z;
    }
    ...
    private void terminate() {
        ...
        synchronized (this) {
            RXLogging.m5744i(TAG, "terminate(" + this + ")" + WebRtcAudioUtils.getThreadInfo());
