...
public class SessionReportingCoordinator {
    ...
    public static /* synthetic */ void m16802a(SessionReportingCoordinator sessionReportingCoordinator, CrashlyticsReport.Session.Event event, EventMetadata eventMetadata, boolean z) {
        sessionReportingCoordinator.getClass();
        Logger.getLogger().m16776d("disk worker: log non-fatal event to persistence");
    ...
    private CrashlyticsReport.Session.Event addLogsCustomKeysAndEventKeysToEvent(CrashlyticsReport.Session.Event event, LogFileManager logFileManager, UserMetadata userMetadata, Map<String, String> map) {
        ...
        if (logString != null) {
            ...
            Logger.getLogger().m16782v("No log data to include with this event.");
        }
    ...
    private static CrashlyticsReport.ApplicationExitInfo convertApplicationExitInfo(ApplicationExitInfo applicationExitInfo) {
        ...
        try {
            ...
            Logger.getLogger().m16784w("Could not get input trace in application exit info: " + applicationExitInfo.toString() + " Error: " + e);
        }
    ...
    private ApplicationExitInfo findRelevantApplicationExitInfo(String str, List<ApplicationExitInfo> list) {
        ...
        while (it.hasNext()) {
            ApplicationExitInfo applicationExitInfoM107616a = c1j.m107616a(it.next());
            if (applicationExitInfoM107616a.getTimestamp() < startTimestampMillis) {
                return null;
            }
            if (applicationExitInfoM107616a.getReason() == 6) {
                return applicationExitInfoM107616a;
            }
        }
        return null;
    }
    ...
    private static List<CrashlyticsReport.CustomAttribute> getSortedCustomAttributes(@NonNull Map<String, String> map) {
        ...
        Collections.sort(arrayList, new Comparator() { // from class: l.ite0
    ...
    public boolean onReportSendComplete(@NonNull Task<CrashlyticsReportWithSessionId> task) {
        if (!task.mo15431p()) {
            Logger.getLogger().m16785w("Crashlytics report could not be enqueued to DataTransport", task.mo15426k());
            return false;
        }
        CrashlyticsReportWithSessionId crashlyticsReportWithSessionIdMo15427l = task.mo15427l();
        Logger.getLogger().m16776d("Crashlytics report successfully enqueued to DataTransport: " + crashlyticsReportWithSessionIdMo15427l.getSessionId());
        File reportFile = crashlyticsReportWithSessionIdMo15427l.getReportFile();
        if (reportFile.delete()) {
            Logger.getLogger().m16776d("Deleted report file: " + reportFile.getPath());
            return true;
        }
        Logger.getLogger().m16784w("Crashlytics could not delete report file: " + reportFile.getPath());
        return true;
    }
    ...
    private void persistEvent(@NonNull Throwable th, @NonNull Thread thread, @NonNull String str, @NonNull final EventMetadata eventMetadata, boolean z) {
        ...
        if (z) {
            ...
            this.crashlyticsWorkers.diskWrite.submit(new Runnable() { // from class: l.hte0
                @Override // java.lang.Runnable
                public final void run() {
                    SessionReportingCoordinator.m16802a(this.f111526a, eventAddMetaDataToEvent, eventMetadata, zEquals);
                }
            });
        }
    }
    ...
    public void finalizeSessionWithNativeEvent(@NonNull String str, @NonNull List<NativeSessionFile> list, CrashlyticsReport.ApplicationExitInfo applicationExitInfo) {
        Logger.getLogger().m16776d("SessionReportingCoordinator#finalizeSessionWithNativeEvent");
        ArrayList arrayList = new ArrayList();
    ...
    public void persistFatalEvent(@NonNull Throwable th, @NonNull Thread thread, @NonNull String str, long j) {
        Logger.getLogger().m16782v("Persisting fatal event for session " + str);
    ...
    public void persistNonFatalEvent(@NonNull Throwable th, @NonNull Thread thread, @NonNull EventMetadata eventMetadata) {
        Logger.getLogger().m16782v("Persisting non-fatal event for session " + eventMetadata.getSessionId());
    ...
    public void persistRelevantAppExitInfoEvent(String str, List<ApplicationExitInfo> list, LogFileManager logFileManager, UserMetadata userMetadata) {
        ...
        if (applicationExitInfoFindRelevantApplicationExitInfo == null) {
            Logger.getLogger().m16782v("No relevant ApplicationExitInfo occurred during session: " + str);
            return;
        }
        ...
        Logger.getLogger().m16776d("Persisting anr for session " + str);
    ...
    public Task<Void> sendReports(@NonNull Executor executor, @Nullable String str) {
        ...
        for (CrashlyticsReportWithSessionId crashlyticsReportWithSessionId : listLoadFinalizedReports) {
            if (str == null || str.equals(crashlyticsReportWithSessionId.getSessionId())) {
                arrayList.add(this.reportsSender.enqueueReport(ensureHasFid(crashlyticsReportWithSessionId), str != null).mo15422g(executor, new s26() { // from class: l.jte0
                    @Override // p153l.s26
                    public final Object then(Task task) {
                        return Boolean.valueOf(this.f122585a.onReportSendComplete(task));
                    }
                }));
            }
        }
