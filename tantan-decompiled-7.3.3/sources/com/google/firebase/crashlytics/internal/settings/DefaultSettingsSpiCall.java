...
class DefaultSettingsSpiCall implements SettingsSpiCall {
    ...
    public DefaultSettingsSpiCall(String str, HttpRequestFactory httpRequestFactory, Logger logger) {
        if (str == null) {
            wg3.m206174a("url must not be null.");
            throw null;
        }
        this.logger = logger;
    ...
    private JSONObject getJsonObjectFrom(String str) {
        try {
            ...
            this.logger.m16785w("Failed to parse settings JSON from " + this.url, e);
            this.logger.m16784w("Settings response " + str);
            return null;
        }
    }
    ...
    public JSONObject handleResponse(HttpResponse httpResponse) {
        ...
        this.logger.m16782v("Settings response code was: " + iCode);
        if (requestWasSuccessful(iCode)) {
            return getJsonObjectFrom(httpResponse.body());
        }
        this.logger.m16778e("Settings request failed; (status: " + iCode + ") from " + this.url);
        return null;
    }
    ...
    public JSONObject invoke(SettingsRequest settingsRequest, boolean z) {
        ...
        if (!z) {
            azk0.m101074a("An invalid data collection token was used.");
            return null;
        }
        try {
            ...
            this.logger.m16776d("Requesting settings from " + this.url);
            this.logger.m16782v("Settings query params were: " + queryParamsFor);
            return handleResponse(httpGetRequestApplyHeadersTo.execute());
        } catch (IOException e) {
            this.logger.m16779e("Settings request failed.", e);
            return null;
        }
    }
