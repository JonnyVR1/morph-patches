...
public class MessagingAnalytics {
    ...
    public static String getInstanceId(Bundle bundle) {
        String string = bundle.getString(Constants.MessagePayloadKeys.f11502TO);
        if (!TextUtils.isEmpty(string)) {
            return string;
        }
        try {
            return (String) toi0.m192063a(FirebaseInstallations.getInstance(FirebaseApp.getInstance()).getId());
    ...
    private static void logToFirelog(MessagingClientEvent.Event event, Intent intent, @Nullable TransportFactory transportFactory) {
        ...
        try {
            transportFactory.getTransport(Constants.FirelogAnalytics.FCM_LOG_SOURCE, MessagingClientEventExtension.class, Encoding.m9346of("proto"), new Transformer() { // from class: l.t900
