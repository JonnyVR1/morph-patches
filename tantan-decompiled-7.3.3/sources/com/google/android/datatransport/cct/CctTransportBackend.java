...
final class CctTransportBackend implements TransportBackend {
    ...
    public static /* synthetic */ HttpRequest m9348b(HttpRequest httpRequest, HttpResponse httpResponse) {
        URL url = httpResponse.redirectUrl;
        if (url == null) {
            return null;
        }
        Logging.m9352d(LOG_TAG, "Following redirect to: %s", url);
    ...
    public HttpResponse doSend(HttpRequest httpRequest) throws IOException {
        Logging.m9356i(LOG_TAG, "Making request to: %s", httpRequest.url);
        ...
        try {
            ...
            try {
                ...
                try {
                    ...
                    Logging.m9356i(LOG_TAG, "Status Code: %d", Integer.valueOf(responseCode));
                    Logging.m9352d(LOG_TAG, "Content-Type: %s", httpURLConnection.getHeaderField("Content-Type"));
                    Logging.m9352d(LOG_TAG, "Content-Encoding: %s", httpURLConnection.getHeaderField("Content-Encoding"));
            ...
            e = e;
            Logging.m9355e(LOG_TAG, "Couldn't encode request, returning with 400", e);
            return new HttpResponse(400, null, 0L);
        } catch (ConnectException e2) {
            e = e2;
            Logging.m9355e(LOG_TAG, "Couldn't open connection, returning with 500", e);
    ...
    private static int getPackageVersionCode(Context context) {
        try {
            ...
            Logging.m9355e(LOG_TAG, "Unable to find version code for package", e);
            return -1;
        }
    }
    ...
    private BatchedLogRequest getRequestBody(BackendRequest backendRequest) {
        ...
        for (Map.Entry entry : map.entrySet()) {
            ...
            for (EventInternal eventInternal3 : (List) entry.getValue()) {
                ...
                if (encoding.equals(Encoding.m9346of("proto"))) {
                    builderProtoBuilder = LogEvent.protoBuilder(encodedPayload.getBytes());
                } else if (encoding.equals(Encoding.m9346of("json"))) {
                    builderProtoBuilder = LogEvent.jsonBuilder(new String(encodedPayload.getBytes(), Charset.forName("UTF-8")));
                } else {
                    Logging.m9357w(LOG_TAG, "Received event of unsupported encoding %s. Skipping...", encoding);
                }
    ...
    private static URL parseUrlOrThrow(String str) {
        try {
            ...
            d7b0.m114716a("Invalid url: ", str, e);
            return null;
        }
    }
    ...
    public EventInternal decorate(EventInternal eventInternal) {
        ...
        return eventInternal.toBuilder().addMetadata(KEY_SDK_VERSION, Build.VERSION.SDK_INT).addMetadata(KEY_MODEL, Build.MODEL).addMetadata(KEY_HARDWARE, Build.HARDWARE).addMetadata("device", Build.DEVICE).addMetadata(KEY_PRODUCT, Build.PRODUCT).addMetadata(KEY_OS_BUILD, Build.ID).addMetadata(KEY_MANUFACTURER, Build.MANUFACTURER).addMetadata(KEY_FINGERPRINT, Build.FINGERPRINT).addMetadata(KEY_TIMEZONE_OFFSET, getTzOffset()).addMetadata(KEY_NETWORK_TYPE, getNetTypeValue(activeNetworkInfo)).addMetadata(KEY_MOBILE_SUBTYPE, getNetSubtypeValue(activeNetworkInfo)).addMetadata("country", Locale.getDefault().getCountry()).addMetadata(KEY_LOCALE, Locale.getDefault().getLanguage()).addMetadata(KEY_MCC_MNC, getMccMncOrEmpty(this.applicationContext)).addMetadata(KEY_APPLICATION_BUILD, Integer.toString(getPackageVersionCode(this.applicationContext))).build();
    }
        ...
        try {
            HttpResponse httpResponse = (HttpResponse) Retries.retry(5, new HttpRequest(urlOrThrow, requestBody, aPIKey), new Function() { // from class: com.google.android.datatransport.cct.a
                ...
                public final Object apply(Object obj) {
                    return this.f6975a.doSend((CctTransportBackend.HttpRequest) obj);
                }
            }, new RetryStrategy() { // from class: com.google.android.datatransport.cct.b
                @Override // com.google.android.datatransport.runtime.retries.RetryStrategy
                public final Object shouldRetry(Object obj, Object obj2) {
                    return CctTransportBackend.m9348b((CctTransportBackend.HttpRequest) obj, (CctTransportBackend.HttpResponse) obj2);
                }
            });
            int i = httpResponse.code;
            if (i == 200) {
                return BackendResponse.m9350ok(httpResponse.nextRequestMillis);
            }
            if (i < 500 && i != 404) {
            ...
            Logging.m9355e(LOG_TAG, "Could not make request to the backend", e);
            return BackendResponse.transientError();
        }
    }
