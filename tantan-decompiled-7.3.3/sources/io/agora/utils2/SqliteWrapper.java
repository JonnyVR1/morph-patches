...
class SqliteWrapper {
    ...
    private SQLiteDatabase f64048db = null;
    ...
    private boolean isDatabaseValid() {
        SQLiteDatabase sQLiteDatabase = this.f64048db;
        return (sQLiteDatabase == null || !sQLiteDatabase.isOpen() || this.f64048db.isReadOnly()) ? false : true;
    }
    ...
    public boolean delete(String str, String str2) {
        ...
        synchronized (this.lock) {
            try {
                try {
                    this.f64048db.execSQL("delete from \"" + str + "\" where key = \"" + str2 + "\";");
                } catch (Exception unused) {
                    return false;
                }
            } catch (Throwable th) {
    ...
    public String[] findAllTableNames() {
        ...
        String str2;
        if (!isDatabaseValid()) {
            Logging.m88113w(TAG, "database is invalid");
            return new String[0];
        }
        synchronized (this.lock) {
            Cursor cursor = null;
            ...
            try {
                try {
                    cursorRawQuery = this.f64048db.rawQuery("select DISTINCT tbl_name from sqlite_master;", new String[0]);
                    if (cursorRawQuery != null) {
                        try {
                            ...
                            cursor = cursorRawQuery;
                            strArr = null;
                            Logging.m88113w(TAG, "findAllTableNames exception");
                        ...
                        if (cursorRawQuery != null) {
                            try {
                                ...
                                Logging.m88113w(TAG, "cursor close exception");
                            }
                        }
                        if (strArr2 == null) {
    ...
    public StorageItem load(String str, String str2, boolean z) {
        ...
        if (TextUtils.isEmpty(str)) {
            Logging.m88113w(TAG, "query is invalid");
            return new StorageItem();
        }
        if (TextUtils.isEmpty(str2)) {
            Logging.m88113w(TAG, "cmd is invalid");
            return new StorageItem();
        }
        synchronized (this.lock) {
            byte[] blob = null;
            try {
                try {
                    cursorRawQuery = this.f64048db.rawQuery(str, new String[0]);
                    if (cursorRawQuery != null) {
                        try {
                    ...
                    if (z2) {
                        cursorRawQuery = this.f64048db.rawQuery(str2, new String[0]);
                    ...
                    Logging.m88113w(TAG, "table is not exist");
                    length = 0;
                    j = 0;
                    str3 = 0;
                    bArr2 = null;
            ...
            try {
                try {
                    if (Build.VERSION.SDK_INT >= 28) {
                        d0g0.m113399a();
                        SQLiteDatabase.OpenParams.Builder builderM107424a = c0g0.m107424a();
                        builderM107424a.setOpenFlags(io.requery.android.database.sqlite.SQLiteDatabase.CREATE_IF_NECESSARY);
                        builderM107424a.setJournalMode(BLiveOperationTitleShowType.off);
                        sQLiteDatabaseOpenDatabase = SQLiteDatabase.openDatabase(new File(str), builderM107424a.build());
                    } else {
                        sQLiteDatabaseOpenDatabase = SQLiteDatabase.openDatabase(str, null, io.requery.android.database.sqlite.SQLiteDatabase.CREATE_IF_NECESSARY);
                    }
                    this.f64048db = sQLiteDatabaseOpenDatabase;
                    try {
                        this.f64048db.execSQL("PRAGMA TEMP_STORE = MEMORY;");
                        this.f64048db.execSQL("PRAGMA SYNCHRONOUS = OFF;");
                    } catch (Exception e) {
                        e.printStackTrace();
                        if (this.f64048db.isOpen()) {
                            this.f64048db.close();
                        }
                        this.f64048db = null;
                        Logging.m88105d(TAG, "Can not open database: " + str);
                        return false;
                    }
                } catch (Exception unused) {
                    SQLiteDatabase sQLiteDatabase = this.f64048db;
    ...
    public long save(String str, String str2, byte[] bArr, long j) {
        ...
        synchronized (this.lock) {
            try {
                ...
                this.f64048db.beginTransaction();
                try {
                    sQLiteStatementCompileStatement = this.f64048db.compileStatement(str3);
                    try {
                ...
                this.f64048db.setTransactionSuccessful();
                this.f64048db.endTransaction();
