...
public class HardwareVideoDecoderFactory implements VideoDecoderFactory {
    ...
    public VideoDecoder createDecoder(VideoCodecInfo videoCodecInfo) {
        ...
        if (capabilitiesForType != null && capabilitiesForType.colorFormats != null) {
            Logging.m88100i(TAG, "supportedColorFormat : " + Arrays.toString(capabilitiesForType.colorFormats));
        }
        ...
        if (map != null && map.containsKey("av_dec_output_byte_frame")) {
            Logging.m88102w(TAG, "force decode to byte frame");
            context2 = null;
        }
        if (surface != null) {
            Logging.m88102w(TAG, "force direct surface");
            context = null;
        } else {
            context = context2;
        }
        ...
        if (numSelectColorFormat == null) {
            Logging.m88102w(TAG, "Unsupported color format!");
            return null;
        }
        Logging.m88100i(TAG, "select color format: " + numSelectColorFormat);
