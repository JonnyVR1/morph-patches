...
public final class WebRtcAudioUtils {
    ...
    private static void logAudioDeviceInfo(String str, AudioManager audioManager) {
        if (runningOnMarshmallowOrHigher()) {
            ...
            Logging.m88097d(str, "Audio Devices: ");
    ...
    private static void logAudioStateBasic(String str, AudioManager audioManager) {
        Logging.m88097d(str, "Audio State: audio mode: " + modeToString(audioManager.getMode()) + ", has mic: " + hasMicrophone() + ", mic muted: " + audioManager.isMicrophoneMute() + ", music active: " + audioManager.isMusicActive() + ", speakerphone: " + audioManager.isSpeakerphoneOn() + ", BT SCO: " + audioManager.isBluetoothScoOn());
    }
    ...
    private static void logAudioStateVolume(String str, AudioManager audioManager) {
        ...
        Logging.m88097d(str, "Audio State: ");
        if (runningOnLollipopOrHigher()) {
            zIsVolumeFixed = audioManager.isVolumeFixed();
            Logging.m88097d(str, "  fixed volume=" + zIsVolumeFixed);
        } else {
    ...
    public static void logDeviceInfo(String str) {
        Logging.m88097d(str, "Android SDK: " + Build.VERSION.SDK_INT + ", Release: " + Build.VERSION.RELEASE + ", Brand: " + Build.BRAND + ", Device: " + Build.DEVICE + ", Id: " + Build.ID + ", Hardware: " + Build.HARDWARE + ", Manufacturer: " + Build.MANUFACTURER + ", Model: " + Build.MODEL + ", Product: " + Build.PRODUCT);
    }
    ...
    public static synchronized void setWebRtcBasedAutomaticGainControl(boolean z) {
        Logging.m88102w(TAG, "setWebRtcBasedAutomaticGainControl() is deprecated");
    }
    ...
    public static synchronized boolean useWebRtcBasedAcousticEchoCanceler() {
        try {
            if (useWebRtcBasedAcousticEchoCanceler) {
                Logging.m88102w(TAG, "Overriding default behavior; now using WebRTC AEC!");
            }
        } catch (Throwable th) {
            throw th;
        }
    ...
    public static synchronized boolean useWebRtcBasedNoiseSuppressor() {
        try {
            if (useWebRtcBasedNoiseSuppressor) {
                Logging.m88102w(TAG, "Overriding default behavior; now using WebRTC NS!");
            }
        } catch (Throwable th) {
            throw th;
        }
