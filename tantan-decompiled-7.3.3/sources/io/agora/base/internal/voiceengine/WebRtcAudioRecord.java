...
public class WebRtcAudioRecord {
    ...
    public class AudioRecordRoutingListener implements AudioRouting.OnRoutingChangedListener {
        ...
        public void onRoutingChanged(AudioRouting audioRouting) {
            ...
            Logging.m88097d(WebRtcAudioRecord.TAG, "[USB] [AudioRecord-audioRouting] onRoutingChanged: " + type);
        }
    }
    ...
    public class AudioRecordThread extends Thread {
        ...
        public void run() {
            ...
            Logging.m88097d(WebRtcAudioRecord.TAG, "AudioRecordThread" + WebRtcAudioUtils.getThreadInfo());
            ...
            while (true) {
                ...
                if (!z) {
                    try {
                        ...
                        Logging.m88098e(WebRtcAudioRecord.TAG, "AudioRecord.stop failed: " + e.getMessage());
                        return;
                    }
                }
                ...
                if (i == WebRtcAudioRecord.this.byteBuffer.capacity()) {
                    ...
                    Logging.m88098e(WebRtcAudioRecord.TAG, str);
                    if (i == -3) {
                ...
                if (WebRtcAudioRecord.this.mFramesRead == 0) {
                    Logging.m88097d(WebRtcAudioRecord.TAG, "AudioRecordThread read first frame : " + (i / WebRtcAudioRecord.this.mBytesPerFrame) + " finished. ");
                }
                ...
                try {
                    ...
                    Logging.m88098e(WebRtcAudioRecord.TAG, "calculateLatencyMillis failed: " + th.getMessage());
                }
            }
        ...
        public void stopThread() {
            Logging.m88097d(WebRtcAudioRecord.TAG, "stopThread");
            this.keepAlive = false;
        }
    }
    ...
    public WebRtcAudioRecord(long j) {
        ...
        Logging.m88097d(TAG, "ctor" + WebRtcAudioUtils.getThreadInfo());
    ...
    public int calculateLatencyMillis() {
        if (this.audioRecord == null) {
            Logging.m88098e(TAG, "audioRecord is null");
            return -1;
        }
        ...
        if (this.audioRecord.getTimestamp(audioTimestamp, 0) != 0) {
            Logging.m88098e(TAG, "audioTimestamp is unavailable");
            return -1;
        }
        ...
        Logging.m88097d(TAG, "AudioRecord calculateLatencyMillis--> latencyMillis: " + iNanoTime + ",extraTime:20");
    ...
    private boolean enableBuiltInAEC(boolean z) {
        Logging.m88097d(TAG, "enableBuiltInAEC(" + z + ')');
        WebRtcAudioEffects webRtcAudioEffects = this.effects;
        if (webRtcAudioEffects != null) {
            return webRtcAudioEffects.setAEC(z);
        }
        Logging.m88098e(TAG, "Built-in AEC is not supported on this platform");
        return false;
    }

    @CalledByNative
    private boolean enableBuiltInAGC(boolean z) {
        Logging.m88097d(TAG, "enableBuiltInAGC(" + z + ')');
        WebRtcAudioEffects webRtcAudioEffects = this.effects;
        if (webRtcAudioEffects != null) {
            return webRtcAudioEffects.setAGC(z);
        }
        Logging.m88098e(TAG, "Built-in AGC is not supported on this platform");
        return false;
    }

    @CalledByNative
    private boolean enableBuiltInNS(boolean z) {
        Logging.m88097d(TAG, "enableBuiltInNS(" + z + ')');
        WebRtcAudioEffects webRtcAudioEffects = this.effects;
        if (webRtcAudioEffects != null) {
            return webRtcAudioEffects.setNS(z);
        }
        Logging.m88098e(TAG, "Built-in NS is not supported on this platform");
        return false;
    }
    ...
    private int initRecording(int i, int i2, int i3) {
        ...
        int i4 = i / 100;
        Logging.m88097d(TAG, "initRecording(sampleRate=" + i + ", channels=" + i2 + ", minBufMs=" + i3 + ") audioRecord=" + this.audioRecord);
        ...
        if (audioRecord != null) {
            if (i == audioRecord.getSampleRate() && i2 == this.audioRecord.getChannelCount() && this.audioRecord.getAudioSource() == this.audioSource) {
                Logging.m88097d(TAG, "using record object " + this.audioRecord);
                Logging.m88097d(TAG, "status=" + this.audioRecord.getState() + ",recordStatus=" + this.audioRecord.getRecordingState());
                registerRoutingListener();
                return i4;
            }
            Logging.m88097d(TAG, "re-create record object for parameter changed");
        ...
        Logging.m88097d(TAG, "AudioRecord.getMinBufferSize: " + minBufferSize);
        int iMax = Math.max(minBufferSize * 2, this.byteBuffer.capacity());
        int i5 = i3 * (i / 1000) * i2 * 2;
        int i6 = iMax < i5 ? i5 : iMax;
        Logging.m88097d(TAG, "bufferSizeInBytes: " + i6);
        Logging.m88097d(TAG, "initRecording audio mode: " + WebRtcAudioUtils.modeToString(this.audioManager.getMode()) + ", source: " + WebRtcAudioUtils.sourceToString(this.audioSource) + ", sampleRate: " + i);
        if (!ContextUtils.getApplicationContext().getPackageManager().hasSystemFeature("android.hardware.microphone")) {
            Logging.m88097d(TAG, "[USB] Before create audiorecord instance: has no PackageManager.FEATURE_MICROPHONE");
        }
        try {
    ...
    private void logMainParameters() {
        Logging.m88097d(TAG, "AudioRecord: session ID: " + this.audioRecord.getAudioSessionId() + ", channels: " + this.audioRecord.getChannelCount() + ", sample rate: " + this.audioRecord.getSampleRate());
    }

    @TargetApi(23)
    private void logMainParametersExtended() {
        if (WebRtcAudioUtils.runningOnMarshmallowOrHigher()) {
            Logging.m88097d(TAG, "AudioRecord: buffer size in frames: " + this.audioRecord.getBufferSizeInFrames());
        }
    }
    ...
    private boolean releaseAudioResources() {
        Logging.m88097d(TAG, "record releaseAudioResources");
        boolean z = false;
        try {
            WebRtcAudioEffects webRtcAudioEffects = this.effects;
            if (webRtcAudioEffects != null) {
            ...
            Logging.m88102w(TAG, "releaseAudioResources e: " + e.getMessage());
            return z;
        }
    }
    ...
    public void reportWebRtcAudioRecordError(String str) {
        Logging.m88098e(TAG, "Run-time recording error: " + str);
    ...
    private void reportWebRtcAudioRecordInitError(String str) {
        Logging.m88098e(TAG, "Init recording error: " + str);
        WebRtcAudioUtils.logAudioState(TAG);
        WebRtcAudioRecordErrorCallback webRtcAudioRecordErrorCallback = errorCallback;
        if (webRtcAudioRecordErrorCallback != null) {
    ...
    private void reportWebRtcAudioRecordStartError(AudioRecordStartErrorCode audioRecordStartErrorCode, String str) {
        Logging.m88098e(TAG, "Start recording error: " + audioRecordStartErrorCode + ". " + str);
        WebRtcAudioUtils.logAudioState(TAG);
        WebRtcAudioRecordErrorCallback webRtcAudioRecordErrorCallback = errorCallback;
        if (webRtcAudioRecordErrorCallback != null) {
    ...
    public static void setErrorCallback(WebRtcAudioRecordErrorCallback webRtcAudioRecordErrorCallback) {
        Logging.m88097d(TAG, "Set error callback");
        errorCallback = webRtcAudioRecordErrorCallback;
    }

    public static void setMicrophoneMute(boolean z) {
        Logging.m88102w(TAG, "setMicrophoneMute(" + z + ")");
        microphoneMute = z;
    }
    ...
    public static void setRecordRouteCallback(AudioRecordRouteCallback audioRecordRouteCallback) {
        Logging.m88097d(TAG, "[USB] Set AudioRecordRouteCallback");
        recordRouteCallback = audioRecordRouteCallback;
    }

    @CalledByNative
    private int startRecording() {
        Logging.m88097d(TAG, "startRecording");
    ...
    public void registerRoutingListener() {
        ...
        if (preferredDevice != null) {
            Logging.m88097d(TAG, "[USB] deviceType = " + preferredDevice.getType());
        } else {
            Logging.m88097d(TAG, "[USB] has no preferred device");
            preferredDevice = this.audioRecord.getRoutedDevice();
            if (preferredDevice != null) {
                Logging.m88097d(TAG, "[USB] initRecording-getRoutedDevice deviceType = " + preferredDevice.getType());
            }
        }
        ...
        Logging.m88097d(TAG, "android.os.Build.VERSION.SDK_INT = " + i);
    ...
    public synchronized int setAudioSource(int i) {
        Logging.m88102w(TAG, "Audio source is changed from: " + this.audioSource + " to " + i);
        this.audioSource = i;
        return 0;
    }
    ...
    public class AudioRecordRoutingListenerForAPI23 implements AudioRecord.OnRoutingChangedListener {
        ...
        public void onRoutingChanged(AudioRecord audioRecord) {
            ...
            Logging.m88097d(WebRtcAudioRecord.TAG, "[USB] [AudioRecord-audioRecord] onRoutingChanged:" + type);
        }
    }
}
