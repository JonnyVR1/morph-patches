...
public class WebRtcAudioManager {
    ...
    public class AudioRecordingCallbackImpl extends AudioManager.AudioRecordingCallback {
        ...
        public AudioRecordingCallbackImpl() {
            Logging.m88097d(WebRtcAudioManager.TAG, "AudioRecordingCallbackImpl ctor " + this);
        }

        @Override // android.media.AudioManager.AudioRecordingCallback
        public void onRecordingConfigChanged(List<AudioRecordingConfiguration> list) {
            WebRtcAudioManager webRtcAudioManager = this.mWebRtcAudioManager.get();
            Logging.m88097d(WebRtcAudioManager.TAG, "onRecordingConfigChanged " + list.size());
            ...
            synchronized (webRtcAudioManager.recorderCallbackLock) {
                try {
                    if (!webRtcAudioManager.initialized) {
                        Logging.m88102w(WebRtcAudioManager.TAG, "dont handle recording callback when destroy!");
                        return;
                    }
                    super.onRecordingConfigChanged(list);
                    Iterator<AudioRecordingConfiguration> it = list.iterator();
                    while (it.hasNext()) {
                        Logging.m88097d(WebRtcAudioManager.TAG, " recording client session ID: " + it.next().getClientAudioSessionId());
                    }
        ...
        public void run() {
            ...
            try {
                ...
                if (applicationContext != null && (volumeChangeReceiver = this.volumeChangeReceiver) != null) {
                    ...
                    Logging.m88097d(WebRtcAudioManager.TAG, "unregister volume change receiver " + this.volumeChangeReceiver);
                }
            } catch (Exception e) {
                Logging.m88099e(WebRtcAudioManager.TAG, "unregister VolumeChangeReceiver failed ", e);
            }
        }
    }
    ...
    public WebRtcAudioManager(long j, int i, boolean z) {
        Logging.m88097d(TAG, "ctor" + WebRtcAudioUtils.getThreadInfo() + ", this " + this);
        ...
        Logging.m88102w(TAG, "ctor finish");
    }
    ...
    private void dispose() {
        Logging.m88097d(TAG, "dispose" + WebRtcAudioUtils.getThreadInfo());
        if (this.initialized) {
    ...
    private boolean enableChatMode(boolean z) {
        ...
        try {
            Logging.m88097d(TAG, "set audio mode to: " + WebRtcAudioUtils.modeToString(i) + " curr: " + WebRtcAudioUtils.modeToString(this.audioManager.getMode()));
            this.audioManager.setMode(i);
        } catch (Exception e) {
            Logging.m88098e(TAG, "audioManager.setMode failed: " + e.getMessage());
        }
        return true;
    }
    ...
    private int getAudioMode() {
        ...
        try {
            ...
            Logging.m88099e(TAG, "get audio mode failed ", e);
            return 0;
        }
    }
    ...
    private int getCallState() {
        ...
        Logging.m88097d(TAG, "mock call state: " + mockedAudioManager.getRingerMode());
    ...
    private int getCurrentPlayoutDevices() {
        ...
        try {
            ...
            Logging.m88097d(TAG, "get current playout devices: " + iIntValue);
            return iIntValue;
        } catch (Exception e) {
            Logging.m88099e(TAG, "Error getDevicesForStream! ", e);
            return iIntValue;
        }
    }
    ...
    private int getNativeOutputSampleRate() {
        if (WebRtcAudioUtils.runningOnEmulator()) {
            Logging.m88097d(TAG, "Running emulator, overriding sample rate to 8 kHz.");
            return TXRecordCommon.AUDIO_SAMPLERATE_8000;
        }
        if (WebRtcAudioUtils.isDefaultSampleRateOverridden()) {
            Logging.m88097d(TAG, "Default sample rate is overriden to " + WebRtcAudioUtils.getDefaultSampleRateHz() + " Hz");
        ...
        Logging.m88097d(TAG, "Sample rate is set to " + sampleRateOnJellyBeanMR10OrHigher + " Hz");
    ...
    private boolean init() {
        Logging.m88097d(TAG, "init" + WebRtcAudioUtils.getThreadInfo());
        if (this.initialized) {
            return true;
        }
    ...
    private boolean isAAudioSupported() {
        Logging.m88102w(TAG, "AAudio support is currently disabled on all devices!");
        return false;
    }
    ...
    private boolean monitorPlayoutVolumeChange(boolean z) {
        Logging.m88097d(TAG, "monitorPlayoutVolumeChange monitor: " + z);
        Context applicationContext = ContextUtils.getApplicationContext();
        if (applicationContext != null && mockedAudioManager == null) {
            if (z) {
                ...
                if (this.mVolumeChangeReceiver == null) {
                    try {
                        ...
                        jj5.m145018l(applicationContext, this.mVolumeChangeReceiver, intentFilter);
                        C18372la.m153467c(this, streamVolume, streamType);
                        Logging.m88097d(TAG, "register volume change receiver " + this.mVolumeChangeReceiver);
                        return true;
                    } catch (Exception e) {
                        Logging.m88099e(TAG, "Unable to create VolumeChangeReceiver, ", e);
                    }
                }
                return false;
            }
            try {
                ...
                Logging.m88099e(TAG, "unregister VolumeChangeReceiver failed ", e2);
            }
        }
        return false;
    }
    ...
    private void notifyPlayoutVolumeChange(int i, int i2) {
        ...
        Logging.m88097d(TAG, "notifyPlayoutVolumeChange: " + i);
    }
    ...
    private int setAudioAttrCtx(AudioAttributes audioAttributes) {
        ...
        Logging.m88097d(TAG, "audioAttr " + audioAttributes.toString());
    ...
    private boolean updatePlayoutVolumeIfNeeded() {
        ...
        Logging.m88097d(TAG, "playout volume is update to " + streamVolume + ", type " + streamType);
        return true;
    }
