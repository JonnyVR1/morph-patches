...
public class TextureBufferHelper {
    ...
    private TextureBufferHelper(EglBase.Context context, Handler handler) {
        ...
        if (handler.getLooper().getThread() != Thread.currentThread()) {
            wtq0.m207906a("TextureBufferHelper must be created on the handler thread");
            throw null;
        }
            ...
            return (TextureBufferHelper) ThreadUtils.invokeAtFrontUninterruptibly(handler, new Callable<TextureBufferHelper>() { // from class: io.agora.base.TextureBufferHelper.1
                ...
                public TextureBufferHelper call() {
                    try {
                        ...
                        Logging.m88109e(TextureBufferHelper.TAG, str + " create failure", e);
                        return null;
                    }
                }
            });
        } catch (Exception e) {
            Logging.m88109e(TAG, str + " create failure", e);
            return null;
        }
    }
    ...
    public void dispose() {
        ...
        try {
            ...
            Logging.m88105d(TAG, "dispose fail: " + e.getMessage());
        }
    }
    ...
    public ByteBuffer parseAlphaData(TextureBuffer textureBuffer, int i, boolean z) {
        ...
        if (byteBuffer != null) {
            ...
            if (iCapacity != i8) {
                Logging.m88113w(TAG, "fillAlphaData size is changed, alphaBuffer.capacity: " + byteBuffer2.capacity() + "  buffer.width: " + width + "  buffer.height: " + height);
