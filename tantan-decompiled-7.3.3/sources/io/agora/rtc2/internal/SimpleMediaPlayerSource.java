...
public class SimpleMediaPlayerSource {
    ...
    public static class MediaExtractorWrapper {
        ...
        public boolean setDataSource(final String str) throws InterruptedException {
            Logging.m88105d(SimpleMediaPlayerSource.TAG, "setDataSource()");
            ...
            this.mHandler.post(new Runnable() { // from class: io.agora.rtc2.internal.SimpleMediaPlayerSource.MediaExtractorWrapper.1
                ...
                public void run() {
                    Logging.m88105d(SimpleMediaPlayerSource.TAG, "setDataSource in thread " + MediaExtractorWrapper.this.threadName + "  url: " + str);
                    try {
                        ...
                        Logging.m88113w(SimpleMediaPlayerSource.TAG, "setDataSource fail: " + e.toString());
                    }
                    countDownLatch.countDown();
                }
            });
            countDownLatch.await(2000L, TimeUnit.MILLISECONDS);
            Logging.m88105d(SimpleMediaPlayerSource.TAG, "setDataSource complete");
        ...
        public void checkPrepared() {
            ...
            wtq0.m207906a("mMediaExtractor hasn't prepared");
        }
        ...
        public void release() {
            Logging.m88105d(SimpleMediaPlayerSource.TAG, "release()");
            this.mHandler.post(new Runnable() { // from class: io.agora.rtc2.internal.SimpleMediaPlayerSource.MediaExtractorWrapper.2
                ...
                public void run() {
                    try {
                        ...
                        Logging.m88109e(SimpleMediaPlayerSource.TAG, "release media extractor exception.", e);
                    }
                    MediaExtractorWrapper.this.mHandler.getLooper().quit();
                    Logging.m88105d(SimpleMediaPlayerSource.TAG, "mediaExtractor released in thread " + MediaExtractorWrapper.this.threadName);
                }
            });
        }
    ...
    private boolean acquireFrames(@NonNull ByteBuffer byteBuffer) {
        ...
        if (!this.mIsExtractorEndOfStream) {
            ...
            if (iDequeueInputBuffer < 0) {
                Logging.m88113w(TAG, "wait for next available input buffer timeout");
            } else {
                int sampleData = this.mExtractor.readSampleData(this.mAudioDecoder.getInputBuffers()[iDequeueInputBuffer], 0);
                if (sampleData < 0) {
                    Logging.m88113w(TAG, "no more samples are available");
        ...
        Logging.m88113w(TAG, "OutputBuffer BUFFER_FLAG_END_OF_STREAM");
        return false;
    }
    ...
    private static int findAudioTrackAndFillStreamInfo(@NonNull SparseArray<MediaStreamInfoWrapper> sparseArray, @NonNull MediaExtractorWrapper mediaExtractorWrapper, @NonNull String str) {
        ...
        int trackCount = mediaExtractorWrapper.getTrackCount();
        Logging.m88105d(TAG, "track count : " + trackCount);
        int i2 = -1;
        for (int i3 = 0; i3 < trackCount; i3++) {
            MediaFormat trackFormat = mediaExtractorWrapper.getTrackFormat(i3);
            String string = trackFormat.getString(IMediaFormat.KEY_MIME);
            Logging.m88105d(TAG, "track : " + i3 + " type : " + string);
    ...
    private int openImpl(@NonNull String str, long j) throws Throwable {
        ...
        if (mediaExtractorWrapper.setDataSource(str) && checkMimeTypeSupported(this.mExtractor)) {
            ...
            if (mediaStreamInfoWrapper == null) {
                ...
                Logging.m88105d(TAG, "DurationMs: " + this.mDurationMs + ", AudioChannels: " + this.mAudioChannels + ", AudioSampleRate: " + this.mAudioSampleRate + ", BytesPerSample: " + this.mBytesPerSample);
                if (this.mBytesPerSample == 0) {
                    ...
                    Logging.m88105d(TAG, " samplesPerChannel: " + i + ", bytesPerInterval: " + i2);
            ...
            Logging.m88108e(TAG, str2);
        } else {
            Logging.m88113w(TAG, "Failed to setDataSource");
        }
        reset();
        return -2;
    }
    ...
    private void reset() {
        ...
        if (mediaCodec != null) {
            try {
                ...
                Logging.m88109e(TAG, "Media decoder stop failed", e);
            }
            try {
                this.mAudioDecoder.release();
            } catch (Exception e2) {
                Logging.m88109e(TAG, "Media decoder release failed", e2);
            }
    ...
    private int selectAudioTrackWithPos(int i, long j) {
        ...
        Logging.m88111i(TAG, "selectAudioTrackWithPos audioTrackIndex: " + i + ", startPos: " + j + " ms, TrackCount: " + trackCount);
        ...
        if (mediaStreamInfoWrapper == null || mediaStreamInfoWrapper.getMediaStreamType() != Constants.MediaStreamType.getValue(Constants.MediaStreamType.STREAM_TYPE_AUDIO)) {
            ...
            this.mAudioSampleRate = mediaStreamInfoWrapper.getAudioSampleRate();
            this.mBytesPerSample = mediaStreamInfoWrapper.getAudioBytesPerSample();
            Logging.m88111i(TAG, "DurationMs: " + this.mDurationMs + ", AudioChannels: " + this.mAudioChannels + ", AudioSampleRate: " + this.mAudioSampleRate + ", BytesPerSample: " + this.mBytesPerSample);
            if (this.mBytesPerSample != 0) {
    ...
    public SMPSIntervalData acquireIntervalData() {
        ...
        if (byteBuffer == null) {
            Logging.m88105d(TAG, "Failed to acquireIntervalData, invalidate frame buffer");
        } else {
            try {
                byteBuffer.clear();
                ...
                Logging.m88109e(TAG, "Failed to acquireIntervalData", th);
            }
        }
    ...
    public boolean checkFormatInfoChanged() {
        ...
        try {
            ...
            Logging.m88108e(TAG, "Error when checking file's new format");
            e.printStackTrace();
            return z;
        }
    }
    ...
    public int open(String str, long j) {
        String str2;
        Logging.m88105d(TAG, "open() url, startPos: " + j);
        ...
        if (list.contains(str3)) {
            ...
            if (!TextUtils.isEmpty(str)) {
                ...
                if (URLUtil.isNetworkUrl(str) && ((str = NetUtil.getEncodedUrl(str)) == null || !NetUtil.testNetworkUrlAvailable(str))) {
                    Logging.m88113w(TAG, "Fail to open, 404 for url");
                    return -2;
                }
                try {
                    return openImpl(str, j);
                } catch (Throwable unused) {
                    Logging.m88113w(TAG, "Failed to open");
                    reset();
                    return -2;
                }
            }
        ...
        Logging.m88113w(TAG, str2);
        return -1;
    }

    @CalledByNative
    public int seek(long j) {
        Logging.m88105d(TAG, "seek() " + j);
    ...
    public int selectAudioTrack(int i) {
        MediaExtractorWrapper mediaExtractorWrapper = this.mExtractor;
        if (mediaExtractorWrapper == null) {
            Logging.m88108e(TAG, "null extractor");
            return -1;
        }
        try {
            return selectAudioTrackWithPos(i, mediaExtractorWrapper.getSampleTime() / 1000);
        } catch (Exception e) {
            Logging.m88109e(TAG, "selectAudioTrackWithPos failed:", e);
            return -1;
        }
    }
}
