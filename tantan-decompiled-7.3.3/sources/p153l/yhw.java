...
public class yhw {
    ...
    public static String m215997a(byte[] bArr) throws Exception {
        ...
        while (i < length) {
            ...
            int i3 = b & 255;
            if (i2 == length) {
                stringBuffer.append(f199997b[i3 >>> 2]);
                stringBuffer.append(f199997b[(b & 3) << 4]);
                stringBuffer.append(hmw.m136007a(new byte[]{HttpTokens.CARRIAGE_RETURN, 91}));
                break;
            }
            int i4 = i + 2;
            byte b2 = bArr[i2];
            if (i4 == length) {
                stringBuffer.append(f199997b[i3 >>> 2]);
                stringBuffer.append(f199997b[((b & 3) << 4) | ((b2 & 240) >>> 4)]);
                stringBuffer.append(f199997b[(b2 & 15) << 2]);
            ...
            stringBuffer.append(f199997b[i3 >>> 2]);
            stringBuffer.append(f199997b[((b & 3) << 4) | ((b2 & 240) >>> 4)]);
            stringBuffer.append(f199997b[((b2 & 15) << 2) | ((b3 & 192) >>> 6)]);
            stringBuffer.append(f199997b[b3 & Utf8.REPLACEMENT_BYTE]);
        }
