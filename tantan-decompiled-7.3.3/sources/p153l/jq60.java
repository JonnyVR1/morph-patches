...
public final class jq60 implements k2m {
    ...
    public jq60(@NotNull PurchaseType purchaseType, @Nullable String str) {
    ...
    public Pair<CharSequence, Boolean> mo136600a(@NotNull PayMethod payMethod, @NotNull C8928d section, boolean newFilterRule) {
        ...
        if (!newFilterRule) {
            if (wib0.m206573q(purchaseType) && C8929e.m54833x(this.from)) {
                return new Pair<>("", Boolean.TRUE);
            }
            return (TEnum.equals(payMethod, "alipay") || TEnum.equals(payMethod, "wechat")) ? new Pair<>("京东支付最高立减8元", Boolean.FALSE) : new Pair<>("随机立减最高8元", Boolean.TRUE);
        }
        List<PayMethod> listM189833x = tab0.m189811h().m189833x(mo136602c(section, purchaseType, this.from).m54784s());
        listM189833x.getClass();
        if ((TEnum.equals(payMethod, "alipay") || TEnum.equals(payMethod, "wechat")) && listM189833x.contains(PayMethod.get(PayMethod.jingdong))) {
            return new Pair<>("京东支付最高立减8元", Boolean.FALSE);
        }
        return (TEnum.equals(payMethod, PayMethod.jingdong) && listM189833x.contains(PayMethod.get(PayMethod.jingdong))) ? new Pair<>("随机立减最高8元", Boolean.TRUE) : new Pair<>("", Boolean.TRUE);
    }
