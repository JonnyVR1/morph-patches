...
public abstract class BaseWebViewRequestParam {
    ...
    public Bundle fillBundle(Bundle bundle) {
        ...
        if (baseWebViewRequestData == null) {
            jfd0.a("构造方法错误，请使用全参数的构造方法构建");
            return null;
        }
        bundle.putSerializable("base", baseWebViewRequestData);
        int i = C06221.$SwitchMap$com$sina$weibo$sdk$web$WebRequestType[this.baseData.getType().ordinal()];
        if (i == 1) {
            bundle.putInt(MiniWidgetProvider.KEY_TYPE, 0);
        } else if (i == 2) {
            bundle.putInt(MiniWidgetProvider.KEY_TYPE, 1);
        } else if (i == 3) {
            bundle.putInt(MiniWidgetProvider.KEY_TYPE, 2);
        }
