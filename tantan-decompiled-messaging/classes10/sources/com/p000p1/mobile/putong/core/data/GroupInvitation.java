...
public class GroupInvitation extends ValueObject implements Cloneable, Serializable {
    ...
    public static ProtobufAdapter<GroupInvitation> PROTOBUF_ADAPTER = new MessageNanoAdapter<GroupInvitation>() { // from class: com.p1.mobile.putong.core.data.GroupInvitation.1
        ...
        public int computeAndCacheSize(GroupInvitation groupInvitation) {
            ...
            if (button != null) {
                iO += CodedOutputByteBufferNano.l(3, button, Button.PROTOBUF_ADAPTER);
            }
            Target target = groupInvitation.target;
            if (target != null) {
                iO += CodedOutputByteBufferNano.l(4, target, Target.PROTOBUF_ADAPTER);
            }
            ((MessageNano) groupInvitation).cachedSize = iO;
            return iO;
        }

        /* JADX INFO: renamed from: parse, reason: merged with bridge method [inline-methods] */
        public GroupInvitation m13091parse(nb5 nb5Var) throws IOException {
            ...
            while (true) {
                ...
                if (iU == 10) {
                    groupInvitation.title = nb5Var.s();
                } else if (iU == 18) {
                    groupInvitation.subTitle = nb5Var.s();
                } else if (iU == 26) {
                    groupInvitation.button = (Button) nb5Var.l(Button.PROTOBUF_ADAPTER);
                } else {
                    if (iU != 34) {
                        if (groupInvitation.title == null) {
                            groupInvitation.title = "";
                    ...
                    groupInvitation.target = (Target) nb5Var.l(Target.PROTOBUF_ADAPTER);
                }
            }
            return groupInvitation;
        }
        ...
        public void serialize(GroupInvitation groupInvitation, CodedOutputByteBufferNano codedOutputByteBufferNano) throws IOException {
            ...
            Button button = groupInvitation.button;
            if (button != null) {
                codedOutputByteBufferNano.K(3, button, Button.PROTOBUF_ADAPTER);
            }
            Target target = groupInvitation.target;
            if (target != null) {
                codedOutputByteBufferNano.K(4, target, Target.PROTOBUF_ADAPTER);
            }
        }
    };
    public static JsonAdapter<GroupInvitation> JSON_ADAPTER = new ObjectJsonAdapter<GroupInvitation>() { // from class: com.p1.mobile.putong.core.data.GroupInvitation.2
        ...
        public GroupInvitation m13092newInstance() {
        ...
        public boolean parseField(GroupInvitation groupInvitation, String str, JsonParser jsonParser, String str2, ArrayList<BaseData> arrayList, DataChecker dataChecker) throws IOException {
            ...
            switch (str) {
                case "subTitle":
                    ...
                    return true;
                case "button":
                    groupInvitation.button = (Button) Button.JSON_ADAPTER.parse(jsonParser, str2, arrayList, dataChecker);
                    return true;
                case "target":
                    groupInvitation.target = (Target) Target.JSON_ADAPTER.parse(jsonParser, str2, arrayList, dataChecker);
                    return true;
                case "title":
    ...
    public GroupInvitation m13090clone() {
        GroupInvitation groupInvitation = new GroupInvitation();
        groupInvitation.title = this.title;
        groupInvitation.subTitle = this.subTitle;
        Button button = this.button;
        if (button != null) {
            groupInvitation.button = button.m11826clone();
        }
        Target target = this.target;
        if (target != null) {
            groupInvitation.target = target.m15937clone();
        }
        return groupInvitation;
    }
