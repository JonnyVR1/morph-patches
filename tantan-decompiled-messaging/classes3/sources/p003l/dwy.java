package p003l;

import android.text.TextUtils;
import com.p1.mobile.android.app.App;
import com.p1.mobile.putong.core.CoreModule;
import com.p1.mobile.putong.core.api.a;
import com.p1.mobile.putong.core.data.Message;
import com.p1.mobile.putong.core.data.MessageType;
import com.tantanapp.common.data.DbObject;
import java.text.SimpleDateFormat;
import java.util.Date;
import java.util.List;
import java.util.TimeZone;
import l.mqi0;
import l.tpd0;

/* JADX INFO: loaded from: /tmp/tantan-dex/classes3.dex */
public class dwy {
    /* JADX INFO: renamed from: a */
    public static Message m3744a(List<Message> list, String str, long j, String str2) {
        Message messageNew_ = Message.new_();
        messageNew_.cid = str;
        tpd0 tpd0Var = App.i;
        ((DbObject) messageNew_).id = DbObject.GENERATE_UNIQUE_FAKE_ID(((Integer) tpd0Var.get()).intValue());
        messageNew_.messageType = MessageType.get("local_countdown_like_success");
        messageNew_.createdTime = !list.isEmpty() ? a.X2(list.get(0).createdTime) : a.W2();
        messageNew_.localCreatedTime = System.nanoTime();
        messageNew_.localCreatedSession = ((Integer) tpd0Var.get()).intValue();
        messageNew_.localInConversation = true;
        messageNew_.owner = CoreModule.H().userId();
        messageNew_.recalled = Boolean.FALSE;
        messageNew_.localExtraInfo = "";
        if (!TextUtils.isEmpty(str2)) {
            messageNew_.localExtraInfo = str2;
            return messageNew_;
        }
        if (j <= 0) {
            messageNew_.localExtraInfo = "00.00.00";
            return messageNew_;
        }
        messageNew_.localExtraInfo = m3745b(j);
        return messageNew_;
    }

    /* JADX INFO: renamed from: b */
    public static String m3745b(long j) {
        long jO = j - mqi0.o();
        if (jO <= 0) {
            return "00.00.00";
        }
        SimpleDateFormat simpleDateFormat = new SimpleDateFormat("HH.mm.ss");
        simpleDateFormat.setTimeZone(TimeZone.getTimeZone("GMT"));
        Date date = new Date();
        date.setTime(jO);
        return simpleDateFormat.format(date);
    }
}
