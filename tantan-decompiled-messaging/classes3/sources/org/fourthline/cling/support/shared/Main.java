...
import org.seamless.util.C0934OS;
...
public abstract class Main implements ShutdownHandler, Thread.UncaughtExceptionHandler {
    ...
    public void init() {
        try {
            if (C0934OS.checkForMac()) {
                NewPlatformApple.setup(this, getAppName());
            }
            UIManager.setLookAndFeel(UIManager.getSystemLookAndFeelClassName());
        } catch (Exception unused) {
        }
        this.errorWindow.setPreferredSize(new Dimension(IMediaPlayer.MEDIA_INFO_TIMED_TEXT_ERROR, HttpStatus.BAD_REQUEST_400));
