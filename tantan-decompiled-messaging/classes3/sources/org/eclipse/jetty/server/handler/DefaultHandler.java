...
public class DefaultHandler extends AbstractHandler {
    ...
    public DefaultHandler() {
        try {
            ...
            if (resource != null) {
                this._favicon = C0746IO.readBytes(Resource.newResource(resource).getInputStream());
            }
        } catch (Exception e) {
            LOG.warn(e);
        }
    ...
    public void handle(String str, Request request, HttpServletRequest httpServletRequest, HttpServletResponse httpServletResponse) throws ServletException, IOException {
        ...
        if (this._serveIcon && this._favicon != null && method.equals(HttpMethods.GET) && httpServletRequest.getRequestURI().equals("/favicon.ico")) {
            if (httpServletRequest.getDateHeader(HttpHeaders.IF_MODIFIED_SINCE) == this._faviconModified) {
            ...
            httpServletResponse.setHeader(HttpHeaders.CACHE_CONTROL, "max-age=360000,public");
            httpServletResponse.getOutputStream().write(this._favicon);
            return;
        }
        if (!method.equals(HttpMethods.GET) || !httpServletRequest.getRequestURI().equals("/")) {
        ...
        zje0 outputStream = httpServletResponse.getOutputStream();
