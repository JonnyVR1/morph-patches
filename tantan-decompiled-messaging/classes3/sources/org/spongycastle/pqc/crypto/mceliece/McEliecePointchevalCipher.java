...
public class McEliecePointchevalCipher implements MessageEncryptor {
    ...
    public void init(boolean z, CipherParameters cipherParameters) {
        if (!z) {
            ...
            if (!(cipherParameters instanceof ParametersWithRandom)) {
                this.f10558sr = new SecureRandom();
            ...
            this.f10558sr = parametersWithRandom.getRandom();
    ...
    public void initCipherDecrypt(McElieceCCA2PrivateKeyParameters mcElieceCCA2PrivateKeyParameters) {
        ...
        this.f10557n = mcElieceCCA2PrivateKeyParameters.getN();
        this.f10556k = mcElieceCCA2PrivateKeyParameters.getK();
        this.f10559t = mcElieceCCA2PrivateKeyParameters.getT();
    }

    public void initCipherEncrypt(McElieceCCA2PublicKeyParameters mcElieceCCA2PublicKeyParameters) {
        SecureRandom secureRandom = this.f10558sr;
        if (secureRandom == null) {
            secureRandom = new SecureRandom();
        }
        this.f10558sr = secureRandom;
        this.messDigest = mcElieceCCA2PublicKeyParameters.getParameters().getDigest();
        this.f10557n = mcElieceCCA2PublicKeyParameters.getN();
        this.f10556k = mcElieceCCA2PublicKeyParameters.getK();
        this.f10559t = mcElieceCCA2PublicKeyParameters.getT();
    }

    @Override // org.spongycastle.pqc.crypto.MessageEncryptor
    public byte[] messageDecrypt(byte[] bArr) throws Exception {
        int i = (this.f10557n + 7) >> 3;
        int length = bArr.length - i;
        byte[][] bArrSplit = ByteUtils.split(bArr, i);
        byte[] bArr2 = bArrSplit[0];
        byte[] bArr3 = bArrSplit[1];
        GF2Vector[] gF2VectorArrDecryptionPrimitive = McElieceCCA2Primitives.decryptionPrimitive((McElieceCCA2PrivateKeyParameters) this.key, GF2Vector.OS2VP(this.f10557n, bArr2));
        ...
        if (Conversions.encode(this.f10557n, this.f10559t, bArr5).equals(gF2Vector)) {
            return ByteUtils.split(bArr4, length - (this.f10556k >> 3))[0];
        }
    ...
    public byte[] messageEncrypt(byte[] bArr) throws Exception {
        int i = this.f10556k >> 3;
        byte[] bArr2 = new byte[i];
        this.f10558sr.nextBytes(bArr2);
        GF2Vector gF2Vector = new GF2Vector(this.f10556k, this.f10558sr);
        ...
        byte[] encoded2 = McElieceCCA2Primitives.encryptionPrimitive((McElieceCCA2PublicKeyParameters) this.key, gF2Vector, Conversions.encode(this.f10557n, this.f10559t, bArr3)).getEncoded();
        DigestRandomGenerator digestRandomGenerator = new DigestRandomGenerator(new SHA1Digest());
        digestRandomGenerator.addSeedMaterial(encoded);
