...
public class Tables64kGCMMultiplier implements GCMMultiplier {
    ...
    public void init(byte[] bArr) {
        ...
        if (this.f10302M == null) {
            this.f10302M = (int[][][]) Array.newInstance((Class<?>) Integer.TYPE, 16, 256, 4);
        } else if (Arrays.areEqual(this.f10301H, bArr)) {
            return;
        }
        this.f10301H = Arrays.clone(bArr);
        GCMUtil.asInts(bArr, this.f10302M[0][128]);
        for (int i2 = 64; i2 >= 1; i2 >>= 1) {
            int[][] iArr = this.f10302M[0];
            GCMUtil.multiplyP(iArr[i2 + i2], iArr[i2]);
        }
        while (true) {
            for (int i3 = 2; i3 < 256; i3 += i3) {
                for (int i4 = 1; i4 < i3; i4++) {
                    int[][] iArr2 = this.f10302M[i];
    ...
    public void multiplyH(byte[] bArr) {
        int[] iArr = new int[4];
        for (int i = 15; i >= 0; i--) {
            int[] iArr2 = this.f10302M[i][bArr[i] & 255];
