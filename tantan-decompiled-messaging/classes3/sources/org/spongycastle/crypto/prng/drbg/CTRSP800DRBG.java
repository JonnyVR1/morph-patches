...
public class CTRSP800DRBG implements SP80090DRBG {
    ...
    public CTRSP800DRBG(BlockCipher blockCipher, int i, int i2, EntropySource entropySource, byte[] bArr, byte[] bArr2) {
        ...
        if (i2 > 256) {
            ig3.a("Requested security strength is not supported by the derivation function");
            throw null;
        }
        if (getMaxSecurityStrength(blockCipher, i) < i2) {
            ig3.a("Requested security strength is not supported by block cipher and key size");
            throw null;
        }
        if (entropySource.entropySize() >= i2) {
            CTR_DRBG_Instantiate_algorithm(getEntropy(), bArr2, bArr);
        } else {
            ig3.a("Not enough entropy for security strength required");
            throw null;
        }
    }
    ...
    private void CTR_DRBG_Reseed_algorithm(byte[] bArr) {
        CTR_DRBG_Update(Block_Cipher_df(Arrays.concatenate(getEntropy(), bArr), this._seedLength), this._Key, this.f10376_V);
        this._reseedCounter = 1L;
    }
    ...
    public int generate(byte[] bArr, byte[] bArr2, boolean z) {
        ...
        if (bArr2 != null) {
            ...
            CTR_DRBG_Update(bArrBlock_Cipher_df, this._Key, this.f10376_V);
        } else {
            bArrBlock_Cipher_df = new byte[i];
        }
        int length = this.f10376_V.length;
        byte[] bArr3 = new byte[length];
        this._engine.init(true, new KeyParameter(expandKey(this._Key)));
        for (int i2 = 0; i2 <= bArr.length / length; i2++) {
            int i3 = i2 * length;
            int length2 = bArr.length - i3 > length ? length : bArr.length - (this.f10376_V.length * i2);
            if (length2 != 0) {
                addOneTo(this.f10376_V);
                this._engine.processBlock(this.f10376_V, 0, bArr3, 0);
