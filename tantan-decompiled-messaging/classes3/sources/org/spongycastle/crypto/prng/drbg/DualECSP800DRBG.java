...
public class DualECSP800DRBG implements SP80090DRBG {
    ...
    private ECPoint f10379_P;

    /* JADX INFO: renamed from: _Q */
    private ECPoint f10380_Q;
    private ECCurve.C1073Fp _curve;
    ...
    static {
        ...
        nistPoints = new DualECPoints[]{new DualECPoints(128, c1073Fp.createPoint(bigInteger, bigInteger2), c1073Fp.createPoint(bigInteger3, bigInteger4), 1), new DualECPoints(192, c1073Fp.createPoint(bigInteger5, bigInteger6), c1073Fp.createPoint(bigInteger7, bigInteger8), 1), new DualECPoints(256, c1073Fp.createPoint(bigInteger9, bigInteger10), c1073Fp.createPoint(bigInteger11, bigInteger12), 1)};
        ECCurve.C1073Fp c1073Fp = (ECCurve.C1073Fp) NISTNamedCurves.getByName("P-256").getCurve();
        ECCurve.C1073Fp c1073Fp2 = (ECCurve.C1073Fp) NISTNamedCurves.getByName("P-384").getCurve();
        ECCurve.C1073Fp c1073Fp3 = (ECCurve.C1073Fp) NISTNamedCurves.getByName("P-521").getCurve();
    }
    ...
    public DualECSP800DRBG(DualECPoints[] dualECPointsArr, Digest digest, int i, EntropySource entropySource, byte[] bArr, byte[] bArr2) {
        ...
        if (Utils.isTooLarge(bArr, 512)) {
            ig3.a("Personalization string too large");
            throw null;
        }
        if (entropySource.entropySize() < i || entropySource.entropySize() > 4096) {
            ctq0.a("EntropySource must provide between ", i, " and 4096 bits");
            throw null;
        }
        ...
        for (int i2 = 0; i2 != dualECPointsArr.length; i2++) {
            if (i <= dualECPointsArr[i2].getSecurityStrength()) {
                if (Utils.getMaxSecurityStrength(digest) < dualECPointsArr[i2].getSecurityStrength()) {
                    ig3.a("Requested security strength is not supported by digest");
                    throw null;
                }
                this._seedlen = dualECPointsArr[i2].getSeedLen();
                this._outlen = dualECPointsArr[i2].getMaxOutlen() / 8;
                this.f10379_P = dualECPointsArr[i2].getP();
                this.f10380_Q = dualECPointsArr[i2].getQ();
                break;
            }
        }
        if (this.f10379_P == null) {
            ig3.a("security strength cannot be greater than 256 bits");
            throw null;
        }
        byte[] bArrHash_df = Utils.hash_df(this._digest, bArrConcatenate, this._seedlen);
        this.f10381_s = bArrHash_df;
    ...
    private byte[] getEntropy() {
        ...
        qkq0.a("Insufficient entropy provided by entropy source");
        return null;
    }
    ...
    public int generate(byte[] bArr, byte[] bArr2, boolean z) {
        ...
        if (Utils.isTooLarge(bArr2, 512)) {
            ig3.a("Additional input too large");
            return 0;
        }
        ...
        BigInteger bigInteger = bArr2 != null ? new BigInteger(1, xor(this.f10381_s, Utils.hash_df(this._digest, bArr2, this._seedlen))) : new BigInteger(1, this.f10381_s);
        Arrays.fill(bArr, (byte) 0);
        int i = 0;
        for (int i2 = 0; i2 < length2; i2++) {
            bigInteger = getScalarMultipleXCoord(this.f10379_P, bigInteger);
            byte[] byteArray = getScalarMultipleXCoord(this.f10380_Q, bigInteger).toByteArray();
        ...
        if (i < bArr.length) {
            bigInteger = getScalarMultipleXCoord(this.f10379_P, bigInteger);
            byte[] byteArray2 = getScalarMultipleXCoord(this.f10380_Q, bigInteger).toByteArray();
        ...
        this.f10381_s = BigIntegers.asUnsignedByteArray(this._sLength, getScalarMultipleXCoord(this.f10379_P, bigInteger));
        return length;
    }
    ...
    public void reseed(byte[] bArr) {
        if (Utils.isTooLarge(bArr, 512)) {
            ig3.a("Additional input string too large");
            return;
        }
        this.f10381_s = Utils.hash_df(this._digest, Arrays.concatenate(pad8(this.f10381_s, this._seedlen), getEntropy(), bArr), this._seedlen);
        this._reseedCounter = 0L;
    }
