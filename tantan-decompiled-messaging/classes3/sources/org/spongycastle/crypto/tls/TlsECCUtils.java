    ...
    public static ECDomainParameters readECParameters(int[] iArr, short[] sArr, InputStream inputStream) throws IOException {
        ...
        try {
            ...
            if (uint8 == 1) {
                ...
                ECCurve.C1073Fp c1073Fp = new ECCurve.C1073Fp(eCParameter, eCFieldElement, eCFieldElement2, eCParameter2, eCParameter3);
                return new ECDomainParameters(c1073Fp, deserializeECPoint(sArr, c1073Fp, opaque8), eCParameter2, eCParameter3);
            }
            if (uint8 != 2) {
                if (uint8 != 3) {
    ...
    public static void writeExplicitECParameters(short[] sArr, ECDomainParameters eCDomainParameters, OutputStream outputStream) throws IOException {
        ...
        if (ECAlgorithms.isFpCurve(curve)) {
            ...
            if (!ECAlgorithms.isF2mCurve(curve)) {
                ig3.a("'ecParameters' not a known curve type");
                return;
            }
            ...
            if (exponentsPresent.length == 3) {
                ...
                if (exponentsPresent.length != 5) {
                    ig3.a("Only trinomial and pentomial curves are supported");
                    return;
                }
                TlsUtils.writeUint8((short) 2, outputStream);
                writeECExponent(exponentsPresent[1], outputStream);
