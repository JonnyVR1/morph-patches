...
public class DSAParametersGenerator {
    ...
    private DSAParameters generateParameters_FIPS186_2() {
        ...
        if (!(this.digest instanceof SHA1Digest)) {
            qkq0.a("can only use SHA-1 for generating FIPS 186-2 parameters");
            return null;
        }
        while (true) {
            ...
            if (isProbablePrime(bigInteger)) {
                ...
                while (i6 < 4096) {
                    ...
                    if (bigIntegerSubtract.bitLength() == this.f10229L && isProbablePrime(bigIntegerSubtract)) {
    ...
    private DSAParameters generateParameters_FIPS186_3() {
        ...
        byte[] bArr = new byte[this.f10230N / 8];
        int i2 = this.f10229L;
        ...
        loop0: while (true) {
            ...
            bit = new BigInteger(1, bArr3).mod(ONE.shiftLeft(this.f10230N - 1)).setBit(0).setBit(this.f10230N - 1);
            if (isProbablePrime(bit)) {
                byte[] bArrClone = Arrays.clone(bArr);
                int i7 = this.f10229L * 4;
                i = 0;
                while (i < i7) {
    ...
    public void init(DSAParameterGenerationParameters dSAParameterGenerationParameters) {
        ...
        if (l2 < 1024 || l2 > 3072 || l2 % 1024 != 0) {
            ig3.a("L values must be between 1024 and 3072 and a multiple of 1024");
            return;
        }
        if (l2 == 1024 && n != 160) {
            ig3.a("N must be 160 for L = 1024");
            return;
        }
        if (l2 == 2048 && n != 224 && n != 256) {
            ig3.a("N must be 224 or 256 for L = 2048");
            return;
        }
        if (l2 == 3072 && n != 256) {
            ig3.a("N must be 256 for L = 3072");
            return;
        }
        if (this.digest.getDigestSize() * 8 < n) {
            qkq0.a("Digest output size too small for value of N");
            return;
        }
        this.f10229L = l2;
        this.f10230N = n;
    ...
    public void init(int i, int i2, SecureRandom secureRandom) {
        this.f10229L = i;
        this.f10230N = getDefaultN(i);
        this.certainty = i2;
        this.iterations = Math.max(getMinimumIterations(this.f10229L), (i2 + 1) / 2);
