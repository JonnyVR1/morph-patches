...
public class OpenBSDBCrypt {
    ...
    public static boolean checkPassword(String str, char[] cArr) {
        if (str.length() != 60) {
            ap50.m2905a("Bcrypt String length: ", str.length(), ", 60 required.");
            return false;
        }
        if (str.charAt(0) != '$' || str.charAt(3) != '$' || str.charAt(6) != '$') {
            ig3.a("Invalid Bcrypt String format.");
            return false;
        }
        if (!str.substring(1, 3).equals(version)) {
            ig3.a("Wrong Bcrypt version, 2a expected.");
            return false;
        }
        try {
            int i = Integer.parseInt(str.substring(4, 6));
            if (i < 4 || i > 31) {
                ctq0.a("Invalid cost factor: ", i, ", 4 < cost < 31 expected.");
                return false;
            }
            if (cArr != null) {
                return str.equals(generate(cArr, decodeSaltString(str.substring(str.lastIndexOf(36) + 1, str.length() - 31)), i));
            }
            ig3.a("Missing password.");
            return false;
        } catch (NumberFormatException unused) {
            ig3.a("Invalid cost factor:".concat(str.substring(4, 6)));
            return false;
        }
    }
    ...
    private static byte[] decodeSaltString(String str) {
        ...
        if (charArray.length != 22) {
            ap50.m2905a("Invalid base64 salt length: ", charArray.length, " , 22 required.");
            return null;
        }
        for (char c : charArray) {
            if (c > 'z' || c < '.' || (c > '9' && c < 'A')) {
                k250.m5546a("Salt string contains invalid character: ", c);
                return null;
            }
        }
    ...
    private static String encodeData(byte[] bArr) {
        ...
        if (bArr.length != 24 && bArr.length != 16) {
            ap50.m2905a("Invalid length: ", bArr.length, ", 24 for key or 16 for salt expected");
            return null;
        }
        if (bArr.length == 16) {
        ...
        for (int i = 0; i < length; i += 3) {
            ...
            byteArrayOutputStream.write(bArr3[b & 63]);
        }
