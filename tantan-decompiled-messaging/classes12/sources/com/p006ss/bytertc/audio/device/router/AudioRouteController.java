...
public class AudioRouteController implements IAudioRouteCallback {
    ...
    private int setAudioRoute(@AudioRouteDeviceManager.RoutingDeviceType int i, boolean z) {
        ...
        try {
            ...
            RXLogging.w(TAG, "AudioRouteController: setAudioRoute, end, routing: " + i + ", isMediaMode: " + z);
            this.rLock.unlock();
        }
    }
    ...
    @Override // com.p006ss.bytertc.audio.device.router.IAudioRouteCallback
    public void onDeviceEvent(int i, boolean z, String str) throws Throwable {
        AudioRouteController audioRouteController;
        Lock lock;
        this.rLock.lock();
        RXLogging.w(TAG, "onDeviceEvent, begin, deviceType: " + i + ", connected: " + z + ", deviceInfo:" + str);
        try {
            if (this.nativeAudioRouteControllerPtr == 0) {
                RXLogging.w(TAG, "AudioRouteController: onDeviceEvent, end");
                lock = this.rLock;
            } else {
                RXLogging.e(TAG, "AudioRouteController: onDeviceEvent(Java) deviceType:" + i + ", connected:" + z + ", deviceInfo:" + str);
