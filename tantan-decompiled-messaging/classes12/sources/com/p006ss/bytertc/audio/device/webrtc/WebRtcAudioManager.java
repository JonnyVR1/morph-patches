...
import com.p006ss.bytertc.audio.device.webrtc.observer.MicMuteChangedObserver;
import com.p006ss.bytertc.audio.device.webrtc.observer.VolumeChangedObserver;
...
public class WebRtcAudioManager implements VolumeChangedObserver.OnVolumeChangedListener, MicMuteChangedObserver.OnMicMuteChangedListener {
        ...
        public void onCallStateChanged(int i, String str) {
            synchronized (WebRtcAudioManager.this) {
                try {
                    ...
                    this.callState = i;
                    RXLogging.i(WebRtcAudioManager.TAG, "onCallStateChanged, from: " + i2 + ", to: " + i);
                    if (i != 0) {
                        if ((i == 1 || i == 2) && i2 == 0) {
    ...
    private int SetAudioMode(int i) {
        RXLogging.i(TAG, "SetAudioMode enter, modeInvalid: " + this.modeInvalid + ", curMode: " + this.curMode + ", setMode: " + i);
        try {
            if (i != this.curMode || this.modeInvalid) {
                this.audioManager.setMode(i);
                RXLogging.w(TAG, "set mode from: " + this.curMode + ", to: " + i + ", modeInvalid is: " + this.modeInvalid);
            }
            int mode = getMode();
            this.curMode = mode;
            this.modeInvalid = mode != i;
            RXLogging.i(TAG, "SetAudioMode leave, modeInvalid: " + this.modeInvalid + ", curMode: " + this.curMode + ", setMode: " + i);
            return 0;
        } catch (Exception e) {
            e.printStackTrace();
            return -1;
        ...
        synchronized (this) {
            try {
                if (this.nativeAudioManager != 0) {
                    RXLogging.i(TAG, "dixing: notifyPhoneStateListenerRegistered: " + z + ", " + this.nativeAudioManager);
        ...
        new Handler(Looper.getMainLooper()).post(new Runnable() { // from class: com.ss.bytertc.audio.device.webrtc.WebRtcAudioManager.1
            ...
            public void run() {
                try {
                    ...
                    RXLogging.i(WebRtcAudioManager.TAG, "registerPhoneStateListener success with event: " + i);
                } catch (Throwable th) {
                    th.printStackTrace();
                    RXLogging.e(WebRtcAudioManager.TAG, "registerPhoneStateListener (" + i + "), error: " + th.getCause() + ", " + th.getMessage());
                }
            }
        });
    }
    ...
    public static synchronized void setStereoInput(boolean z) {
        RXLogging.w(TAG, "Overriding default input behavior: setStereoInput(" + z + ')');
        useStereoInput = z;
    }

    public static synchronized void setStereoOutput(boolean z) {
        RXLogging.w(TAG, "Overriding default output behavior: setStereoOutput(" + z + ')');
        useStereoOutput = z;
    }
