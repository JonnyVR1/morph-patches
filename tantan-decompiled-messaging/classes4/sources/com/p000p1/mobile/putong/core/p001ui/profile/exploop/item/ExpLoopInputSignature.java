...
public class ExpLoopInputSignature extends ExpLoopInputType {
    ...
    public ExpLoopInputSignature() {
        super(14, "简单介绍下自己", ExpLoopInputType.ICON_SIGNATURE, ExpLoopInputType.INPUT_CUSTOM_SIGNATURE);
        this.needCount = 0;
    }
    ...
    public boolean hasInfoExist(User user, LoopFragmentFactory.LoopCreateEntryType loopCreateEntryType) {
        if (loopCreateEntryType == LoopFragmentFactory.LoopCreateEntryType.ENTRY_FROM_PICKS) {
            return !TextUtils.isEmpty(user.description) && user.description.length() >= this.needCount && TextUtils.isEmpty(CoreModule.c.q2.n3());
        }
    ...
    public void saveUserInfo(User user) {
        ...
        user.description = vwb.k(getData().getTextType().text1.trim());
    }
