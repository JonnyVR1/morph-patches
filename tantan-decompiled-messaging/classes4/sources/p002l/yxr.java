...
public class yxr extends vp2<LongLinkLiveMessage.Block> {
    ...
    private String m26973z(LongLinkLiveMessage.Block block) {
        if (block.getTempdata().getFieldsList().isEmpty()) {
            return ypv.k().c4();
        }
        String fields = block.getTempdata().getFields(0);
        return wxr.INSTANCE.m25544a(w8u.u(!vdt.b(2) ? R$string.f2768Hf : R$string.f3683y0, fields), fields, -1).toString();
    }
    ...
    public LiveControlMessage mo10148w(String str, LongLinkLiveMessage.Block block, String str2) {
        ...
        CrashHelper.c(new IllegalStateException("live boot message send in block msg handler"));
        return new LiveControlMessage(str, "boot").setContent(m26973z(block)).setFrom(LiveControlMessage.FROM_LIVE_ROOM_BLOCK);
    }
