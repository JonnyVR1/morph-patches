...
public class WebRtcAudioManager {
    ...
    public static class VolumeLogger {
        ...
        public class LogVolumeTask extends TimerTask {
            ...
            public void run() {
                ...
                if (mode == 1) {
                    Logging.m468d(WebRtcAudioManager.TAG, "STREAM_RING stream volume: " + VolumeLogger.this.audioManager.getStreamVolume(2) + " (max=" + this.maxRingVolume + ")");
                    return;
                }
                if (mode == 3) {
                    Logging.m468d(WebRtcAudioManager.TAG, "VOICE_CALL stream volume: " + VolumeLogger.this.audioManager.getStreamVolume(0) + " (max=" + this.maxVoiceCallVolume + ")");
                }
            }
        }
        ...
        public void start() {
            ...
            this.timer = timer;
            timer.schedule(new LogVolumeTask(this.audioManager.getStreamMaxVolume(2), this.audioManager.getStreamMaxVolume(3)), 0L, 30000L);
        }
    }
    ...
    private void dispose() {
        Logging.m468d(TAG, "dispose" + WebRtcAudioUtils.getThreadInfo());
        if (this.initialized) {
    ...
    private int getNativeOutputSampleRate() {
        if (WebRtcAudioUtils.runningOnEmulator()) {
            Logging.m468d(TAG, "Running emulator, overriding sample rate to 8 kHz.");
            return 8000;
        }
        if (WebRtcAudioUtils.isDefaultSampleRateOverridden()) {
            Logging.m468d(TAG, "Default sample rate is overriden to " + WebRtcAudioUtils.getDefaultSampleRateHz() + " Hz");
            return WebRtcAudioUtils.getDefaultSampleRateHz();
        }
        int sampleRateForApiLevel = getSampleRateForApiLevel();
        Logging.m468d(TAG, "Sample rate is set to " + sampleRateForApiLevel + " Hz");
    ...
    private boolean init() {
        Logging.m468d(TAG, "init" + WebRtcAudioUtils.getThreadInfo());
        if (this.initialized) {
    ...
    private boolean isAAudioSupported() {
        Logging.m472w(TAG, "AAudio support is currently disabled on all devices!");
        return false;
    }
    ...
    private boolean isDeviceBlacklistedForOpenSLESUsage() {
        ...
        if (zDeviceIsBlacklistedForOpenSLESUsage) {
            Logging.m468d(TAG, Build.MODEL + " is blacklisted for OpenSL ES usage!");
        }
    ...
    public static synchronized void setStereoInput(boolean z) {
        Logging.m472w(TAG, "Overriding default input behavior: setStereoInput(" + z + ')');
        useStereoInput = z;
    }

    public static synchronized void setStereoOutput(boolean z) {
        Logging.m472w(TAG, "Overriding default output behavior: setStereoOutput(" + z + ')');
        useStereoOutput = blacklistDeviceForAAudioUsage;
    }
