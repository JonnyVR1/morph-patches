...
class AndroidVideoDecoder implements VideoDecoder, VideoSink {
    ...
    public AndroidVideoDecoder(MediaCodecWrapperFactory mediaCodecWrapperFactory, String str, VideoCodecType videoCodecType, int i, EglBase.Context context) {
        if (!isSupportedColorFormat(i)) {
            k250.a("Unsupported color format: ", i);
            throw null;
        }
        Logging.m468d(TAG, "ctor name: " + str + " type: " + videoCodecType + " color format: " + i + " context: " + context);
    ...
    private void deliverByteFrame(int i, MediaCodec.BufferInfo bufferInfo, int i2, Integer num) {
        ...
        if (i7 < ((i3 * i4) * 3) / 2) {
            Logging.m469e(TAG, "Insufficient output buffer size: " + bufferInfo.size);
            return;
        }
    ...
    private VideoCodecStatus initDecodeInternal(int i, int i2) {
        ...
        Logging.m468d(TAG, "initDecodeInternal name: " + this.codecName + " type: " + this.codecType + " width: " + i + " height: " + i2);
        if (this.outputThread != null) {
            Logging.m469e(TAG, "initDecodeInternal called while the codec is already running");
        ...
        try {
            ...
            try {
                ...
                Logging.m468d(TAG, "initDecodeInternal done");
            ...
            Logging.m469e(TAG, "Cannot create media decoder " + this.codecName);
            return VideoCodecStatus.FALLBACK_SOFTWARE;
        }
    }
    ...
    private void reformat(MediaFormat mediaFormat) {
        ...
        Logging.m468d(TAG, "Decoder format changed: " + mediaFormat.toString());
    ...
    public void releaseCodecOnOutputThread() {
        ...
        try {
            ...
            Logging.m470e(TAG, "Media decoder stop failed", e);
        }
        try {
            this.codec.release();
        } catch (Exception e2) {
            Logging.m470e(TAG, "Media decoder release failed", e2);
    ...
    private VideoCodecStatus releaseInternal() {
        ...
        if (!this.running) {
            Logging.m468d(TAG, "release: Decoder is not running.");
            return VideoCodecStatus.OK;
        }
        try {
            this.running = false;
            if (!ThreadUtils.joinUninterruptibly(this.outputThread, 5000L)) {
                Logging.m470e(TAG, "Media decoder release timeout", new RuntimeException());
                this.codec = null;
                ...
                Logging.m470e(TAG, "Media decoder release error", new RuntimeException(this.shutdownException));
    ...
    public VideoCodecStatus decode(EncodedImage encodedImage, VideoDecoder.DecodeInfo decodeInfo) {
        ...
        if (byteBuffer == null) {
            Logging.m469e(TAG, "decode() - no input data");
            return VideoCodecStatus.ERR_PARAMETER;
        }
        int iRemaining = byteBuffer.remaining();
        if (iRemaining == 0) {
            Logging.m469e(TAG, "decode() - input buffer empty");
            return VideoCodecStatus.ERR_PARAMETER;
        }
        synchronized (this.dimensionLock) {
            i = this.width;
        ...
        if (this.keyFrameRequired) {
            if (encodedImage.frameType != EncodedImage.FrameType.VideoFrameKey) {
                Logging.m469e(TAG, "decode() - key frame required first");
                return VideoCodecStatus.NO_OUTPUT;
            }
            if (!encodedImage.completeFrame) {
                Logging.m469e(TAG, "decode() - complete frame required first");
                return VideoCodecStatus.NO_OUTPUT;
            }
        }
        try {
            ...
            if (iDequeueInputBuffer < 0) {
                Logging.m469e(TAG, "decode() - no HW buffers available; decoder falling behind");
                return VideoCodecStatus.ERROR;
            }
            try {
                ByteBuffer byteBuffer2 = this.codec.getInputBuffers()[iDequeueInputBuffer];
                if (byteBuffer2.capacity() < iRemaining) {
                    Logging.m469e(TAG, "decode() - HW buffer too small");
                    return VideoCodecStatus.ERROR;
                }
    ...
    public void deliverDecodedFrame() {
        ...
        try {
            ...
            if (iDequeueOutputBuffer < 0) {
                Logging.m471v(TAG, "dequeueOutputBuffer returned " + iDequeueOutputBuffer);
                return;
            }
            ...
            Logging.m470e(TAG, "deliverDecodedFrame failed", e);
        }
    }
