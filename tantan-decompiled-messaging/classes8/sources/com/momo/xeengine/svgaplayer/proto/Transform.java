package com.momo.xeengine.svgaplayer.proto;

import com.squareup.wire.FieldEncoding;
import com.squareup.wire.Message;
import com.squareup.wire.ProtoAdapter;
import com.squareup.wire.ProtoReader;
import com.squareup.wire.ProtoWriter;
import com.squareup.wire.WireField;
import com.squareup.wire.internal.Internal;
import java.io.IOException;
import okio.ByteString;

/* JADX INFO: loaded from: /tmp/tantan-dex/classes8.dex */
public final class Transform extends Message<Transform, Builder> {
    public static final ProtoAdapter<Transform> ADAPTER = new ProtoAdapter_Transform();
    public static final Float DEFAULT_A;
    public static final Float DEFAULT_B;
    public static final Float DEFAULT_C;
    public static final Float DEFAULT_D;
    public static final Float DEFAULT_TX;
    public static final Float DEFAULT_TY;
    private static final long serialVersionUID = 0;

    /* JADX INFO: renamed from: a */
    @WireField(adapter = "com.squareup.wire.ProtoAdapter#FLOAT", tag = 1)
    public final Float f433a;

    /* JADX INFO: renamed from: b */
    @WireField(adapter = "com.squareup.wire.ProtoAdapter#FLOAT", tag = 2)
    public final Float f434b;

    /* JADX INFO: renamed from: c */
    @WireField(adapter = "com.squareup.wire.ProtoAdapter#FLOAT", tag = 3)
    public final Float f435c;

    /* JADX INFO: renamed from: d */
    @WireField(adapter = "com.squareup.wire.ProtoAdapter#FLOAT", tag = 4)
    public final Float f436d;

    /* JADX INFO: renamed from: tx */
    @WireField(adapter = "com.squareup.wire.ProtoAdapter#FLOAT", tag = 5)
    public final Float f437tx;

    /* JADX INFO: renamed from: ty */
    @WireField(adapter = "com.squareup.wire.ProtoAdapter#FLOAT", tag = 6)
    public final Float f438ty;

    public static final class Builder extends Message.Builder<Transform, Builder> {

        /* JADX INFO: renamed from: a */
        public Float f439a;

        /* JADX INFO: renamed from: b */
        public Float f440b;

        /* JADX INFO: renamed from: c */
        public Float f441c;

        /* JADX INFO: renamed from: d */
        public Float f442d;

        /* JADX INFO: renamed from: tx */
        public Float f443tx;

        /* JADX INFO: renamed from: ty */
        public Float f444ty;

        /* JADX INFO: renamed from: a */
        public Builder m598a(Float f) {
            this.f439a = f;
            return this;
        }

        /* JADX INFO: renamed from: b */
        public Builder m599b(Float f) {
            this.f440b = f;
            return this;
        }

        /* JADX INFO: renamed from: build, reason: merged with bridge method [inline-methods] */
        public Transform m12166build() {
            return new Transform(this.f439a, this.f440b, this.f441c, this.f442d, this.f443tx, this.f444ty, super.buildUnknownFields());
        }

        /* JADX INFO: renamed from: c */
        public Builder m600c(Float f) {
            this.f441c = f;
            return this;
        }

        /* JADX INFO: renamed from: d */
        public Builder m601d(Float f) {
            this.f442d = f;
            return this;
        }

        /* JADX INFO: renamed from: tx */
        public Builder m602tx(Float f) {
            this.f443tx = f;
            return this;
        }

        /* JADX INFO: renamed from: ty */
        public Builder m603ty(Float f) {
            this.f444ty = f;
            return this;
        }
    }

    public static final class ProtoAdapter_Transform extends ProtoAdapter<Transform> {
        public ProtoAdapter_Transform() {
            super(FieldEncoding.LENGTH_DELIMITED, Transform.class);
        }

        /* JADX INFO: renamed from: decode, reason: merged with bridge method [inline-methods] */
        public Transform m12167decode(ProtoReader protoReader) throws IOException {
            Builder builder = new Builder();
            long jBeginMessage = protoReader.beginMessage();
            while (true) {
                int iNextTag = protoReader.nextTag();
                if (iNextTag == -1) {
                    protoReader.endMessage(jBeginMessage);
                    return builder.m12166build();
                }
                switch (iNextTag) {
                    case 1:
                        builder.m598a((Float) ProtoAdapter.FLOAT.decode(protoReader));
                        break;
                    case 2:
                        builder.m599b((Float) ProtoAdapter.FLOAT.decode(protoReader));
                        break;
                    case 3:
                        builder.m600c((Float) ProtoAdapter.FLOAT.decode(protoReader));
                        break;
                    case 4:
                        builder.m601d((Float) ProtoAdapter.FLOAT.decode(protoReader));
                        break;
                    case 5:
                        builder.m602tx((Float) ProtoAdapter.FLOAT.decode(protoReader));
                        break;
                    case 6:
                        builder.m603ty((Float) ProtoAdapter.FLOAT.decode(protoReader));
                        break;
                    default:
                        FieldEncoding fieldEncodingPeekFieldEncoding = protoReader.peekFieldEncoding();
                        builder.addUnknownField(iNextTag, fieldEncodingPeekFieldEncoding, fieldEncodingPeekFieldEncoding.rawProtoAdapter().decode(protoReader));
                        break;
                }
            }
        }

        public void encode(ProtoWriter protoWriter, Transform transform) throws IOException {
            Float f = transform.f433a;
            if (f != null) {
                ProtoAdapter.FLOAT.encodeWithTag(protoWriter, 1, f);
            }
            Float f2 = transform.f434b;
            if (f2 != null) {
                ProtoAdapter.FLOAT.encodeWithTag(protoWriter, 2, f2);
            }
            Float f3 = transform.f435c;
            if (f3 != null) {
                ProtoAdapter.FLOAT.encodeWithTag(protoWriter, 3, f3);
            }
            Float f4 = transform.f436d;
            if (f4 != null) {
                ProtoAdapter.FLOAT.encodeWithTag(protoWriter, 4, f4);
            }
            Float f5 = transform.f437tx;
            if (f5 != null) {
                ProtoAdapter.FLOAT.encodeWithTag(protoWriter, 5, f5);
            }
            Float f6 = transform.f438ty;
            if (f6 != null) {
                ProtoAdapter.FLOAT.encodeWithTag(protoWriter, 6, f6);
            }
            protoWriter.writeBytes(transform.unknownFields());
        }

        public int encodedSize(Transform transform) {
            Float f = transform.f433a;
            int iEncodedSizeWithTag = f != null ? ProtoAdapter.FLOAT.encodedSizeWithTag(1, f) : 0;
            Float f2 = transform.f434b;
            int iEncodedSizeWithTag2 = iEncodedSizeWithTag + (f2 != null ? ProtoAdapter.FLOAT.encodedSizeWithTag(2, f2) : 0);
            Float f3 = transform.f435c;
            int iEncodedSizeWithTag3 = iEncodedSizeWithTag2 + (f3 != null ? ProtoAdapter.FLOAT.encodedSizeWithTag(3, f3) : 0);
            Float f4 = transform.f436d;
            int iEncodedSizeWithTag4 = iEncodedSizeWithTag3 + (f4 != null ? ProtoAdapter.FLOAT.encodedSizeWithTag(4, f4) : 0);
            Float f5 = transform.f437tx;
            int iEncodedSizeWithTag5 = iEncodedSizeWithTag4 + (f5 != null ? ProtoAdapter.FLOAT.encodedSizeWithTag(5, f5) : 0);
            Float f6 = transform.f438ty;
            return iEncodedSizeWithTag5 + (f6 != null ? ProtoAdapter.FLOAT.encodedSizeWithTag(6, f6) : 0) + transform.unknownFields().size();
        }

        public Transform redact(Transform transform) {
            Builder builderM12165newBuilder = transform.m12165newBuilder();
            builderM12165newBuilder.clearUnknownFields();
            return builderM12165newBuilder.m12166build();
        }
    }

    static {
        Float fValueOf = Float.valueOf(0.0f);
        DEFAULT_A = fValueOf;
        DEFAULT_B = fValueOf;
        DEFAULT_C = fValueOf;
        DEFAULT_D = fValueOf;
        DEFAULT_TX = fValueOf;
        DEFAULT_TY = fValueOf;
    }

    public Transform(Float f, Float f2, Float f3, Float f4, Float f5, Float f6, ByteString byteString) {
        super(ADAPTER, byteString);
        this.f433a = f;
        this.f434b = f2;
        this.f435c = f3;
        this.f436d = f4;
        this.f437tx = f5;
        this.f438ty = f6;
    }

    public boolean equals(Object obj) {
        if (obj == this) {
            return true;
        }
        if (!(obj instanceof Transform)) {
            return false;
        }
        Transform transform = (Transform) obj;
        return unknownFields().equals(transform.unknownFields()) && Internal.equals(this.f433a, transform.f433a) && Internal.equals(this.f434b, transform.f434b) && Internal.equals(this.f435c, transform.f435c) && Internal.equals(this.f436d, transform.f436d) && Internal.equals(this.f437tx, transform.f437tx) && Internal.equals(this.f438ty, transform.f438ty);
    }

    public int hashCode() {
        int i = ((Message) this).hashCode;
        if (i != 0) {
            return i;
        }
        int iHashCode = unknownFields().hashCode() * 37;
        Float f = this.f433a;
        int iHashCode2 = (iHashCode + (f != null ? f.hashCode() : 0)) * 37;
        Float f2 = this.f434b;
        int iHashCode3 = (iHashCode2 + (f2 != null ? f2.hashCode() : 0)) * 37;
        Float f3 = this.f435c;
        int iHashCode4 = (iHashCode3 + (f3 != null ? f3.hashCode() : 0)) * 37;
        Float f4 = this.f436d;
        int iHashCode5 = (iHashCode4 + (f4 != null ? f4.hashCode() : 0)) * 37;
        Float f5 = this.f437tx;
        int iHashCode6 = (iHashCode5 + (f5 != null ? f5.hashCode() : 0)) * 37;
        Float f6 = this.f438ty;
        int iHashCode7 = iHashCode6 + (f6 != null ? f6.hashCode() : 0);
        ((Message) this).hashCode = iHashCode7;
        return iHashCode7;
    }

    /* JADX INFO: renamed from: newBuilder, reason: merged with bridge method [inline-methods] */
    public Builder m12165newBuilder() {
        Builder builder = new Builder();
        builder.f439a = this.f433a;
        builder.f440b = this.f434b;
        builder.f441c = this.f435c;
        builder.f442d = this.f436d;
        builder.f443tx = this.f437tx;
        builder.f444ty = this.f438ty;
        builder.addUnknownFields(unknownFields());
        return builder;
    }

    public String toString() {
        StringBuilder sb = new StringBuilder();
        if (this.f433a != null) {
            sb.append(", a=");
            sb.append(this.f433a);
        }
        if (this.f434b != null) {
            sb.append(", b=");
            sb.append(this.f434b);
        }
        if (this.f435c != null) {
            sb.append(", c=");
            sb.append(this.f435c);
        }
        if (this.f436d != null) {
            sb.append(", d=");
            sb.append(this.f436d);
        }
        if (this.f437tx != null) {
            sb.append(", tx=");
            sb.append(this.f437tx);
        }
        if (this.f438ty != null) {
            sb.append(", ty=");
            sb.append(this.f438ty);
        }
        StringBuilder sbReplace = sb.replace(0, 2, "Transform{");
        sbReplace.append('}');
        return sbReplace.toString();
    }

    public Transform(Float f, Float f2, Float f3, Float f4, Float f5, Float f6) {
        this(f, f2, f3, f4, f5, f6, ByteString.EMPTY);
    }
}
