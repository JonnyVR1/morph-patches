...
public class Stylizeface {
    ...
    public static boolean init() {
        if (OsUtils.isWindows()) {
            ...
            System.loadLibrary("mmcv_api_stylizeface");
            return true;
        }
        try {
    ...
    private static native boolean nativeLoadModel_opencl(long j, byte[] bArr, String str);

    private static native boolean nativeProcessFrame(long j, MMFrame mMFrame, StylizefaceParams stylizefaceParams, StylizefaceInfo stylizefaceInfo);
    ...
    public synchronized boolean LoadModel_opencl(byte[] bArr, String str) {
        try {
            if (!initok) {
                return false;
            }
            ...
            long j = this.mOBJPtr;
            if (j != 0 && !this.inited) {
                this.inited = nativeLoadModel_opencl(j, bArr, str);
            }
            return this.inited;
        } catch (Throwable th) {
            throw th;
    ...
    public synchronized boolean ProcessFrame(MMFrame mMFrame, StylizefaceParams stylizefaceParams, StylizefaceInfo stylizefaceInfo) {
        if (!initok) {
            return false;
        }
        if (OsUtils.maceSyncLoading()) {
        ...
        return nativeProcessFrame(j, mMFrame, stylizefaceParams, stylizefaceInfo);
    }

    public synchronized void Release() {
        this.releasing = true;
        new Thread(new Runnable() { // from class: com.momocv.stylizeface.Stylizeface.1
            @Override // java.lang.Runnable
            public void run() {
                Stylizeface.this.ReleaseFunctor();
            }
        }, "StylizefaceRelease").start();
    }
