...
public class Handsg {
    ...
    public static boolean init() {
        if (OsUtils.isWindows()) {
            ...
            System.loadLibrary("mmcv_api_handsg");
            return true;
        }
        try {
    ...
    private static native boolean nativeProcessFrame(long j, MMFrame mMFrame, HandsgParams handsgParams, HandsgInfo handsgInfo);
    ...
    public synchronized boolean ProcessFrame(MMFrame mMFrame, HandsgParams handsgParams, HandsgInfo handsgInfo) {
        if (!initok) {
            return false;
        }
        if (OsUtils.maceSyncLoading()) {
        ...
        return nativeProcessFrame(j, mMFrame, handsgParams, handsgInfo);
    }

    public synchronized void Release() {
        this.releasing = true;
        new Thread(new Runnable() { // from class: com.momocv.handsg.Handsg.1
            @Override // java.lang.Runnable
            public void run() {
                Handsg.this.ReleaseFunctor();
            }
        }, "HandsgRelease").start();
    }
