...
public final class di60 implements qzl {
    ...
    public Pair<CharSequence, Boolean> mo13456a(@NotNull PayMethod payMethod, @NotNull d section, boolean newFilterRule) {
        ...
        section.getClass();
        if (!newFilterRule) {
            return TEnum.equals(payMethod, "alipay") ? new Pair<>(CoreModule.m1854P().m11706a().m5239D6(), Boolean.TRUE) : new Pair<>("随机立减最高8元", Boolean.FALSE);
        }
        List listX = p2b0.h().x(mo13458c(section, this.purchaseType, this.from).s());
        listX.getClass();
        return (TEnum.equals(payMethod, "alipay") && listX.contains(PayMethod.get("alipay"))) ? new Pair<>(CoreModule.m1854P().m11706a().m5239D6(), Boolean.TRUE) : new Pair<>("", Boolean.TRUE);
    }
    ...
    public boolean mo13457b(@NotNull PayMethod payMethod) {
        payMethod.getClass();
        return CoreModule.m1854P().m11706a().m5462jh() && TEnum.equals(payMethod, "alipay");
    }
