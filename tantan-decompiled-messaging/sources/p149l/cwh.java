package p149l;

import android.app.PendingIntent;
import com.p046p1.mobile.putong.api.push.PushTrackData;
import com.p046p1.mobile.putong.app.TantanApp;
import com.p046p1.mobile.putong.data.PushMessage;
import com.p046p1.mobile.putong.data.PushMessageIntent;
import com.p046p1.mobile.putong.feed.FeedModule;
import com.tantanapp.common.utils.NullChecker;

/* JADX INFO: loaded from: classes12.dex */
public class cwh {

    /* JADX INFO: renamed from: l.cwh$a */
    public class C16264a implements ydb0.InterfaceC21375g {
        @Override // p149l.ydb0.InterfaceC21375g
        /* JADX INFO: renamed from: a */
        public boolean mo98941a(String str, String str2, PushMessage pushMessage, PushTrackData pushTrackData) {
            if (qib0.f154713b0.f139230a.signedIn_() && !vqg.m199582y0() && NullChecker.m81303a(FeedModule.f38855d)) {
                return cwh.m109047a(pushMessage, pushTrackData);
            }
            return false;
        }

        @Override // p149l.ydb0.InterfaceC21375g
        /* JADX INFO: renamed from: b */
        public PendingIntent mo98942b(String str, String str2, PushMessage pushMessage, PushTrackData pushTrackData) {
            return w4i.m201509a(pushMessage, pushTrackData, true);
        }
    }

    /* JADX INFO: renamed from: a */
    public static boolean m109047a(PushMessage pushMessage, PushTrackData pushTrackData) {
        boolean z = false;
        if (NullChecker.m81303a(pushMessage)) {
            if (PushMessageIntent.moment_single_like.equals(pushMessage.intent)) {
                ijb0.m136545I();
                FeedModule.f38854c.m60698S2();
                if (!TantanApp.f17180c.m214249N()) {
                    return true;
                }
            } else if (PushMessageIntent.moment_single_comment.equals(pushMessage.intent)) {
                ijb0.m136545I();
                FeedModule.f38854c.m60698S2();
                if (!TantanApp.f17180c.m214249N()) {
                    return true;
                }
            } else {
                if (PushMessageIntent.moment_single_unlike.equals(pushMessage.intent)) {
                    FeedModule.f38854c.m60751s4(rib0.m179488a(), pushMessage.content.parent.f38759id);
                    TantanApp.f17180c.m214249N();
                    return true;
                }
                if (PushMessageIntent.followship_conversation_list.equals(pushMessage.intent)) {
                    ijb0.m136545I();
                    return true;
                }
                if (PushMessageIntent.followship_single.equals(pushMessage.intent)) {
                    ijb0.m136545I();
                } else {
                    if (PushMessageIntent.moment_match_post.equals(pushMessage.intent) || "moment.match.like".equals(pushMessage.intent) || "moment.match.comment".equals(pushMessage.intent) || "moment.match.post.feed".equals(pushMessage.intent) || "state.match.post".equals(pushMessage.intent) || "moment.match.post.like_feed".equals(pushMessage.intent)) {
                    }
                    if (!pushMessage.silent && z) {
                        w4i.m201512d(pushMessage, pushTrackData);
                    }
                }
            }
            z = true;
            if (!pushMessage.silent) {
                w4i.m201512d(pushMessage, pushTrackData);
            }
        }
        return z;
    }

    /* JADX INFO: renamed from: b */
    public static void m109048b() {
        TantanApp.f17180c.m214262a0(new C16264a(), vwb.m200324f0(PushMessageIntent.moment_single_unlike, PushMessageIntent.moment_single_like, PushMessageIntent.moment_single_comment, PushMessageIntent.followship_single, PushMessageIntent.followship_conversation_list, "moment.match.like", "moment.match.comment", PushMessageIntent.moment_match_post, "moment.match.post.feed", "state.match.post", "moment.match.post.like_feed"));
    }
}
