...
public class agw {
    ...
    public static String m96355a(byte[] bArr) throws Exception {
        ...
        while (i < length) {
            ...
            int i3 = b & 255;
            if (i2 == length) {
                stringBuffer.append(f69527b[i3 >>> 2]);
                stringBuffer.append(f69527b[(b & 3) << 4]);
                stringBuffer.append(ijw.m136753a(new byte[]{HttpTokens.CARRIAGE_RETURN, 91}));
                break;
            }
            int i4 = i + 2;
            byte b2 = bArr[i2];
            if (i4 == length) {
                stringBuffer.append(f69527b[i3 >>> 2]);
                stringBuffer.append(f69527b[((b & 3) << 4) | ((b2 & 240) >>> 4)]);
                stringBuffer.append(f69527b[(b2 & 15) << 2]);
            ...
            stringBuffer.append(f69527b[i3 >>> 2]);
            stringBuffer.append(f69527b[((b & 3) << 4) | ((b2 & 240) >>> 4)]);
            stringBuffer.append(f69527b[((b2 & 15) << 2) | ((b3 & 192) >>> 6)]);
            stringBuffer.append(f69527b[b3 & Utf8.REPLACEMENT_BYTE]);
        }
