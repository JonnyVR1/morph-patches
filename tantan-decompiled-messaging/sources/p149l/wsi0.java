...
public class wsi0 extends xj10 {
    ...
    private int[] f187938a;

    /* JADX INFO: renamed from: b */
    private int[] f187939b;

    /* JADX INFO: renamed from: c */
    private int[] f187940c;

    /* JADX INFO: renamed from: d */
    private int[] f187941d;

    /* JADX INFO: renamed from: l.wsi0$a */
    public class C20955a implements Comparator<Point> {
        public C20955a() {
        }
    ...
    public wsi0(Point[] pointArr, Point[] pointArr2, Point[] pointArr3, Point[] pointArr4) {
        super(2);
        float[] fArrM205510R1 = m205510R1(pointArr);
        float[] fArrM205510R2 = m205510R1(pointArr3);
        float[] fArrM205510R3 = m205510R1(pointArr2);
        float[] fArrM205510R4 = m205510R1(pointArr4);
        this.f187938a = new int[256];
        this.f187939b = new int[256];
        this.f187940c = new int[256];
        for (int i = 0; i < 256; i++) {
            float f = i;
            this.f187938a[i] = (int) Math.min(Math.max(fArrM205510R1[i] + f + fArrM205510R4[i], 0.0f), 255.0f);
            this.f187939b[i] = (int) Math.min(Math.max(fArrM205510R3[i] + f + fArrM205510R4[i], 0.0f), 255.0f);
            this.f187940c[i] = (int) Math.min(Math.max(f + fArrM205510R2[i] + fArrM205510R4[i], 0.0f), 255.0f);
        }
    }

    /* JADX INFO: renamed from: Q1 */
    private void m205509Q1() {
        int[] iArr = new int[256];
        for (int i = 0; i < 256; i++) {
            iArr[i] = (this.f187938a[i] & 255) | ((this.f187939b[i] << 8) & 65280) | ((this.f187940c[i] << 16) & 16711680) | RoundedDrawable.DEFAULT_BORDER_COLOR;
        }
        int[] iArr2 = new int[1];
        this.f187941d = iArr2;
        GLES20.glGenTextures(1, iArr2, 0);
        GLES20.glBindTexture(3553, this.f187941d[0]);
    ...
    private float[] m205510R1(Point[] pointArr) {
        Arrays.sort(pointArr, new C20955a());
        List<Point> listM205511S1 = m205511S1(pointArr);
        if (listM205511S1.get(0).x > 0) {
            for (int i = listM205511S1.get(0).x; i >= 0; i--) {
                listM205511S1.add(0, new Point(i, 0));
            }
        }
        if (listM205511S1.get(listM205511S1.size() - 1).x < 255) {
            for (int i2 = listM205511S1.get(listM205511S1.size() - 1).x; i2 < 256; i2++) {
                listM205511S1.add(new Point(i2, 255));
            }
        }
        float[] fArr = new float[listM205511S1.size()];
        for (int i3 = 0; i3 < listM205511S1.size(); i3++) {
            Point point = listM205511S1.get(i3);
            int i4 = point.x;
    ...
    private List<Point> m205511S1(Point[] pointArr) {
        double[] dArrM205512T1 = m205512T1(pointArr);
        int length = dArrM205512T1.length;
        boolean z = true;
        if (length < 1) {
            return null;
        }
    ...
    private double[] m205512T1(Point[] pointArr) {
        int i;
    ...
    public void newTextureReady(int i, mcj mcjVar, boolean z) {
        ...
        super.newTextureReady(this.f187941d[0], this, z);
