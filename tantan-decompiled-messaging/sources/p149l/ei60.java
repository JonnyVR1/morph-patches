...
public final class ei60 implements qzl {
    ...
    public ei60(@NotNull PurchaseType purchaseType, @Nullable String str) {
    ...
    public Pair<CharSequence, Boolean> mo107051a(@NotNull PayMethod payMethod, @NotNull C8765d section, boolean newFilterRule) {
        ...
        if (!newFilterRule) {
            if (sab0.m182899q(purchaseType) && C8766e.m53650x(this.from)) {
        ...
        List<PayMethod> listM167155x = p2b0.m167133h().m167155x(mo107053c(section, purchaseType, this.from).m53601s());
        listM167155x.getClass();
        if ((TEnum.equals(payMethod, "alipay") || TEnum.equals(payMethod, "wechat")) && listM167155x.contains(PayMethod.get(PayMethod.jingdong))) {
            return new Pair<>("京东支付最高立减8元", Boolean.FALSE);
        }
        return (TEnum.equals(payMethod, PayMethod.jingdong) && listM167155x.contains(PayMethod.get(PayMethod.jingdong))) ? new Pair<>("随机立减最高8元", Boolean.TRUE) : new Pair<>("", Boolean.TRUE);
    }
