...
public class CodecInfoCheck {
    ...
    private static byte[] decodeFrame(byte[] bArr) throws IOException {
        ...
        byte[] bArr2 = null;
        try {
            mediaCodecCreateDecoderByType = MediaCodec.createDecoderByType("video/avc");
        } catch (IOException e) {
            e.printStackTrace();
            mediaCodecCreateDecoderByType = null;
        }
        if (mediaCodecCreateDecoderByType != null) {
            MediaFormat mediaFormatCreateVideoFormat = MediaFormat.createVideoFormat("video/avc", 720, 720);
            ...
            if (iDequeueInputBuffer >= 0) {
                ...
                if (bArr != null) {
                    ...
                    int iDequeueOutputBuffer = mediaCodecCreateDecoderByType.dequeueOutputBuffer(bufferInfo, HuiYanOsConfig.LONG_CHECK_OVERSEAS_DEFAULT_TIME_OUT);
                    if (iDequeueOutputBuffer < 0) {
                        iDequeueOutputBuffer = mediaCodecCreateDecoderByType.dequeueOutputBuffer(bufferInfo, HuiYanOsConfig.LONG_CHECK_OVERSEAS_DEFAULT_TIME_OUT);
                    }
                    if (iDequeueOutputBuffer < 0) {
    ...
    private static byte[] encodeYUV(byte[] bArr, int i) throws IOException {
        ...
        if (mediaCodecCreateEncoderByType != null) {
            ...
            if (iDequeueInputBuffer >= 0) {
                ...
                MediaCodec.BufferInfo bufferInfo = new MediaCodec.BufferInfo();
                int iDequeueOutputBuffer = mediaCodecCreateEncoderByType.dequeueOutputBuffer(bufferInfo, HuiYanOsConfig.LONG_CHECK_OVERSEAS_DEFAULT_TIME_OUT);
                if (iDequeueOutputBuffer < 0) {
                    iDequeueOutputBuffer = mediaCodecCreateEncoderByType.dequeueOutputBuffer(bufferInfo, HuiYanOsConfig.LONG_CHECK_OVERSEAS_DEFAULT_TIME_OUT);
                }
                byte[] bArr2 = null;
                byte[] bArr3 = null;
                while (iDequeueOutputBuffer >= 0) {
    ...
    public static boolean isSupportHWEncoding(int i, int i2, int i3, boolean z, Context context) {
        ...
        int i4;
        int supportMaxWidth;
        synchronized ("video/avc") {
            try {
                SharePreferenceUtils sharePreferenceUtils = new SharePreferenceUtils();
                boolean zIsSupportNV12 = isSupportNV12("video/avc");
                z2 = false;
                if (context != null) {
                ...
                int encoderMaxSupportBitrate = getEncoderMaxSupportBitrate("video/avc");
                boolean z4 = encoderMaxSupportBitrate > 0 && i <= encoderMaxSupportBitrate;
                int iIsSupportHeight = isSupportHeight("video/avc", z);
                boolean z5 = iIsSupportHeight > 0 && i3 <= iIsSupportHeight && (supportMaxWidth = getSupportMaxWidth(i3, "video/avc", z)) > 0 && i2 <= supportMaxWidth;
