...
public class Camera1Enumerator implements CameraEnumerator {
    ...
    private static List<CameraEnumerationAndroid.CaptureFormat> enumerateFormats(int i) {
        int i2;
        Logging.m20197d(TAG, "Get supported formats for camera index " + i + "");
        long jElapsedRealtime = SystemClock.elapsedRealtime();
        Camera cameraOpen = null;
        try {
            try {
                Logging.m20197d(TAG, "Opening camera with index " + i);
                ...
                try {
                    ...
                    Logging.m20199e(TAG, "getSupportedFormats() failed on camera index " + i, e);
                }
                Logging.m20197d(TAG, "Get supported formats for camera index " + i + " done. Time spent: " + (SystemClock.elapsedRealtime() - jElapsedRealtime) + " ms.");
                return arrayList;
            } catch (RuntimeException e2) {
                Logging.m20199e(TAG, "Open camera failed on camera index " + i, e2);
    ...
    public static int getCameraIndex(String str) {
        Logging.m20197d(TAG, "getCameraIndex: " + str);
        for (int i = 0; i < Camera.getNumberOfCameras(); i++) {
            if (str.equals(getDeviceName(i))) {
                return i;
            }
        }
        qhg0.m174539a("No such camera: ", str);
        return 0;
    }
    ...
    private static Camera.CameraInfo getCameraInfo(int i) {
        ...
        try {
            ...
            Logging.m20199e(TAG, "getCameraInfo failed on index " + i, e);
            return null;
        }
    }
    ...
    @Override // com.momo.rtcbase.CameraEnumerator
    ...
    public String[] getDeviceNames() {
        ...
        for (int i = 0; i < Camera.getNumberOfCameras(); i++) {
            ...
            if (deviceName != null) {
                ...
                Logging.m20197d(TAG, "Index: " + i + MinimalPrettyPrinter.DEFAULT_ROOT_VALUE_SEPARATOR + deviceName);
            } else {
                Logging.m20198e(TAG, "Index: " + i + ". Failed to query camera name.");
            }
        }
