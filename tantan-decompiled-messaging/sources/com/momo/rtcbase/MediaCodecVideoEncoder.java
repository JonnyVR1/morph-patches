...
public class MediaCodecVideoEncoder {
    ...
    public static class HwEncoderFactory implements VideoEncoderFactory {
        ...
        private static VideoCodecInfo[] getSupportedHardwareCodecs() {
            ...
            if (MediaCodecVideoEncoder.isVp8HwSupported()) {
                Logging.m20197d(MediaCodecVideoEncoder.TAG, "VP8 HW Encoder supported.");
                arrayList.add(new VideoCodecInfo("VP8", new HashMap()));
            }
            if (MediaCodecVideoEncoder.isVp9HwSupported()) {
                Logging.m20197d(MediaCodecVideoEncoder.TAG, "VP9 HW Encoder supported.");
                arrayList.add(new VideoCodecInfo("VP9", new HashMap()));
            }
            if (MediaCodecVideoDecoder.isH264HighProfileHwSupported()) {
                Logging.m20197d(MediaCodecVideoEncoder.TAG, "H.264 High Profile HW Encoder supported.");
                arrayList.add(H264Utils.DEFAULT_H264_HIGH_PROFILE_CODEC);
            }
            if (MediaCodecVideoEncoder.isH264HwSupported()) {
                Logging.m20197d(MediaCodecVideoEncoder.TAG, "H.264 HW Encoder supported.");
        ...
        public VideoEncoder createEncoder(final VideoCodecInfo videoCodecInfo) {
            if (isCodecSupported(this.supportedHardwareCodecs, videoCodecInfo)) {
                Logging.m20197d(MediaCodecVideoEncoder.TAG, "Create HW video encoder for " + videoCodecInfo.name);
            ...
            Logging.m20197d(MediaCodecVideoEncoder.TAG, "No HW video encoder for codec " + videoCodecInfo.name);
            return null;
        }
    ...
    public static void disableH264HwCodec() {
        Logging.m20201w(TAG, "H.264 encoding is disabled by application.");
        hwEncoderDisabledTypes.add("video/avc");
    }

    public static void disableVp8HwCodec() {
        Logging.m20201w(TAG, "VP8 encoding is disabled by application.");
        hwEncoderDisabledTypes.add(VP8_MIME_TYPE);
    }

    public static void disableVp9HwCodec() {
        Logging.m20201w(TAG, "VP9 encoding is disabled by application.");
    ...
    private static EncoderProperties findHwEncoder(String str, MediaCodecProperties[] mediaCodecPropertiesArr, int[] iArr) {
        ...
        if (str.equals("video/avc")) {
            ...
            if (listAsList.contains(str2)) {
                Logging.m20201w(TAG, "Model: " + str2 + " has black listed H.264 encoder.");
                return null;
            }
        }
        int i = 0;
        while (i < MediaCodecList.getCodecCount()) {
            try {
                ...
                Logging.m20199e(TAG, "Cannot retrieve encoder codec info", e);
            ...
            if (codecInfoAt == 0 || !codecInfoAt.isEncoder()) {
                ...
                if (name == 0) {
                    ...
                    Logging.m20200v(TAG, "Found candidate encoder ".concat(name));
                    BitrateAdjustmentType bitrateAdjustmentType = BitrateAdjustmentType.NO_ADJUSTMENT;
                        ...
                        if (name.startsWith(mediaCodecProperties.codecPrefix)) {
                            ...
                            if (i4 >= mediaCodecProperties.minSdk) {
                                ...
                                if (bitrateAdjustmentType2 != BitrateAdjustmentType.NO_ADJUSTMENT) {
                                    Logging.m20201w(TAG, "Codec " + name + " requires bitrate adjustment: " + bitrateAdjustmentType2);
                                    bitrateAdjustmentType = bitrateAdjustmentType2;
                                }
                                z = true;
                                break;
                            }
                            Logging.m20201w(TAG, "Codec " + name + " is disabled due to SDK version " + i4);
                        }
                        i3++;
                    }
                    if (z) {
                        try {
                            ...
                            for (int i5 : capabilitiesForType.colorFormats) {
                                Logging.m20200v(TAG, "   Color: 0x" + Integer.toHexString(i5));
                            }
                            for (int i6 : iArr) {
                                ...
                                while (i7 < length3) {
                                    ...
                                    int i8 = iArr2[i7];
                                    if (i8 == i6) {
                                        Logging.m20197d(TAG, "Found target encoder for mime " + str + " : " + name + ". Color: 0x" + Integer.toHexString(i8) + ". Bitrate adjustment: " + bitrateAdjustmentType);
                            ...
                            encoderProperties = encoderProperties2;
                            Logging.m20199e(TAG, "Cannot retrieve encoder capabilities", e2);
                        }
                    }
                    encoderProperties = encoderProperties2;
                }
    ...
    public static void printStackTrace() {
        ...
        if (stackTrace.length > 0) {
            Logging.m20197d(TAG, "MediaCodecVideoEncoder stacks trace:");
            for (StackTraceElement stackTraceElement : stackTrace) {
                Logging.m20197d(TAG, stackTraceElement.toString());
            }
        }
    }
    ...
    private void reportEncodedFrame(int i) {
        ...
        if (this.bitrateObservationTimeMs > 3000.0d) {
            Logging.m20197d(TAG, "Acc: " + ((int) this.bitrateAccumulator) + ". Max: " + ((int) this.bitrateAccumulatorMax) + ". ExpScale: " + this.bitrateAdjustmentScaleExp);
            ...
            Logging.m20197d(TAG, "Adjusting bitrate scale to " + this.bitrateAdjustmentScaleExp + ". Value: " + getBitrateScale(this.bitrateAdjustmentScaleExp));
    ...
    public static void setEglContext(EglBase.Context context) {
        if (staticEglBase != null) {
            Logging.m20201w(TAG, "Egl context already set.");
    ...
    public static void setErrorCallback(MediaCodecVideoEncoderErrorCallback mediaCodecVideoEncoderErrorCallback) {
        Logging.m20197d(TAG, "Set error callback");
    ...
    private boolean setRates(int i, int i2) {
        ...
        if (bitrateAdjustmentType == BitrateAdjustmentType.FRAMERATE_ADJUSTMENT && i2 > 0) {
            ...
            Logging.m20200v(TAG, "setRates: " + i + " -> " + (bitrateScale / 1000) + " kbps. Fps: " + this.targetFps);
        } else if (bitrateAdjustmentType == bitrateAdjustmentType2) {
            Logging.m20200v(TAG, "setRates: " + i + " kbps. Fps: " + this.targetFps + ". ExpScale: " + this.bitrateAdjustmentScaleExp);
            int i4 = this.bitrateAdjustmentScaleExp;
            if (i4 != 0) {
                bitrateScale = (int) (((double) bitrateScale) * getBitrateScale(i4));
            }
        } else {
            Logging.m20200v(TAG, "setRates: " + i + " kbps. Fps: " + this.targetFps);
        }
        try {
            ...
            Logging.m20199e(TAG, "setRates failed", e);
            return false;
        }
    }
    ...
    public void checkKeyFrameRequired(boolean z, long j) {
        ...
        if (z || z2) {
            if (z) {
                Logging.m20197d(TAG, "Sync frame request");
            } else {
                Logging.m20197d(TAG, "Sync frame forced");
            }
            Bundle bundle = new Bundle();
    ...
    public int dequeueInputBuffer() {
        ...
        try {
            ...
            Logging.m20199e(TAG, "dequeueIntputBuffer failed", e);
            return -2;
        }
    }
    ...
    public OutputBufferInfo dequeueOutputBuffer() {
        ...
        try {
            ...
            if (iDequeueOutputBuffer >= 0 && (bufferInfo.flags & 2) != 0) {
                Logging.m20197d(TAG, "Config frame generated. Offset: " + bufferInfo.offset + ". Size: " + bufferInfo.size);
            ...
            if (z) {
                Logging.m20197d(TAG, "Sync frame generated");
            }
            ...
            Logging.m20197d(TAG, "Appending config frame of size " + this.configData.capacity() + " to output buffer with offset " + bufferInfo.offset + ", size " + bufferInfo.size);
            ...
            Logging.m20199e(TAG, "dequeueOutputBuffer failed", e);
    ...
    public boolean encodeBuffer(boolean z, int i, int i2, long j) {
        ...
        try {
            ...
            Logging.m20199e(TAG, "encodeBuffer failed", e);
            return false;
        }
    }
    ...
    public boolean encodeFrame(long j, boolean z, VideoFrame videoFrame, int i, long j2) {
        ...
        try {
            ...
            Logging.m20199e(TAG, "encodeFrame failed", e);
            return false;
        }
    }
    ...
    public ByteBuffer[] getInputBuffers() {
        ...
        Logging.m20197d(TAG, "Input buffers: " + inputBuffers.length);
        return inputBuffers;
    }
    ...
    public boolean initEncode(VideoCodecType videoCodecType, int i, int i2, int i3, int i4, int i5, boolean z) {
        ...
        Logging.m20197d(TAG, "Java initEncode: " + videoCodecType + ". Profile: " + i + " : " + i2 + " x " + i3 + ". @ " + i4 + " kbps. Fps: " + i5 + ". Encode from texture : " + z);
        this.profile = i;
        this.width = i2;
        this.height = i3;
        if (this.mediaCodecThread != null) {
            upk0.m194883a("Forgot to release()?");
            return false;
        }
        ...
        if (videoCodecType != videoCodecType2) {
            if (videoCodecType == VideoCodecType.VIDEO_CODEC_VP9) {
                ...
                if (videoCodecType != VideoCodecType.VIDEO_CODEC_H264) {
                    ibe.m135253a("initEncode: Non-supported codec ", videoCodecType);
                    return false;
                }
                str = "video/avc";
                ...
                if (i == H264Profile.CONSTRAINED_HIGH.getValue()) {
                    if (findHwEncoder("video/avc", h264HighProfileHwList, z ? supportedSurfaceColorList : supportedColorList) != null) {
                        Logging.m20197d(TAG, "High profile H.264 encoder supported.");
                        z2 = true;
                    } else {
                        Logging.m20197d(TAG, "High profile H.264 encoder requested, but not supported. Use baseline.");
                        z2 = false;
                    }
                } else {
                    z2 = false;
            ...
            if (encoderPropertiesFindHwEncoder != null) {
                ibe.m135253a("Can not find HW encoder for ", videoCodecType);
                return false;
            }
            ...
            Logging.m20197d(TAG, "Color format: " + this.colorFormat + ". Bitrate adjustment: " + this.bitrateAdjustmentType + ". Key frame interval: " + this.forcedKeyFrameMs + " . Initial fps: " + iMin);
            int i7 = i4 * 1000;
            ...
            try {
                ...
                Logging.m20197d(TAG, "  Format: " + mediaFormatCreateVideoFormat);
                ...
                if (mediaCodecCreateByCodecName == null) {
                    Logging.m20198e(TAG, "Can not create media encoder");
                    release();
                    return false;
                }
                ...
                this.outputBuffers = this.mediaCodec.getOutputBuffers();
                Logging.m20197d(TAG, "Output buffers: " + this.outputBuffers.length);
                return true;
            } catch (IllegalStateException e) {
                Logging.m20199e(TAG, "initEncode failed", e);
                release();
                return false;
            }
        }
    ...
    @CalledByNativeUnchecked
    public void release() {
        Logging.m20197d(TAG, "Java releaseEncoder");
        checkOnMediaCodecThread();
        ...
        if (this.mediaCodec != null) {
            ...
            new Thread(new Runnable() { // from class: com.momo.rtcbase.MediaCodecVideoEncoder.1
                ...
                public void run() {
                    Logging.m20197d(MediaCodecVideoEncoder.TAG, "Java releaseEncoder on release thread");
                    try {
                        MediaCodecVideoEncoder.this.mediaCodec.stop();
                    } catch (Exception e) {
                        Logging.m20199e(MediaCodecVideoEncoder.TAG, "Media encoder stop failed", e);
                    }
                    try {
                        MediaCodecVideoEncoder.this.mediaCodec.release();
                    } catch (Exception e2) {
                        Logging.m20199e(MediaCodecVideoEncoder.TAG, "Media encoder release failed", e2);
                        c1CaughtException.f14545e = e2;
                    }
                    Logging.m20197d(MediaCodecVideoEncoder.TAG, "Java releaseEncoder on release thread done");
                    countDownLatch.countDown();
                }
            }).start();
            if (!ThreadUtils.awaitUninterruptibly(countDownLatch, Constants.PN_IMAGE_DOWNLOAD_TIMEOUT_IN_MILLIS)) {
                Logging.m20198e(TAG, "Media encoder release timeout");
                z = true;
            }
        ...
        if (!z) {
            Exception exc = c1CaughtException.f14545e;
            if (exc == null) {
                Logging.m20197d(TAG, "Java releaseEncoder done");
                return;
            } else {
                RuntimeException runtimeException = new RuntimeException(exc);
                runtimeException.setStackTrace(ThreadUtils.concatStackTraces(c1CaughtException.f14545e.getStackTrace(), runtimeException.getStackTrace()));
                throw runtimeException;
            }
        }
        codecErrors++;
        if (errorCallback != null) {
            Logging.m20198e(TAG, "Invoke codec error callback. Errors: " + codecErrors);
            errorCallback.onMediaCodecVideoEncoderCriticalError(codecErrors);
        }
        upk0.m194883a("Media encoder release timeout.");
    }
    ...
    public boolean releaseOutputBuffer(int i) {
        ...
        try {
            ...
            Logging.m20199e(TAG, "releaseOutputBuffer failed", e);
            return false;
        }
    }
}
