...
public class WebRtcAudioManager {
    ...
    public static class VolumeLogger {
        ...
        public class LogVolumeTask extends TimerTask {
            ...
            public void run() {
                ...
                if (mode == 1) {
                    Logging.m20197d(WebRtcAudioManager.TAG, "STREAM_RING stream volume: " + VolumeLogger.this.audioManager.getStreamVolume(2) + " (max=" + this.maxRingVolume + ")");
                    return;
                }
                if (mode == 3) {
                    Logging.m20197d(WebRtcAudioManager.TAG, "VOICE_CALL stream volume: " + VolumeLogger.this.audioManager.getStreamVolume(0) + " (max=" + this.maxVoiceCallVolume + ")");
                }
            }
        }
        ...
        public void start() {
            ...
            this.timer = timer;
            timer.schedule(new LogVolumeTask(this.audioManager.getStreamMaxVolume(2), this.audioManager.getStreamMaxVolume(3)), 0L, HuiYanOsConfig.LONG_CHECK_OVERSEAS_DEFAULT_TIME_OUT);
        }
    }
    ...
    private void dispose() {
        Logging.m20197d(TAG, "dispose" + WebRtcAudioUtils.getThreadInfo());
        if (this.initialized) {
    ...
    private int getNativeOutputSampleRate() {
        if (WebRtcAudioUtils.runningOnEmulator()) {
            Logging.m20197d(TAG, "Running emulator, overriding sample rate to 8 kHz.");
            return TXRecordCommon.AUDIO_SAMPLERATE_8000;
        }
        if (WebRtcAudioUtils.isDefaultSampleRateOverridden()) {
            Logging.m20197d(TAG, "Default sample rate is overriden to " + WebRtcAudioUtils.getDefaultSampleRateHz() + " Hz");
            return WebRtcAudioUtils.getDefaultSampleRateHz();
        }
        int sampleRateForApiLevel = getSampleRateForApiLevel();
        Logging.m20197d(TAG, "Sample rate is set to " + sampleRateForApiLevel + " Hz");
    ...
    private boolean init() {
        Logging.m20197d(TAG, "init" + WebRtcAudioUtils.getThreadInfo());
        if (this.initialized) {
            return true;
        }
        Logging.m20197d(TAG, "audio mode is: " + WebRtcAudioUtils.modeToString(this.audioManager.getMode()));
    ...
    private boolean isAAudioSupported() {
        Logging.m20201w(TAG, "AAudio support is currently disabled on all devices!");
        return false;
    }
    ...
    private boolean isDeviceBlacklistedForOpenSLESUsage() {
        ...
        if (zDeviceIsBlacklistedForOpenSLESUsage) {
            Logging.m20197d(TAG, Build.MODEL + " is blacklisted for OpenSL ES usage!");
        }
    ...
    public static synchronized void setStereoInput(boolean z) {
        Logging.m20201w(TAG, "Overriding default input behavior: setStereoInput(" + z + ')');
        useStereoInput = z;
    }

    public static synchronized void setStereoOutput(boolean z) {
        Logging.m20201w(TAG, "Overriding default output behavior: setStereoOutput(" + z + ')');
