...
public final class WebRtcAudioUtils {
    ...
    private static void logAudioDeviceInfo(String str, AudioManager audioManager) {
        ...
        Logging.m20197d(str, "Audio Devices: ");
        for (AudioDeviceInfo audioDeviceInfo : devices) {
            ...
            Logging.m20197d(str, sb.toString());
        }
    }
    ...
    private static void logAudioStateBasic(String str, AudioManager audioManager) {
        Logging.m20197d(str, "Audio State: audio mode: " + modeToString(audioManager.getMode()) + ", has mic: " + hasMicrophone() + ", mic muted: " + audioManager.isMicrophoneMute() + ", music active: " + audioManager.isMusicActive() + ", speakerphone: " + audioManager.isSpeakerphoneOn() + ", BT SCO: " + audioManager.isBluetoothScoOn());
    }

    private static void logAudioStateVolume(String str, AudioManager audioManager) {
        int[] iArr = {0, 3, 2, 4, 5, 1};
        Logging.m20197d(str, "Audio State: ");
        boolean zIsVolumeFixed = isVolumeFixed(audioManager);
        Logging.m20197d(str, "  fixed volume=" + zIsVolumeFixed);
        if (zIsVolumeFixed) {
            return;
        }
        for (int i = 0; i < 6; i++) {
    ...
    public static void logDeviceInfo(String str) {
        Logging.m20197d(str, "Android SDK: " + Build.VERSION.SDK_INT + ", Release: " + Build.VERSION.RELEASE + ", Brand: " + Build.BRAND + ", Device: " + Build.DEVICE + ", Id: " + Build.ID + ", Hardware: " + Build.HARDWARE + ", Manufacturer: " + Build.MANUFACTURER + ", Model: " + Build.MODEL + ", Product: " + Build.PRODUCT);
    }
    ...
    public static synchronized void setWebRtcBasedAutomaticGainControl(boolean z) {
        Logging.m20201w(TAG, "setWebRtcBasedAutomaticGainControl() is deprecated");
    }
    ...
    public static synchronized boolean useWebRtcBasedAcousticEchoCanceler() {
        try {
            if (useWebRtcBasedAcousticEchoCanceler) {
                Logging.m20201w(TAG, "Overriding default behavior; now using WebRTC AEC!");
            }
        } catch (Throwable th) {
            throw th;
        }
    ...
    public static synchronized boolean useWebRtcBasedNoiseSuppressor() {
        try {
            if (useWebRtcBasedNoiseSuppressor) {
                Logging.m20201w(TAG, "Overriding default behavior; now using WebRTC NS!");
            }
        } catch (Throwable th) {
            throw th;
        }
