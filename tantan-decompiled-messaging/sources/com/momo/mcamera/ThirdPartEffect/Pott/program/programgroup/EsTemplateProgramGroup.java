...
public abstract class EsTemplateProgramGroup implements ProgramGroupRenderInterface, q7m {
    ...
    public void addRunerTask(n23 n23Var, MMFrameGeomeAttrInfo mMFrameGeomeAttrInfo) {
        this.renderRunners.add(new RenderRunner(mMFrameGeomeAttrInfo, n23Var));
    }
    ...
    public synchronized void drawGroup() {
        if (!this.initialized) {
            MDLog.m7399w(MDLogTag.FILTER_TAG, "Current render group is not initilaized !! Error !!");
            return;
        }
    ...
    public boolean initAllInfo() {
        if (!initProgram()) {
            MDLog.m7391e(MDLogTag.FILTER_TAG, "Init all program error !!");
            return false;
        }
        if (parseConfig()) {
            return true;
        }
        MDLog.m7391e(MDLogTag.FILTER_TAG, "Parse config  error !!");
        return false;
    }
    ...
    public boolean parseConfig() {
        ...
        for (Layers layers : this.lottieConfig.getLayers()) {
            ...
            if (!onParseLayers(this.lottieConfig, layers, (AssetValue) map.get(layers.getRefImageId()))) {
                MDLog.m7391e(MDLogTag.FILTER_TAG, "Parse lottie layers failed !! return !");
                recycleAllData();
                return false;
            }
        }
    ...
    public void setErrorListener(fc90 fc90Var) {
    }
}
