...
public class BodyWarpFilter extends FaceDetectFilter {
    private List<pjw> mvpLists = Collections.synchronizedList(new ArrayList());
    ...
    @Override // p149l.mcj
    public void drawSub() {
        if (this.mvpLists.size() <= 0 || !this.mvpLists.get(0).m169957u()) {
            ...
            GLES20.glUniform1i(this.textureHandle, 0);
            GLES20.glUseProgram(this.programHandle);
            float[] fArr = this.mvpLists.get(0).m169943d().src_warp_points_;
            int length = fArr.length;
            int length2 = fArr.length / 6;
            float[] fArr2 = this.mvpLists.get(0).m169943d().dst_warp_points_;
