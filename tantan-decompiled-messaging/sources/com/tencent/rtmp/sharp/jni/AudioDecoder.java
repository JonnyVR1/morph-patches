...
public class AudioDecoder {
        ...
        try {
            ...
            if (this.mediaExtractor.getTrackCount() > 1) {
                if (QLog.isColorLevel()) {
                    QLog.m84595w("TRAE", 2, "m_nIndex: " + this.m_nIndex + " initMediaDecode mediaExtractor container video, getTrackCount: " + this.mediaExtractor.getTrackCount());
                }
                this.codeOver = true;
                return -2;
            }
            for (int i2 = 0; i2 < this.mediaExtractor.getTrackCount(); i2++) {
                ...
                if (QLog.isColorLevel()) {
                    QLog.m84595w("TRAE", 2, "m_nIndex: " + this.m_nIndex + " initMediaDecode mediaExtractor audio type:" + string);
                }
                if (string.startsWith(DLNAProfiles.DLNAMimeTypes.MIME_AUDIO_MPEG)) {
                    ...
                    QLog.m84595w("TRAE", 2, "m_nIndex: " + this.m_nIndex + " initMediaDecode open succeed, mp3 format:(" + this.sampleRate + Constants.SEPARATOR_COMMA + this.channels + "), fileTotalMs:" + this.fileTotalMs + "ms RingBufferFrame:" + i);
                    break;
                }
            }
            ...
            TXCLog.m82967e("AudioDecoder", "init media decode failed.", e);
            this.codeOver = true;
            return -1;
        }
    }
    ...
    private void srcAudioFormatToPCM() {
        ...
        if (this.decodeInputBuffers.length <= 1) {
            if (QLog.isColorLevel()) {
                QLog.m84595w("TRAE", 2, "m_nIndex: " + this.m_nIndex + " srcAudioFormatToPCM decodeInputBuffers.length to small," + this.decodeInputBuffers.length);
            }
            this.codeOver = true;
            return;
        }
        ...
        if (iDequeueInputBuffer < 0) {
            if (QLog.isColorLevel()) {
                QLog.m84595w("TRAE", 2, "m_nIndex: " + this.m_nIndex + " srcAudioFormatToPCM decodeInputBuffers.inputIndex <0");
            }
            this.codeOver = true;
            return;
        }
        ...
        if (sampleData < 0) {
            if (QLog.isColorLevel()) {
                QLog.m84595w("TRAE", 2, "m_nIndex: " + this.m_nIndex + " srcAudioFormatToPCM readSampleData over,end");
            }
            this.codeOver = true;
        } else {
            ...
            try {
                ...
                if (ringBuffer != null && (i = this.decodeBufferInfo.size) > 0) {
                    ...
                    if (i2 > 0 && QLog.isColorLevel()) {
                        QLog.m84595w("TRAE", 2, "m_nIndex: " + this.m_nIndex + " DecodeOneFrame size: " + this.decodeBufferInfo.size + " Remain: " + (this.decRingBuffer.RemainRead() / this.nFrameSize));
                    }
                }
                ...
                if (QLog.isColorLevel()) {
                    QLog.m84595w("TRAE", 2, "m_nIndex: " + this.m_nIndex + " srcAudioFormatToPCM wrong outputIndex: " + iDequeueOutputBuffer);
                }
                this.codeOver = true;
                return;
            }
    ...
    public int ReadOneFrame(byte[] bArr, int i) {
        ...
        if (!this.IsTenFramesReady) {
            ...
            if (QLog.isColorLevel()) {
                QLog.m84595w("TRAE", 2, "m_nIndex: " + this.m_nIndex + " 10 FramesReady Remain frame: " + (this.decRingBuffer.RemainRead() / this.nFrameSize));
            }
    ...
    public int SeekTo(int i) {
        ...
        if (QLog.isColorLevel()) {
            QLog.m84595w("TRAE", 2, "m_nIndex: " + this.m_nIndex + " current PlayMs: " + (sampleTime / 1000) + " SeekTo: " + iRemainRead);
        }
        ...
        if (QLog.isColorLevel()) {
            QLog.m84595w("TRAE", 2, "m_nIndex: " + this.m_nIndex + " total SeekTo time: " + i2 + " t2:" + (sampleTime2 / 1000));
        }
        return i2;
    }
