...
class WXApiImplComm {
    ...
    private WXApiImplComm() {
        throw new RuntimeException(WXApiImplComm.class.getSimpleName().concat(" should not be instantiated"));
    }
        ...
        if (z) {
            for (Signature signature : signatureArr) {
                if (signature != null) {
                    ...
                    Log.m84174d(TAG, "check signature:" + lowerCase);
                    if (lowerCase.equals(WX_APP_SIGNATURE)) {
                        str = StateEvent.ActionValue.STAGE_PASS;
                    }
                }
            }
            return false;
    ...
    public static boolean validateAppSignatureForPackage(Context context, String str, boolean z) {
        ...
        Log.m84174d(TAG, "ignore wechat app signature validation");
        return true;
    }
}
