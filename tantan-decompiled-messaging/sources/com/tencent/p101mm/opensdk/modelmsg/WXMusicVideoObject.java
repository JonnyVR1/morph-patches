package com.tencent.p101mm.opensdk.modelmsg;
...
public class WXMusicVideoObject implements WXMediaMessage.IMediaObject {
    ...
    private int getFileSize(String str) {
        return C14340b.m84180a(str);
    }

    @Override // com.tencent.mm.opensdk.modelmsg.WXMediaMessage.IMediaObject
    public boolean checkArgs() {
        String str;
        if (C14340b.m84183b(this.musicUrl) || this.musicUrl.length() > URL_LENGTH_LIMIT) {
            str = "musicUrl.length exceeds the limit";
        } else if (C14340b.m84183b(this.musicDataUrl) || this.musicDataUrl.length() > URL_LENGTH_LIMIT) {
            str = "musicDataUrl.length exceeds the limit";
        } else if (C14340b.m84183b(this.singerName) || this.singerName.length() > 1024) {
            str = "singerName.length exceeds the limit";
        } else if (!C14340b.m84183b(this.songLyric) && this.songLyric.length() > 32768) {
            str = "songLyric.length exceeds the limit";
        } else if (!C14340b.m84183b(this.hdAlbumThumbFilePath) && this.hdAlbumThumbFilePath.length() > 1024) {
            str = "hdAlbumThumbFilePath.length exceeds the limit";
        } else if (!C14340b.m84183b(this.hdAlbumThumbFilePath) && getFileSize(this.hdAlbumThumbFilePath) > HD_ALBUM_FILE_LENGTH) {
            str = "hdAlbumThumbFilePath file length exceeds the limit";
        } else if (!C14340b.m84183b(this.musicGenre) && this.musicGenre.length() > 1024) {
            str = "musicGenre.length exceeds the limit";
        } else if (!C14340b.m84183b(this.identification) && this.identification.length() > 1024) {
            str = "identification.length exceeds the limit";
        } else {
            if (C14340b.m84183b(this.musicOperationUrl) || this.musicOperationUrl.length() <= URL_LENGTH_LIMIT) {
                return true;
            }
