...
public class TXCAudioSysRecord implements Runnable {
    ...
    private WeakReference<InterfaceC13990c> mWeakRefListener;
    ...
    private void init() {
        ...
        String str = TAG;
        TXCLog.m82969i(str, String.format("audio record sampleRate = %d, channels = %d, bits = %d, aectype = %d", Integer.valueOf(i), Integer.valueOf(i2), Integer.valueOf(i3), Integer.valueOf(i4)));
        int i5 = i2 == 1 ? 16 : 12;
        int i6 = i3 == 8 ? 3 : 2;
        int minBufferSize = AudioRecord.getMinBufferSize(i, i5, i6);
        try {
            TXCLog.m82969i(str, "audio record type: system normal");
            this.mMic = new AudioRecord(1, i, i5, i6, minBufferSize * 2);
        } catch (IllegalArgumentException e) {
            TXCLog.m82967e(TAG, "create AudioRecord failed.", e);
        }
        AudioRecord audioRecord = this.mMic;
        if (audioRecord == null || audioRecord.getState() != 1) {
            TXCLog.m82966e(TAG, "audio record: initialize the mic failed.");
            uninit();
        ...
        TXCLog.m82969i(TAG, String.format("audio record: mic open rate=%dHZ, channels=%d, bits=%d, buffer=%d/%d, state=%d", Integer.valueOf(i), Integer.valueOf(i2), Integer.valueOf(i3), Integer.valueOf(minBufferSize), Integer.valueOf(this.mRecordBuffer.length), Integer.valueOf(this.mMic.getState())));
        AudioRecord audioRecord2 = this.mMic;
        if (audioRecord2 != null) {
            try {
                audioRecord2.startRecording();
            } catch (Exception e2) {
                TXCLog.m82967e(TAG, "mic startRecording failed.", e2);
    ...
    private void onRecordError(int i, String str) {
        ...
        synchronized (this) {
            try {
                WeakReference<InterfaceC13990c> weakReference = this.mWeakRefListener;
        ...
        if (interfaceC13990c != null) {
            interfaceC13990c.onAudioRecordError(i, str);
        } else {
            TXCLog.m82966e(TAG, "onRecordError:no callback");
        }
    }

    private void onRecordPcmData(byte[] bArr, int i, long j) {
        WeakReference<InterfaceC13990c> weakReference = this.mWeakRefListener;
        InterfaceC13990c interfaceC13990c = weakReference != null ? weakReference.get() : null;
        if (interfaceC13990c != null) {
            interfaceC13990c.onAudioRecordPCM(bArr, i, j);
        } else {
            TXCLog.m82966e(TAG, "onRecordPcmData:no callback");
        }
    }
    ...
    private void onRecordStart() {
        ...
        if (interfaceC13990c != null) {
            ...
            TXCLog.m82966e(TAG, "onRecordStart:no callback");
        }
    }
    ...
    private void onRecordStop() {
        ...
        if (interfaceC13990c != null) {
            ...
            TXCLog.m82966e(TAG, "onRecordStop:no callback");
        }
    }

    private void uninit() {
        if (this.mMic != null) {
            TXCLog.m82969i(TAG, "stop mic");
            try {
                this.mMic.setRecordPositionUpdateListener(null);
                this.mMic.stop();
                this.mMic.release();
            } catch (Exception e) {
                TXCLog.m82967e(TAG, "stop AudioRecord failed.", e);
            }
        }
        this.mMic = null;
    ...
    public void pause(boolean z) {
        TXCLog.m82969i(TAG, "system audio record pause");
        this.mPause.set(true);
        this.mSendMuteData = z;
    }

    public void resume() {
        TXCLog.m82969i(TAG, "system audio record resume");
    ...
    public void run() {
        ...
        if (!this.mIsRunning) {
            TXCLog.m82973w(TAG, "audio record: abandom start audio sys record thread!");
            return;
        }
        onRecordStart();
        TXCLog.m82969i(TAG, "start capture audio data ...,mIsRunning:" + this.mIsRunning + " Thread.interrupted:" + Thread.interrupted() + " mMic:" + this.mMic);
        init();
        int i = 0;
        int i2 = 0;
        while (this.mIsRunning && !Thread.interrupted() && this.mMic != null && i <= 5) {
            ...
            if (i3 == this.mRecordBuffer.length - i2) {
                ...
                TXCLog.m82966e(TAG, "read pcm error, len =" + i3);
                i++;
            } else {
                i2 += i3;
            }
        ...
        TXCLog.m82964d(TAG, "stop capture audio data ...,mIsRunning:" + tXCAudioSysRecord2.mIsRunning + " mMic:" + tXCAudioSysRecord2.mMic + " nFailedCount:" + i);
    ...
    public synchronized void setAudioRecordListener(InterfaceC13990c interfaceC13990c) {
        try {
            if (interfaceC13990c == null) {
                this.mWeakRefListener = null;
            } else {
                this.mWeakRefListener = new WeakReference<>(interfaceC13990c);
            }
        } catch (Throwable th) {
            throw th;
        }
    ...
    public void start(int i, int i2, int i3) {
        ...
        TXCLog.m82969i(str, "start ok");
    }

    public void stop() {
        String str;
        TXCLog.m82969i(TAG, "stop");
        synchronized (this.threadMutex) {
            ...
            if (thread == null || !thread.isAlive() || Thread.currentThread().getId() == this.mRecordThread.getId()) {
                str = TAG;
                TXCLog.m82969i(str, "stop ok,stop record cost time(MS): " + (System.currentTimeMillis() - jCurrentTimeMillis));
                this.mRecordThread = null;
            } else {
                try {
                    this.mRecordThread.join();
                } catch (Exception e) {
                    TXCLog.m82966e(TAG, "record stop Exception: " + e.getMessage());
                }
                str = TAG;
                TXCLog.m82969i(str, "stop ok,stop record cost time(MS): " + (System.currentTimeMillis() - jCurrentTimeMillis));
                this.mRecordThread = null;
