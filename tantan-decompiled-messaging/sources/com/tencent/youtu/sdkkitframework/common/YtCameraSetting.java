...
public class YtCameraSetting {
    ...
    public static int chooseFixedPreviewFps(Camera.Parameters parameters, int i) {
        ...
        while (true) {
            ...
            int[] next = it.next();
            String str3 = TAG;
            YtLogger.m84637d(str3, "entry: " + next[0] + " - " + next[1]);
            int i6 = next[0];
            int i7 = next[1];
            if (i6 == i7 && i6 == i) {
                parameters.setPreviewFpsRange(i6, i7);
                YtLogger.m84637d(str3, "use preview fps range: " + next[0] + MinimalPrettyPrinter.DEFAULT_ROOT_VALUE_SEPARATOR + next[1]);
                return next[0];
            }
        }
    }
    ...
    public static Camera.Size getOptimalPreviewSize(List<Camera.Size> list, int i, int i2) {
        ...
        YtLogger.m84637d(TAG, "sizes size=" + list.size());
        ...
        if (size == null) {
            YtLogger.m84637d(TAG, "No preview size match the aspect ratio");
    ...
    public static int getRotateTag(int i, int i2) {
        ...
        if (i == 90) {
            ...
            YtLogger.m84639i(TAG, "camera rotate not 90degree or 180degree, input: " + i);
            i3 = 1;
        }
    ...
    public static int getVideoRotate(Context context, int i) {
        ...
        YtLogger.m84639i(TAG, "debug camera orientation is " + cameraInfo.orientation + " ui degrees is " + i2);
        return i5;
    }
    ...
    public static int initCamera(Context context, Camera camera, int i, int i2, boolean z) {
        ...
        try {
            ...
            for (int i3 = 0; i3 < supportedFocusModes.size(); i3++) {
                YtLogger.m84641v(TAG, "suporrtedFocusModes " + i3 + " :" + supportedFocusModes.get(i3));
            }
            if (supportedFocusModes.indexOf("continuous-video") >= 0) {
                parameters.setFocusMode("continuous-video");
                YtLogger.m84637d(TAG, "set camera focus mode continuous video");
            } else if (supportedFocusModes.indexOf("auto") >= 0) {
                parameters.setFocusMode("auto");
                YtLogger.m84637d(TAG, "set camera focus mode auto");
            } else {
                YtLogger.m84637d(TAG, "NOT set camera focus mode");
            }
            try {
                try {
                ...
                YtLogger.m84638e(TAG, "Camera.setParameters.setPreviewSize failed!!: ", e);
            }
            Camera.Parameters parameters2 = camera.getParameters();
            int videoRotate = getVideoRotate(context, i);
            camera.setDisplayOrientation(videoRotate);
            String str = TAG;
            YtLogger.m84637d(str, "videoOrietation is" + videoRotate);
            if (CamcorderProfile.hasProfile(i, i2)) {
                camcorderProfile = CamcorderProfile.get(i, i2);
                YtLogger.m84637d(str, "custom camcorderProfile:" + camcorderProfile.videoFrameWidth + BaseSei.f13930X + camcorderProfile.videoFrameHeight);
            } else if (CamcorderProfile.hasProfile(i, 4)) {
                camcorderProfile = CamcorderProfile.get(i, 4);
                YtLogger.m84637d(str, "480P camcorderProfile:" + camcorderProfile.videoFrameWidth + BaseSei.f13930X + camcorderProfile.videoFrameHeight);
            } else if (CamcorderProfile.hasProfile(i, 5)) {
                camcorderProfile = CamcorderProfile.get(i, 5);
                YtLogger.m84637d(str, "720P camcorderProfile:" + camcorderProfile.videoFrameWidth + BaseSei.f13930X + camcorderProfile.videoFrameHeight);
            } else {
                camcorderProfile = CamcorderProfile.get(i, 1);
                YtLogger.m84637d(str, "High camcorderProfile:" + camcorderProfile.videoFrameWidth + BaseSei.f13930X + camcorderProfile.videoFrameHeight);
            }
            List<Camera.Size> supportedPictureSizes = camera.getParameters().getSupportedPictureSizes();
            List<Camera.Size> supportedPreviewSizes = camera.getParameters().getSupportedPreviewSizes();
            for (int i4 = 0; i4 < supportedPictureSizes.size(); i4++) {
                Camera.Size size = supportedPictureSizes.get(i4);
                YtLogger.m84637d("pictureSize", size.width + " x " + size.height);
            }
            for (int i5 = 0; i5 < supportedPreviewSizes.size(); i5++) {
                Camera.Size size2 = supportedPreviewSizes.get(i5);
                YtLogger.m84637d("previewSize", size2.width + " x " + size2.height);
            }
            ...
            YtLogger.m84637d(TAG, "mDesiredPreviewWidth: " + mDesiredPreviewWidth + ", mDesiredPreviewHeight: " + mDesiredPreviewHeight);
            parameters2.setPreviewSize(mDesiredPreviewWidth, mDesiredPreviewHeight);
            parameters2.setPreviewFormat(17);
            try {
                camera.setParameters(parameters2);
            } catch (Exception e2) {
                YtLogger.m84638e(TAG, "Camera.setParameters.setPreviewSize failed!!: ", e2);
            }
            Camera.Parameters parameters3 = camera.getParameters();
            int iChooseFixedPreviewFps = chooseFixedPreviewFps(parameters3, customFPS);
            YtLogger.m84637d(TAG, "choose camera fps is : " + iChooseFixedPreviewFps);
            try {
                camera.setParameters(parameters3);
            } catch (Exception e3) {
                YtLogger.m84638e(TAG, "Camera.setParameters.preview fps failed!!: ", e3);
            }
            ...
            String str2 = TAG;
            YtLogger.m84637d(str2, "after set parameters getPreviewFpsRange=" + iArr[0] + "-" + iArr[1] + " ;after set parameter fps=" + previewFrameRate);
            ...
            YtLogger.m84637d(str2, sb.toString());
            return 0;
        } catch (Exception e4) {
            YtLogger.m84638e(TAG, "get camera parameters failed. 1. Check Camera.getParameters() interface. 2. Get logs for more detail.", e4);
            return 1;
        }
    }
    ...
    public static void setVideoSize(Camera.Parameters parameters, CamcorderProfile camcorderProfile) {
        ...
        if (parameters.getSupportedVideoSizes() == null) {
            String str = TAG;
            YtLogger.m84637d(str, "video size from profile is : " + camcorderProfile.videoFrameWidth + MinimalPrettyPrinter.DEFAULT_ROOT_VALUE_SEPARATOR + camcorderProfile.videoFrameHeight);
            if (getOptimalPreviewSize(supportedPreviewSizes, camcorderProfile.videoFrameWidth, camcorderProfile.videoFrameHeight) == null) {
                YtLogger.m84637d(str, "do not find proper preview size, use default");
        ...
        YtLogger.m84637d(TAG, "select video size camcorderProfile:" + camcorderProfile.videoFrameWidth + BaseSei.f13930X + camcorderProfile.videoFrameHeight);
    }
    ...
    public static int transBackFacingCameraRatateTag(int i) {
        ...
        YtLogger.m84642w(TAG, "[YtCameraSetting.transBackFacingCameraRatateTag] unsurported rotateTag: " + i, null);
        return 0;
    }
