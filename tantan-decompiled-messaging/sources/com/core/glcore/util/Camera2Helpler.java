...
public class Camera2Helpler {
    ...
    private static Size chooseOptimalSize(List<Size> list, Size size, int i, float f) {
        ...
        Log4Cam.m7325i("Camera2 Size: " + size3.getWidth() + ", " + size3.getHeight());
        return size3;
    }
    ...
    public static void selectCameraFpsRange(CameraCharacteristics cameraCharacteristics, ulw ulwVar) {
        ...
        final int i = ulwVar.f177107F;
        if (ulwVar.f177149k0.size() > 0) {
            ulwVar.f177149k0.clear();
        }
        ...
        ulwVar.f177122U = ((Integer) ((Range) listAsList.get(0)).getLower()).intValue();
        ulwVar.f177121T = ((Integer) ((Range) listAsList.get(0)).getUpper()).intValue();
        for (Range range : listAsList) {
            ulwVar.f177149k0.add(new int[]{((Integer) range.getLower()).intValue() * 1000, ((Integer) range.getUpper()).intValue() * 1000});
        }
        Log4Cam.m7322e("Camera2 select fps Range: " + ulwVar.f177122U + "-" + ulwVar.f177121T);
    }

    public static Size selectCameraPreviewWH(CameraCharacteristics cameraCharacteristics, ulw ulwVar, Size size) {
        Size[] outputSizes = ((StreamConfigurationMap) cameraCharacteristics.get(CameraCharacteristics.SCALER_STREAM_CONFIGURATION_MAP)).getOutputSizes(SurfaceTexture.class);
        if (outputSizes == null || outputSizes.length == 0) {
            return null;
        }
        return chooseOptimalSize(Arrays.asList(outputSizes), size, ulwVar.f177125X, ulwVar.f177126Y);
    }
}
