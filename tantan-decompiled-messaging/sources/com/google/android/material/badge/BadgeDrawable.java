...
public class BadgeDrawable extends Drawable implements TextDrawableHelper.TextDrawableDelegate {
    ...
    @StyleRes
    private static final int DEFAULT_STYLE = C2466R.style.Widget_MaterialComponents_Badge;

    @AttrRes
    private static final int DEFAULT_THEME_ATTR = C2466R.attr.badgeStyle;
    ...
    private BadgeDrawable(@NonNull Context context) {
        ...
        this.badgeRadius = resources.getDimensionPixelSize(C2466R.dimen.mtrl_badge_radius);
        this.badgeWidePadding = resources.getDimensionPixelSize(C2466R.dimen.mtrl_badge_long_text_horizontal_padding);
        this.badgeWithTextRadius = resources.getDimensionPixelSize(C2466R.dimen.mtrl_badge_with_text_radius);
        TextDrawableHelper textDrawableHelper = new TextDrawableHelper(this);
        this.textDrawableHelper = textDrawableHelper;
        textDrawableHelper.getTextPaint().setTextAlign(Paint.Align.CENTER);
        this.savedState = new SavedState(context);
        setTextAppearanceResource(C2466R.style.TextAppearance_MaterialComponents_Badge);
    }
    ...
    private void calculateCenterAndBounds(@NonNull Context context, @NonNull Rect rect, @NonNull View view) {
        ...
        int dimensionPixelSize = context.getResources().getDimensionPixelSize(hasNumber() ? C2466R.dimen.mtrl_badge_text_horizontal_edge_offset : C2466R.dimen.mtrl_badge_horizontal_edge_offset);
        int i2 = this.savedState.badgeGravity;
        if (i2 == 8388659 || i2 == 8388691) {
            this.badgeCenterX = gbl0.m125228x(view) == 0 ? (rect.left - this.halfBadgeWidth) + dimensionPixelSize : (rect.right + this.halfBadgeWidth) - dimensionPixelSize;
        } else {
            this.badgeCenterX = gbl0.m125228x(view) == 0 ? (rect.right + this.halfBadgeWidth) - dimensionPixelSize : (rect.left - this.halfBadgeWidth) + dimensionPixelSize;
        }
    }
    ...
    private String getBadgeText() {
        ...
        return context == null ? "" : context.getString(C2466R.string.mtrl_exceed_max_badge_number_suffix, Integer.valueOf(this.maxBadgeNumber), "+");
    }

    private void loadDefaultStateFromAttributes(Context context, AttributeSet attributeSet, @AttrRes int i, @StyleRes int i2) {
        TypedArray typedArrayObtainStyledAttributes = ThemeEnforcement.obtainStyledAttributes(context, attributeSet, C2466R.styleable.Badge, i, i2, new int[0]);
        setMaxCharacterCount(typedArrayObtainStyledAttributes.getInt(C2466R.styleable.Badge_maxCharacterCount, 4));
        if (typedArrayObtainStyledAttributes.hasValue(C2466R.styleable.Badge_number)) {
            setNumber(typedArrayObtainStyledAttributes.getInt(C2466R.styleable.Badge_number, 0));
        }
        setBackgroundColor(readColorFromAttributes(context, typedArrayObtainStyledAttributes, C2466R.styleable.Badge_backgroundColor));
        if (typedArrayObtainStyledAttributes.hasValue(C2466R.styleable.Badge_badgeTextColor)) {
            setBadgeTextColor(readColorFromAttributes(context, typedArrayObtainStyledAttributes, C2466R.styleable.Badge_badgeTextColor));
        }
        setBadgeGravity(typedArrayObtainStyledAttributes.getInt(C2466R.styleable.Badge_badgeGravity, TOP_END));
    ...
    public static final class SavedState implements Parcelable {
        ...
        public SavedState(@NonNull Context context) {
            this.alpha = 255;
            this.number = -1;
            this.badgeTextColor = new TextAppearance(context, C2466R.style.TextAppearance_MaterialComponents_Badge).textColor.getDefaultColor();
            this.contentDescriptionNumberless = context.getString(C2466R.string.mtrl_badge_numberless_content_description);
            this.contentDescriptionQuantityStrings = C2466R.plurals.mtrl_badge_content_description;
        }
    }
}
