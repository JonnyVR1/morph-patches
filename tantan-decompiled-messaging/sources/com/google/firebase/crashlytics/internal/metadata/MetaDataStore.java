...
public class MetaDataStore {
    ...
    private static List<RolloutAssignment> jsonToRolloutsState(String str) throws JSONException {
        ...
        for (int i = 0; i < jSONArray.length(); i++) {
            ...
            try {
                ...
                Logger.getLogger().m16730w("Failed de-serializing rollouts state. " + string, e);
            }
        }
        return arrayList;
    }
    ...
    private static String rolloutsStateToJson(List<RolloutAssignment> list) {
        ...
        for (int i = 0; i < list.size(); i++) {
            try {
                ...
                Logger.getLogger().m16730w("Exception parsing rollout assignment!", e);
            }
        }
    ...
    private static void safeDeleteCorruptFile(File file) {
        if (file.exists() && file.delete()) {
            Logger.getLogger().m16725i("Deleted corrupt file: " + file.getAbsolutePath());
        }
    }
        ...
        try {
            try {
                ...
                try {
                    ...
                    e = e2;
                    Logger.getLogger().m16730w("Error deserializing user metadata.", e);
    ...
    public String readUserId(String str) throws Throwable {
                ...
                if (r3 != 0) {
                    try {
                        ...
                        try {
                            ...
                            Logger.getLogger().m16721d("Loaded userId " + strJsonToUserId + " for session " + str);
                            CommonUtils.closeOrLog(fileInputStream, "Failed to close user metadata file.");
        ...
        Logger.getLogger().m16721d("No userId set for session " + str);
        safeDeleteCorruptFile(userDataFileForSession);
        return null;
    }
        ...
        try {
            ...
            try {
                try {
                    ...
                    e = e2;
                    Logger.getLogger().m16730w("Error serializing key/value metadata.", e);
                    safeDeleteCorruptFile(internalKeysFileForSession);
                    CommonUtils.closeOrLog(bufferedWriter, "Failed to close key/value metadata file.");
                }
            } catch (Throwable th) {
        ...
        try {
            try {
                ...
                try {
                    ...
                    e = e2;
                    Logger.getLogger().m16730w("Error serializing rollouts state.", e);
                    safeDeleteCorruptFile(rolloutsStateForSession);
                    CommonUtils.closeOrLog(bufferedWriter, "Failed to close rollouts state file.");
                }
            } catch (Throwable th) {
        ...
        try {
            try {
                ...
                try {
                    ...
                    Logger.getLogger().m16730w("Error serializing user metadata.", e);
    ...
    private static void safeDeleteCorruptFile(File file, String str) {
        if (file.exists() && file.delete()) {
            Logger.getLogger().m16725i(String.format("Deleted corrupt file: %s\nReason: %s", file.getAbsolutePath(), str));
        }
    }
