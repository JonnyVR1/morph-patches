...
public class IdManager implements InstallIdProvider {
    ...
    public IdManager(Context context, String str, FirebaseInstallationsApi firebaseInstallationsApi, DataCollectionArbiter dataCollectionArbiter) {
        if (context == null) {
            ig3.m135964a("appContext must not be null");
            throw null;
        }
        if (str == null) {
            ig3.m135964a("appIdentifier must not be null");
            throw null;
        }
    ...
    private synchronized String createAndCacheCrashlyticsInstallId(String str, SharedPreferences sharedPreferences) {
        ...
        Logger.getLogger().m16727v("Created new Crashlytics installation ID: " + id + " for FID: " + str);
    ...
    public FirebaseInstallationId fetchTrueFid(boolean z) {
        ...
        if (z) {
            try {
                token = ((InstallationTokenResult) tfi0.m188730b(this.firebaseInstallations.getToken(false), 10000L, timeUnit)).getToken();
            } catch (Exception e) {
                Logger.getLogger().m16730w("Error getting Firebase authentication token.", e);
                token = null;
            }
        } else {
            token = null;
        ...
        try {
            str = (String) tfi0.m188730b(this.firebaseInstallations.getId(), 10000L, timeUnit);
        } catch (Exception e2) {
            Logger.getLogger().m16730w("Error getting Firebase installation id.", e2);
        }
    ...
    public synchronized InstallIdProvider.InstallIds getInstallIds() {
        ...
        Logger.getLogger().m16727v("Determining Crashlytics installation ID...");
        SharedPreferences sharedPrefs = CommonUtils.getSharedPrefs(this.appContext);
        String string = sharedPrefs.getString(PREFKEY_FIREBASE_IID, null);
        Logger.getLogger().m16727v("Cached Firebase Installation ID: " + string);
        if (this.dataCollectionArbiter.isAutomaticDataCollectionEnabled()) {
            FirebaseInstallationId firebaseInstallationIdFetchTrueFid = fetchTrueFid(false);
            Logger.getLogger().m16727v("Fetched Firebase Installation ID: " + firebaseInstallationIdFetchTrueFid.getFid());
        ...
        Logger.getLogger().m16727v("Install IDs: " + this.installIds);
        return this.installIds;
    }
