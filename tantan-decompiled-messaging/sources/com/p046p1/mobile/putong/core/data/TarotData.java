...
public class TarotData extends ValueObject implements Cloneable, Serializable {
    ...
    public static ProtobufAdapter<TarotData> PROTOBUF_ADAPTER = new MessageNanoAdapter<TarotData>() { // from class: com.p1.mobile.putong.core.data.TarotData.1
        ...
        public int computeAndCacheSize(TarotData tarotData) {
            int iM17226h = CodedOutputByteBufferNano.m17226h(1, tarotData.cardType);
            User user = tarotData.card;
            if (user != null) {
                iM17226h += CodedOutputByteBufferNano.m17230l(2, user, User.PROTOBUF_ADAPTER);
            }
            tarotData.cachedSize = iM17226h;
            return iM17226h;
        }

        @Override // com.tantanapp.common.data.ProtobufAdapter
        public TarotData parse(nb5 nb5Var) throws IOException {
            ...
            while (true) {
                ...
                if (iM158752u == 8) {
                    tarotData.cardType = nb5Var.m158741j();
                } else {
                    if (iM158752u != 18) {
                        if (tarotData.card != null) {
                            break;
                    ...
                    tarotData.card = (User) nb5Var.m158743l(User.PROTOBUF_ADAPTER);
                }
            }
            return tarotData;
        }
        ...
        public void serialize(TarotData tarotData, CodedOutputByteBufferNano codedOutputByteBufferNano) throws IOException {
            codedOutputByteBufferNano.m17250G(1, tarotData.cardType);
            User user = tarotData.card;
            if (user != null) {
    ...
    public TarotData mo223809clone() {
        TarotData tarotData = new TarotData();
        tarotData.cardType = this.cardType;
        User user = this.card;
        if (user != null) {
            tarotData.card = user.mo223809clone();
        }
        return tarotData;
    }
