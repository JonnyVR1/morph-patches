...
class MMFileUploader {
        ...
        public void handleMessage(Message message) {
            ...
            try {
                ...
                if (timeConfig != timeConfig2) {
                    Logger.m19637d("MMFile", "MMFileUploader: 被插队，放弃本次任务。old: %d, new: %d", Long.valueOf(timeConfig.getNextClockTime()), Long.valueOf(timeConfig2 == null ? -1L : timeConfig2.getNextClockTime()));
                } else {
                    Logger.m19637d("MMFile", "MMFileUploader: clock upload mmfile: %s", timeConfig2.toString());
    ...
    public void enqueueUploadConfig(TimeConfig timeConfig) {
        ...
        Logger.m19637d("MMFile", "MMFileUploader: enqueueUploadConfig add uploadConfig: %s", timeConfig.toString());
        launchNextTask();
    }
    ...
    private void launchNextTask() {
        if (this.isPause) {
            Logger.m19637d("MMFile", "mmfile upload task is paused", new Object[0]);
            return;
        }
        ...
        if (timeConfig != null) {
            ...
            Logger.m19637d("MMFile", "MMFileUploader: 插个队", new Object[0]);
            this.mHandler.removeMessages(1);
        }
        TimeConfig timeConfig2 = this.configs.get(0);
        this.nextConfig = timeConfig2;
        Logger.m19637d("MMFile", "MMFileUploader: next config: %s, time clock after: %d", timeConfig2.toString(), Long.valueOf(timeConfig2.getNextClockTime() - SystemClock.uptimeMillis()));
    ...
    public static void performUpload(TimeConfig timeConfig) {
        ...
        if (files == null || files.length == 0) {
            Logger.m19640i("MMFile", "no file need to upload", new Object[0]);
            return;
        }
        Logger.m19640i("MMFile", "prepare to upload file size: %d", Integer.valueOf(files.length));
        for (File file : files) {
    ...
    public static void uploadFile(FileUploadConfig fileUploadConfig, File file) {
        Logger.m19637d("MMFile", "MMFileUploader: uploadFile, %s", file.getAbsolutePath());
        if (file.exists()) {
    ...
    public void forceUploadMMFile() {
        Logger.m19637d("MMFile", "MMFileUploader: forceUploadMMFile", new Object[0]);
        try {
            this.lock.lock();
    ...
    public void setPause(boolean z) {
        ...
        this.isPause = z;
        if (z2) {
            Logger.m19637d("MMFile", "resume upload task", new Object[0]);
            launchNextTask();
        } else {
            Logger.m19637d("MMFile", "pause upload task", new Object[0]);
    ...
    public void setUploadClockTimeSeconds(FileUploadConfig fileUploadConfig, long j) {
        TimeConfig next;
        Logger.m19637d("MMFile", "MMFileUploader: setUploadClockTimeSeconds", new Object[0]);
        forceUploadMMFile();
        try {
            this.lock.lock();
