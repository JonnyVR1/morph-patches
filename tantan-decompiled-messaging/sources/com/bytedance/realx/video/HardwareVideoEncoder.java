...
class HardwareVideoEncoder extends MediaCodec.Callback implements VideoEncoder {
    ...
    public static /* synthetic */ void m5704a(HardwareVideoEncoder hardwareVideoEncoder, MediaCodecWrapper mediaCodecWrapper) {
        hardwareVideoEncoder.getClass();
        if (mediaCodecWrapper == null) {
            return;
        }
        RXLogging.m5688e(TAG, "mediacodec release start!");
        try {
            mediaCodecWrapper.stop();
        } catch (Exception e) {
            RXLogging.m5689e(TAG, "Media encoder stop failed", e);
        }
        try {
            mediaCodecWrapper.release();
        } catch (Exception e2) {
            RXLogging.m5689e(TAG, "Media encoder release failed", e2);
            hardwareVideoEncoder.shutdownException = e2;
        }
        RXLogging.m5688e(TAG, "mediacodec release end!");
    }

    /* JADX INFO: renamed from: b */
    public static /* synthetic */ void m5705b(HardwareVideoEncoder hardwareVideoEncoder, HandlerThread handlerThread) {
    ...
    private void caculateDts(long j, EncodedImage.Builder builder) {
        if (isBFrameEnabled()) {
            ...
            if (lPollLast == null) {
                RXLogging.m5688e(TAG, "dtsQueue is empty.");
                return;
            }
            if (!this.encodedFirstFrame) {
                this.encodedFirstFrame = true;
                this.firstFrameDiffUs = j - lPollLast.longValue();
                RXLogging.m5692w(TAG, "firstFrameDiffUs: " + this.firstFrameDiffUs + MinimalPrettyPrinter.DEFAULT_ROOT_VALUE_SEPARATOR + j);
            }
            ...
            if (jLongValue >= 0 && j2 >= 0) {
                ...
                RXLogging.m5688e(TAG, "ctsUs < 0; " + j2 + MinimalPrettyPrinter.DEFAULT_ROOT_VALUE_SEPARATOR + lValueOf);
            }
        }
    }
    ...
    private void drainOutputData(int i, MediaCodec.BufferInfo bufferInfo, ByteBuffer byteBuffer) {
        ...
        if ((i2 & 2) != 0) {
            RXLogging.m5690i(TAG, "Config frame generated. Offset: " + bufferInfo.offset + ". Size: " + bufferInfo.size);
            ...
            boolean z = (i2 & 1) != 0;
            if (z) {
                RXLogging.m5690i(TAG, "Sync frame generated");
            }
            ...
            if (z && (this.codecDesc.getStandard() == RXVideoCodecStandard.H264 || this.codecDesc.getStandard() == RXVideoCodecStandard.ByteVC1)) {
                RXLogging.m5690i(TAG, "Prepending config frame of size " + this.configBuffer.capacity() + " to output buffer with offset " + bufferInfo.offset + ", size " + bufferInfo.size);
    ...
    private VideoCodecStatus encodeByteBuffer(RXVideoFrameInterface rXVideoFrameInterface) {
        ...
        if (this.useAsyncMode) {
            ...
            if (iDequeueInputBuffer == -1) {
                RXLogging.m5690i(TAG, "Dropped frame, no input buffers available");
                return VideoCodecStatus.NO_OUTPUT;
            }
            try {
                inputBuffer = this.codec.getInputBuffer(iDequeueInputBuffer);
            } catch (Exception e) {
                RXLogging.m5689e(TAG, "getInputBuffer failed", e);
                return VideoCodecStatus.FALLBACK_SOFTWARE;
            }
        } else {
            try {
                ...
                try {
                    ...
                    RXLogging.m5689e(TAG, "getInputBuffers failed", e2);
                    return VideoCodecStatus.FALLBACK_SOFTWARE;
                }
            } catch (Exception e3) {
    ...
    private VideoCodecStatus encodeTextureBuffer(RXVideoFrameInterface rXVideoFrameInterface) {
        ...
        try {
            ...
            if (iGlGetError != 0) {
                RXLogging.m5688e(TAG, "got egl err:" + iGlGetError);
            }
            ...
            RXLogging.m5689e(TAG, "encodeTexture failed", e);
            return VideoCodecStatus.FALLBACK_SOFTWARE;
        }
    }
    ...
    private VideoCodecStatus initEncodeInternal() {
        ...
        try {
            ...
            try {
                ...
                RXLogging.m5692w(TAG, "Format: " + mediaFormatCreateVideoFormat);
                if (this.useAsyncMode) {
                ...
                if (isSvcEnabled() && (outputFormat = this.codec.getOutputFormat()) != null) {
                    ...
                    RXLogging.m5692w(TAG, "" + this.svcRealLayerNum + "，output format after configure: " + outputFormat.toString());
                }
                ...
                RXLogging.m5689e(TAG, "initEncodeInternal failed", e);
                release();
                return VideoCodecStatus.FALLBACK_SOFTWARE;
            }
        } catch (Exception e2) {
            RXLogging.m5689e(TAG, "Cannot create media encoder " + this.codecName, e2);
            return VideoCodecStatus.FALLBACK_SOFTWARE;
        }
    }
    ...
    private void initFrameDrawer() {
        if (this.enableNativeRender) {
            ...
            RXLogging.m5690i(TAG, "created native frame drawer 0x" + Long.toHexString(this.videoFrameDrawerHandler));
            return;
        }
        this.textureDrawer = new GlRectDrawer();
        this.videoFrameDrawer = new VideoFrameDrawer();
        RXLogging.m5690i(TAG, "created java frame drawer " + this.videoFrameDrawer);
    }
    ...
    private void initMediaFormat(MediaFormat mediaFormat) {
        ...
        for (Map.Entry<String, String> entry : this.privateParams.entrySet()) {
            RXLogging.m5692w(TAG, "set android hardware encoder private param with Key:" + entry.getKey() + " Value:" + entry.getValue());
    ...
    private int parseRealLayerNum(MediaFormat mediaFormat) {
        ...
        if (TextUtils.isEmpty(string) || string.length() < 17) {
            RXLogging.m5692w(TAG, "output temporal str is: " + string);
            return 1;
        }
        ...
        if (isNumeric(strSubstring)) {
            try {
                ...
                RXLogging.m5688e(TAG, e.toString());
            }
        }
        RXLogging.m5688e(TAG, "temporal layer invalid: ".concat(string));
        return 1;
    }
    ...
    public void releaseCodecOnOutputThread() {
        try {
            ...
            RXLogging.m5688e(TAG, "check thread error.");
        }
        ...
        if (this.releaseAsync) {
            new Thread(new Runnable() { // from class: com.bytedance.realx.video.d
                ...
                public final void run() {
                    HardwareVideoEncoder.m5704a(this.f4744a, mediaCodecWrapper);
                }
            }).start();
    ...
    private void releaseFrameDrawer() {
        ...
        if (j != 0) {
            ...
            RXLogging.m5690i(TAG, "released native frame drawer 0x" + Long.toHexString(this.videoFrameDrawerHandler));
        ...
        if (videoFrameDrawer != null) {
            ...
            RXLogging.m5690i(TAG, "released java frame drawer " + this.videoFrameDrawer);
    ...
    private void requestKeyFrame(long j) {
        ...
        try {
            ...
            RXLogging.m5689e(TAG, "requestKeyFrame failed", e);
        }
    }

    private VideoCodecStatus resetCodec(int i, int i2, boolean z) {
        this.encodeThreadChecker.checkIsOnValidThread();
        RXLogging.m5692w(TAG, "resetCodec: " + i + MinimalPrettyPrinter.DEFAULT_ROOT_VALUE_SEPARATOR + i2 + MinimalPrettyPrinter.DEFAULT_ROOT_VALUE_SEPARATOR + z);
    ...
    private void setBFrameNumber(MediaFormat mediaFormat) {
        ...
        if (this.codecDesc.getStandard() == RXVideoCodecStandard.H264 && (profile = this.codecDesc.getProfile()) != RXVideoCodecProfile.ProfileHigh && profile != RXVideoCodecProfile.ProfileConstrainedHigh) {
            RXLogging.m5692w(TAG, "enabel b frame, set high profile explicitly");
    ...
    private void setExtCodecForHisi(int i, int i2) {
        ...
        try {
            ...
            RXLogging.m5689e(TAG, "getOutputFormat failed", e);
            outputFormat = null;
        }
        if (outputFormat == null) {
            return;
        }
        RXLogging.m5690i(TAG, "output format before start: " + outputFormat.toString());
        boolean z = false;
        ...
        if (z && isSvcEnabled() && this.svcRealLayerNum <= 1) {
            ...
            RXLogging.m5692w(TAG, "support Non Ref PFrame.");
        }
        if (z2) {
        ...
        if (z || z2) {
            try {
                ...
                RXLogging.m5689e(TAG, "setExtCodecForHisi failed", e2);
            }
        }
    }
    ...
    private void setIntegerParam(MediaFormat mediaFormat, String str, Integer num) {
        for (Map.Entry<String, String> entry : this.privateParams.entrySet()) {
            if (entry.getKey().equals(str)) {
                if (isNumeric(entry.getValue())) {
                    mediaFormat.setInteger(entry.getKey(), Integer.parseInt(entry.getValue()));
                    RXLogging.m5692w(TAG, "the encoder params " + entry.getKey() + " is coverd by vpaas with value:" + entry.getValue());
                    this.privateParams.remove(str);
                    return;
                }
                this.privateParams.remove(str);
                if (entry.getValue().length() <= 0) {
                    RXLogging.m5692w(TAG, "the encoder params " + entry.getKey() + " is disabled by vpaas,skip it");
                    return;
                }
                RXLogging.m5692w(TAG, "the encoder params " + entry.getKey() + " set by vpaas is invalid with value:" + entry.getValue() + ",skip it");
            }
        }
        RXLogging.m5692w(TAG, "encoder int params has been set with Key:" + str + " Value:" + num);
    ...
    private void setProfileForLive(MediaFormat mediaFormat) {
        ...
        if (codecProfileLevel == null) {
            RXLogging.m5692w(TAG, "not set profile");
        } else {
    ...
    private void setProfileForRTC(MediaFormat mediaFormat) {
        RXLogging.m5692w(TAG, "close setting profile:" + this.settings.closeSetProfile);
        RXVideoCodecStandard standard = this.codecDesc.getStandard();
        ...
        if (standard == RXVideoCodecStandard.H264) {
            RXVideoCodecProfile profile = this.codecDesc.getProfile();
            int i = C11372.$SwitchMap$com$bytedance$realx$video$RXVideoCodecProfile[profile.ordinal()];
            if (i == 1 || i == 2) {
                RXLogging.m5692w(TAG, "setting high profile ");
                setIntegerParam(mediaFormat, "profile", 8);
                setIntegerParam(mediaFormat, FirebaseAnalytics.Param.LEVEL, 256);
                return;
            } else if (i == 3 || i == 4 || i == 5) {
                RXLogging.m5692w(TAG, "setting baseline profile ");
                setIntegerParam(mediaFormat, "profile", 1);
                setIntegerParam(mediaFormat, FirebaseAnalytics.Param.LEVEL, 256);
                return;
            } else {
                RXLogging.m5692w(TAG, "Unknown profile level id: " + profile + "default to baseline profile");
                setIntegerParam(mediaFormat, "profile", 1);
                setIntegerParam(mediaFormat, FirebaseAnalytics.Param.LEVEL, 256);
                return;
            }
        ...
        int i2 = C11372.$SwitchMap$com$bytedance$realx$video$RXVideoCodecProfile[profile2.ordinal()];
        if (i2 == 6) {
            setIntegerParam(mediaFormat, "profile", 1);
        ...
        if (i2 == 7) {
            ...
            RXLogging.m5692w(TAG, "Unknown profile level id: " + profile2 + "default to Main profile");
            setIntegerParam(mediaFormat, "profile", 1);
            setIntegerParam(mediaFormat, FirebaseAnalytics.Param.LEVEL, 8);
        }
    }
    ...
    private void setStringParam(MediaFormat mediaFormat, String str, String str2) {
        ...
        RXLogging.m5692w(TAG, "encoder string params has been set with Key:" + str + " Value:" + str2);
    ...
    private VideoCodecStatus updateBitrate(int i) {
        ...
        try {
            ...
            RXLogging.m5689e(TAG, "updateBitrate failed", e);
    ...
    public void deliverEncodedImage() {
        try {
            ...
            if (iDequeueOutputBuffer == -2) {
                RXLogging.m5690i(TAG, "output format changed: " + this.codec.getOutputFormat().toString());
                return;
            }
            ...
            if (iDequeueOutputBuffer != -3 && iDequeueOutputBuffer < 0) {
                RXLogging.m5688e(TAG, "should not be here: index:" + iDequeueOutputBuffer);
                return;
            }
            drainOutputData(iDequeueOutputBuffer, bufferInfo, this.codec.getOutputBuffers()[iDequeueOutputBuffer]);
        } catch (Exception e) {
            RXLogging.m5689e(TAG, "deliverOutput failed", e);
        }
    }
        ...
        synchronized (this.callbackLock) {
            try {
                ...
                if (bool == null || bool.booleanValue() != z2) {
                    ...
                    RXLogging.m5688e(TAG, "isTextureBuffer: " + this.isTextureBuffer);
                }
                ...
                if (z4 && this.useSurfaceMode && !RXVideoSurfaceController.getInstance().usePassSurfaceMode()) {
                    RXLogging.m5692w(TAG, "egl env changed, create new shared context from the new videoframe egl context");
                    releaseFrameDrawer();
                    ...
                    try {
                        ...
                        RXLogging.m5689e(TAG, "egl failed, fallback software:", e);
                        return VideoCodecStatus.FALLBACK_SOFTWARE;
                    }
                }
                resetCodecWhenBitrateChange();
                if (z) {
                    RXLogging.m5690i(TAG, "Request key frame");
                }
    ...
    public VideoCodecStatus initEncode(VideoEncoder.Settings settings, VideoEncoder.Callback callback) {
        ...
        if (RXVideoSurfaceController.getInstance().usePassSurfaceMode()) {
            RXLogging.m5688e(TAG, "using pass surface mode");
    ...
    public void onError(MediaCodec mediaCodec, MediaCodec.CodecException codecException) {
        MediaCodecWrapper mediaCodecWrapper;
        RXLogging.m5692w(TAG, "mediacodec error: " + codecException.getDiagnosticInfo() + MinimalPrettyPrinter.DEFAULT_ROOT_VALUE_SEPARATOR + codecException);
        if (this.running && (mediaCodecWrapper = this.codec) != null && mediaCodecWrapper.getRealCodec() == mediaCodec) {
            synchronized (this.callbackLock) {
                this.isError = true;
            }
        } else {
            RXLogging.m5688e(TAG, "onError not same codec " + codecException);
        }
    }
    ...
    public void onInputBufferAvailable(MediaCodec mediaCodec, int i) {
        MediaCodecWrapper mediaCodecWrapper;
        if (!this.running || (mediaCodecWrapper = this.codec) == null || mediaCodecWrapper.getRealCodec() != mediaCodec) {
            RXLogging.m5688e(TAG, "onInputBufferAvailable not same codec");
        } else {
            synchronized (this.callbackLock) {
    ...
    public void onOutputBufferAvailable(MediaCodec mediaCodec, int i, MediaCodec.BufferInfo bufferInfo) {
        MediaCodecWrapper mediaCodecWrapper;
        if (!this.running || (mediaCodecWrapper = this.codec) == null || mediaCodecWrapper.getRealCodec() != mediaCodec) {
            RXLogging.m5688e(TAG, "onOutputBufferAvailable not same codec");
            return;
        }
        try {
            drainOutputData(i, bufferInfo, mediaCodec.getOutputBuffer(i));
        } catch (Exception e) {
            RXLogging.m5689e(TAG, "onOutputBufferAvailable failed", e);
        }
    }
    ...
    public void onOutputFormatChanged(MediaCodec mediaCodec, MediaFormat mediaFormat) {
        MediaCodecWrapper mediaCodecWrapper;
        if (!this.running || (mediaCodecWrapper = this.codec) == null || mediaCodecWrapper.getRealCodec() != mediaCodec) {
            RXLogging.m5688e(TAG, "onOutputFormatChanged not same codec");
            return;
        }
        RXLogging.m5690i(TAG, "output format changed: " + mediaFormat.toString());
    }

    @Override // com.bytedance.realx.video.VideoEncoder
    public VideoCodecStatus release() {
        this.encodeThreadChecker.checkIsOnValidThread();
        RXLogging.m5692w(TAG, "hardware video encoder release start.");
        ...
        if (this.useAsyncMode) {
            ...
            if (handlerThread != null) {
                this.asyncHandler.post(new Runnable() { // from class: com.bytedance.realx.video.c
                    @Override // java.lang.Runnable
                    public final void run() {
                        HardwareVideoEncoder.m5705b(this.f4742a, handlerThread);
                    }
                });
                if (!ThreadUtils.joinUninterruptibly(this.asyncOutputThread, Constants.PN_IMAGE_DOWNLOAD_TIMEOUT_IN_MILLIS)) {
                    RXLogging.m5688e(TAG, "Media encoder release timeout");
            ...
            if (!ThreadUtils.joinUninterruptibly(this.outputThread, Constants.PN_IMAGE_DOWNLOAD_TIMEOUT_IN_MILLIS)) {
                ...
                RXLogging.m5689e(TAG, "Media encoder release exception", this.shutdownException);
                videoCodecStatus = VideoCodecStatus.ERROR;
            }
        }
        RXLogging.m5692w(TAG, "Media encoder release ok.");
        releaseFrameDrawer();
        ...
        RXLogging.m5692w(TAG, "textureEglBase release ok.");
        Surface surface = this.textureInputSurface;
        if (surface != null) {
            surface.release();
            this.textureInputSurface = null;
        }
        RXLogging.m5692w(TAG, "textureInputSurface release ok.");
        this.codec = null;
        this.outputThread = null;
        this.encodeThreadChecker.detachThread();
        RXLogging.m5692w(TAG, "hardware video encoder release end." + videoCodecStatus);
    ...
    public void setColorSpace(MediaFormat mediaFormat) {
        if (!this.useSurfaceMode && this.codecName.contains("OMX.hisi.") && this.settings.rangeId == 0 && (this.yuvColorFormat.intValue() == 19 || this.yuvColorFormat.intValue() == 21)) {
            RXLogging.m5692w(TAG, "kirin chips skip setting MediaFormat COLOR_RANGE");
        } else if (this.settings.rangeId == 1) {
            RXLogging.m5692w(TAG, "setting MediaFormat COLOR_RANGE_FULL");
            setIntegerParam(mediaFormat, "color-range", 1);
        } else {
            RXLogging.m5692w(TAG, "setting MediaFormat COLOR_RANGE_LIMITED");
    ...
    public VideoCodecStatus setPrivateParam(String str, String str2) {
        RXLogging.m5692w(TAG, "got android hardware encoder setPrivateParam with key:" + str + " value:" + str2);
        str.getClass();
        switch (str) {
