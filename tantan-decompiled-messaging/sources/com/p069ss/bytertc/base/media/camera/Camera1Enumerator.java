...
public class Camera1Enumerator implements CameraEnumerator {
    ...
    private static List<CameraEnumerationAndroid.CaptureFormat> enumerateFormats(int i) {
        int i2;
        RXLogging.m5692w(TAG, "Get supported formats for camera index " + i + ".");
        long jElapsedRealtime = SystemClock.elapsedRealtime();
        Camera cameraOpen = null;
        try {
            try {
                RXLogging.m5692w(TAG, "Opening camera with index " + i);
                ...
                if (parameters != null) {
                    try {
                        ...
                        RXLogging.m5689e(TAG, "getSupportedFormats() failed on camera index " + i, e);
                    }
                }
                RXLogging.m5692w(TAG, "Get supported formats for camera index " + i + " done. Time spent: " + (SystemClock.elapsedRealtime() - jElapsedRealtime) + " ms.");
                return arrayList;
            } catch (Throwable th) {
                if (cameraOpen != null) {
                    cameraOpen.release();
            ...
            RXLogging.m5689e(TAG, "Open camera failed on camera index " + i, e2);
    ...
    public static int getCameraIndex(String str) {
        RXLogging.m5692w(TAG, "getCameraIndex: " + str);
    ...
    private static Camera.CameraInfo getCameraInfo(int i) {
        ...
        try {
            ...
            RXLogging.m5689e(TAG, "getCameraInfo failed on index " + i, e);
            return null;
        }
    }
    ...
    @Override // com.p069ss.bytertc.base.media.camera.CameraEnumerator
    ...
    public String[] getDeviceNames() {
        ...
        for (int i = 0; i < Camera.getNumberOfCameras(); i++) {
            ...
            if (deviceName != null) {
                ...
                RXLogging.m5692w(TAG, "Index: " + i + ". " + deviceName);
            } else {
                RXLogging.m5688e(TAG, "Index: " + i + ". Failed to query camera name.");
            }
        }
