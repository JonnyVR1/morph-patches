...
class Camera1Session implements CameraSession, Camera.FaceDetectionListener {
    ...
    public class C133613 implements Camera.PreviewCallback {
        public C133613() {
        }

        /* JADX INFO: renamed from: a */
        public static /* synthetic */ void m80857a(C133613 c133613, byte[] bArr) {
        ...
        public void onPreviewFrame(final byte[] bArr, Camera camera) {
            ...
            if (camera != Camera1Session.this.camera) {
                RXLogging.m5688e(Camera1Session.TAG, "Callback from a different camera. This should never happen.");
                return;
            }
            if (Camera1Session.this.state != SessionState.RUNNING) {
                RXLogging.m5692w(Camera1Session.TAG, "Bytebuffer frame captured but camera is no longer running.");
                return;
            }
    ...
    private Camera1Session(CameraSession.Events events, boolean z, boolean z2, Context context, SurfaceTextureHelper surfaceTextureHelper, int i, Camera camera, Camera.CameraInfo cameraInfo, CameraEnumerationAndroid.CaptureFormat captureFormat, long j, int i2, int i3, boolean z3, boolean z4) {
        ...
        RXLogging.m5692w(TAG, "Create new camera1 session on camera " + i);
    ...
    /* JADX INFO: renamed from: a */
    public static /* synthetic */ void m80856a(Camera1Session camera1Session, VideoFrame videoFrame) {
        ...
        if (camera1Session.state != SessionState.RUNNING) {
            RXLogging.m5692w(TAG, "Texture frame captured but camera is no longer running.");
            return;
        }
        ...
        if (i3 == 0 || (i = camera1Session.scaleHeight) == 0 || ((i2 = (captureFormat = camera1Session.captureFormat).width) == i3 && captureFormat.height == i)) {
            if (camera1Session.scaleHeight == 0) {
                RXLogging.m5688e(TAG, "scaleHeight is 0");
                return;
            }
        ...
        if (bufferCropAndScale == null) {
            RXLogging.m5688e(TAG, "cropandscale return null buffer");
            return;
        }
    ...
    public static void create(CameraSession.CreateSessionCallback createSessionCallback, CameraSession.Events events, boolean z, boolean z2, Context context, SurfaceTextureHelper surfaceTextureHelper, int i, int i2, int i3, int i4, boolean z3, boolean z4) {
        ...
        RXLogging.m5692w(TAG, "Open camera " + i + " brand:" + Build.BRAND + ", modle:" + Build.MODEL + ", width:" + i2 + ", height:" + i3 + ", framerate:" + i4 + ", faceae:" + z3 + ", followGravity:" + z4 + ", captureToTexture:" + z);
    ...
    private static CameraEnumerationAndroid.CaptureFormat findClosestCaptureFormat(Camera.Parameters parameters, int i, int i2, int i3) {
        ...
        if (parameters.getSupportedPreviewFpsRange() == null) {
            RXLogging.m5688e(TAG, "camera parameters is null!");
            return null;
        }
        ...
        RXLogging.m5692w(TAG, "Available fps ranges: " + listConvertFramerates);
        if (Build.MODEL.equalsIgnoreCase("PEGM00") && i3 == 15) {
            ...
            RXLogging.m5692w(TAG, "unsupported framerate for special device found, hardcode framerange to:" + closestSupportedFramerateRange.toString());
        } else {
            closestSupportedFramerateRange = CameraEnumerationAndroid.getClosestSupportedFramerateRange(listConvertFramerates, i3);
        }
        RXLogging.m5692w(TAG, "selected fps range :" + closestSupportedFramerateRange.toString());
        List<Size> listConvertSizes = Camera1Enumerator.convertSizes(parameters.getSupportedPreviewSizes());
        RXLogging.m5692w(TAG, "Available supportedPreviewSizes : " + listConvertSizes.toString());
        Size upClosestSupportedSize = CameraEnumerationAndroid.getUpClosestSupportedSize(listConvertSizes, i, i2);
        RXLogging.m5692w(TAG, "selected preview size" + upClosestSupportedSize.toString());
    ...
    private void listenForBytebufferFrames() {
        this.camera.setPreviewCallbackWithBuffer(new C133613());
    }
    ...
    private void listenForTextureFrames() {
        this.surfaceTextureHelper.startListening(new VideoSink() { // from class: com.ss.bytertc.base.media.camera.a
            ...
            public final void onFrame(VideoFrame videoFrame) {
                Camera1Session.m80856a(this.f55583a, videoFrame);
            }
        });
    }

    private void startCapturing() {
        RXLogging.m5692w(TAG, "Start capturing");
        ...
        this.camera.setErrorCallback(new Camera.ErrorCallback() { // from class: com.ss.bytertc.base.media.camera.Camera1Session.2
            ...
            public void onError(int i, Camera camera) {
                ...
                RXLogging.m5688e(Camera1Session.TAG, str);
        ...
        try {
            ...
            RXLogging.m5690i(TAG, "supportedFaces is " + maxNumDetectedFaces);
            ...
            if (this.mEnableFaceAE && !z) {
                RXLogging.m5692w(TAG, "mEnableFaceAE is ture but camera not support faceae");
            }
    ...
    public void stopInternal() {
        RXLogging.m5692w(TAG, "Stop internal");
        checkIsOnCameraThread();
        SessionState sessionState = this.state;
        SessionState sessionState2 = SessionState.STOPPED;
        if (sessionState == sessionState2) {
            RXLogging.m5692w(TAG, "Camera is already stopped");
            return;
        }
        ...
        RXLogging.m5692w(TAG, "Stop done");
    }
    ...
    private static void updateCameraParameters(Camera camera, Camera.Parameters parameters, CameraEnumerationAndroid.CaptureFormat captureFormat, Size size, boolean z) {
        ...
        RXLogging.m5692w(TAG, "support FOCUS_MODE list: ");
        for (int i = 0; i < supportedFocusModes.size(); i++) {
            RXLogging.m5692w(TAG, i + " : " + supportedFocusModes.get(i));
        }
        RXLogging.m5692w(TAG, "start set FOCUS_MODE_CONTINUOUS_VIDEO");
        if (supportedFocusModes.contains("continuous-video")) {
            parameters.setFocusMode("continuous-video");
            RXLogging.m5692w(TAG, "FOCUS_MODE_CONTINUOUS_VIDEO support and set success");
        }
        camera.setParameters(parameters);
    }

    @Override // com.p069ss.bytertc.base.media.camera.CameraSession
    ...
    public float getCameraZoomMaxRatio() {
        RXLogging.m5692w(TAG, "turn on flash light camera1, cameraId:" + this.cameraId);
        checkIsOnCameraThread();
    ...
    @Override // com.p069ss.bytertc.base.media.camera.CameraSession
    public void stop() {
        RXLogging.m5692w(TAG, "Stop camera1 session on camera " + this.cameraId);
        checkIsOnCameraThread();
        if (this.state != SessionState.STOPPED) {
            System.nanoTime();
            stopInternal();
    ...
    public void turnOffFlashLight() {
        ...
        if (this.info.facing == 1) {
            RXLogging.m5692w(TAG, "front camera not support flash light");
