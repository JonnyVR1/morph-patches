...
public class VideoDeviceMonitorAndroid {
    ...
    private Boolean isUsbCamera(UsbDevice usbDevice) {
        RXLogging.m5692w(TAG, "isUsbCamera");
        ...
        RXLogging.m5692w(TAG, "DeviceClass: " + usbDevice.getDeviceClass() + " result: " + bool);
        return bool;
    }
    ...
    public void processorAttached(UsbDevice usbDevice) {
        if (isUsbCamera(usbDevice).booleanValue()) {
            ...
            RXLogging.m5692w(TAG, "processorAttached, is not Usb camera");
        }
    }
    ...
    public void processorDetached(UsbDevice usbDevice) {
        if (isUsbCamera(usbDevice).booleanValue()) {
            ...
            RXLogging.m5692w(TAG, "processorDetached, is not Usb camera");
        }
    }

    @CalledByNative
    public void start() {
        RXLogging.m5692w(TAG, "registerUsbAction");
        if (this.mUsbReceiver == null || this.mContext == null) {
            RXLogging.m5692w(TAG, "registerUsbAction fail");
            return;
        }
        IntentFilter intentFilter = new IntentFilter();
        intentFilter.addAction("android.hardware.usb.action.USB_DEVICE_ATTACHED");
        intentFilter.addAction("android.hardware.usb.action.USB_DEVICE_DETACHED");
        ii5.m136342l(this.mContext, this.mUsbReceiver, intentFilter);
        CameraManager cameraManager = (CameraManager) this.mContext.getSystemService("camera");
        try {
            if (this.mAvailabilityCallback != null) {
                RXLogging.m5692w(TAG, "registerAvailabilityCallback");
            ...
            RXLogging.m5692w(TAG, "registerAvailabilityCallback fail");
            e.printStackTrace();
        }
    }
    ...
    public void stop() {
        Context context;
        RXLogging.m5692w(TAG, "unregisterUsbAction");
        BroadcastReceiver broadcastReceiver = this.mUsbReceiver;
        if (broadcastReceiver == null || (context = this.mContext) == null) {
            RXLogging.m5692w(TAG, "unregisterUsbAction fail");
            return;
        }
        ...
        try {
            if (this.mAvailabilityCallback != null) {
                RXLogging.m5692w(TAG, "unregisterAvailabilityCallback");
            ...
            RXLogging.m5692w(TAG, "unregisterAvailabilityCallback fail");
            e.printStackTrace();
        }
    }
}
