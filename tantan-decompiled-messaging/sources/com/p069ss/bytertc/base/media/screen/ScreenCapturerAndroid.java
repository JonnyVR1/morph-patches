...
public class ScreenCapturerAndroid implements VideoCapturer, VideoSink {
    ...
    public synchronized void startCapture(int i, int i2, int i3, int i4) {
        if (!this.needStartService || Build.VERSION.SDK_INT <= 28) {
            ...
            while (!RXScreenCaptureService.serviceStarted.get()) {
                try {
                    ...
                    try {
                        ...
                        RXLogging.m5692w(TAG, "[ScreenCapture] sleep exception");
                        e.printStackTrace();
                    }
                    i5 = i6;
                } catch (ForegroundServiceStartNotAllowedException unused) {
            ...
            if (i5 > 0) {
                RXLogging.m5692w(TAG, String.format("[ScreenCapture]  call RXScreenCaptureService.COMMAND_START success, wait times:%d ms", Integer.valueOf((3 - i5) * 50)));
                Context context = this.mContext;
                context.startForegroundService(RXScreenCaptureService.getServiceIntent(context, 1, intent));
            } else {
                RXLogging.m5692w(TAG, String.format("[ScreenCapture] wait start service timeout", new Object[0]));
            }
        }
    }
