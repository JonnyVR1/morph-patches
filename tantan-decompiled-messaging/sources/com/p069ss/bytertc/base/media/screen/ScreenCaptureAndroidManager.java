...
import com.p069ss.bytertc.base.media.VideoSink;
import com.p069ss.bytertc.base.media.camera.CameraSession;
...
public class ScreenCaptureAndroidManager implements VideoCapturer, VideoSink {
    ...
    private void checkNotDisposed() {
        if (this.isDisposed) {
            upk0.m194883a("capturer is disposed.");
        }
    }
    ...
    public void createVirtualDisplay() {
        ...
        if (updateVirtualDisplayConfig()) {
            RXLogging.m5692w(TAG, String.format("[ScreenCapture] call updateVirtualDisplayConfig success, width=%d, height=%d", Integer.valueOf(this.width), Integer.valueOf(this.height)));
            return;
        }
        if (this.mediaProjection == null) {
            RXLogging.m5688e(TAG, "createVirtualDisplay failed, mediaProjection is null");
            return;
        }
        if (this.virtualDisplay != null) {
            RXLogging.m5688e(TAG, "[ScreenCapture] release current virtualdisplay");
            this.virtualDisplay.release();
            this.virtualDisplay = null;
        }
        this.surfaceTextureHelper.setTextureSize(this.width, this.height);
        try {
            RXLogging.m5692w(TAG, String.format("[ScreenCapture] call |createVirtualDisplay|,  width=%d, height=%d", Integer.valueOf(this.width), Integer.valueOf(this.height)));
            ...
            if (str != null) {
                ...
                this.virtualDisplay = mediaProjection.createVirtualDisplay(VIRTUAL_DISPLAY_NAME, this.width, this.height, 400, 11, new Surface(this.surfaceTextureHelper.getSurfaceTexture()), null, null);
            }
    ...
    private boolean maybeDoRotation() {
        ...
        RXLogging.m5692w(TAG, String.format("[ScreenCapture] call |maybeDoRotation| and orientation != mCurrentOrientation, rotation:%d, orientation:%s, orientation1:%s", Integer.valueOf(deviceRotation), displayOrientation.toString(), deviceOrientation.toString()));
    ...
    private void rotateCaptureOrientation(DeviceOrientation deviceOrientation) {
        if (this.mEnableDoOrientation) {
            ...
            RXLogging.m5692w(TAG, String.format("[ScreenCapture] call |rotateCaptureOrientation| before(orientation:%s, width:%d, height:%d)", deviceOrientation.toString(), Integer.valueOf(this.width), Integer.valueOf(this.height)));
            int i = this.width;
            int i2 = this.height;
            this.height = i;
            this.width = i + (i2 - i);
            RXLogging.m5692w(TAG, String.format("[ScreenCapture] call |rotateCaptureOrientation| after(orientation:%s, width:%d, height:%d)", deviceOrientation.toString(), Integer.valueOf(this.width), Integer.valueOf(this.height)));
        }
    }

    private boolean updateVirtualDisplayConfig() {
        RXLogging.m5692w(TAG, String.format("[ScreenCapture] updateVirtualDisplayConfig width=%d, height=%d", Integer.valueOf(this.width), Integer.valueOf(this.height)));
        if (this.mediaProjection == null) {
            RXLogging.m5688e(TAG, "updateVirtualDisplayConfig failed, mediaProjection is null");
            return false;
        }
        VirtualDisplay virtualDisplay = this.virtualDisplay;
        if (virtualDisplay == null) {
            RXLogging.m5688e(TAG, "updateVirtualDisplayConfig failed, virtualDisplay is null");
            return false;
        }
    ...
    public synchronized void changeCaptureFormat(int i, int i2, int i3) {
        ...
        if (i4 == this.width && i5 == this.height) {
            RXLogging.m5692w(TAG, "[ScreenCapture] call |changeCaptureFormat|, width & height are same as previous, ignore the request");
            return;
        }
        this.width = i4;
        this.height = i5;
    ...
    public synchronized void initialize(EglBase.Context context, Context context2, CapturerObserver capturerObserver) {
        ...
        this.mDisplay = ((WindowManager) context2.getSystemService(BLiveBottomPopupStyleTypeEnum.window)).getDefaultDisplay();
    }
    ...
    @Override // com.p069ss.bytertc.base.media.VideoSink
    ...
    public synchronized void startCapture(int i, int i2, int i3, int i4) {
        try {
            checkNotDisposed();
            RXLogging.m5692w(TAG, String.format("[ScreenCapture] call |startCapture|,  width=%d, height=%d", Integer.valueOf(i), Integer.valueOf(i2)));
            if (i <= 0 || i2 <= 0) {
                DisplayMetrics displayMetrics = new DisplayMetrics();
                this.mDisplay.getRealMetrics(displayMetrics);
                ...
                RXLogging.m5692w(TAG, String.format("[ScreenCapture] call |startCapture| , get screen real size, width=%d, height=%d", Integer.valueOf(i5), Integer.valueOf(displayMetrics.heightPixels)));
            } else {
                this.width = i;
                this.height = i2;
            }
    ...
    public synchronized void startCapture(int i, int i2, int i3, int i4, MediaProjection mediaProjection) {
        try {
            ...
            if (i > 0 && i2 > 0) {
                ...
                this.width = i5;
                this.height = displayMetrics.heightPixels;
                RXLogging.m5692w(TAG, String.format("[ScreenCapture] call |startCapture with MediaProjection| , get screen real size, width=%d, height=%d", Integer.valueOf(i5), Integer.valueOf(displayMetrics.heightPixels)));
            }
            this.width = ((this.width + 7) / 8) * 8;
            this.height = ((this.height + 7) / 8) * 8;
            maybeDoRotation();
