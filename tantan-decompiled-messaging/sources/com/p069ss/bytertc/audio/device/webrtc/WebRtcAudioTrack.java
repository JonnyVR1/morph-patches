...
public class WebRtcAudioTrack {
    ...
    public class AudioTrackThread extends Thread {
        ...
        public void run() {
            ...
            if (WebRtcAudioTrack.this.audioTrack == null) {
                RXLogging.m5688e(WebRtcAudioTrack.TAG, "null audio track instance");
                this.keepAlive = false;
            } else {
                RXLogging.m5690i(WebRtcAudioTrack.TAG, "AudioTrackThread" + WebRtcAudioUtils.getThreadInfo());
                RXLogging.m5690i(WebRtcAudioTrack.TAG, "AudioTrack state: " + WebRtcAudioTrack.this.audioTrack.getPlayState());
            }
            ...
            while (this.keepAlive) {
                ...
                try {
                    ...
                    this.keepAlive = false;
                    RXLogging.m5688e(WebRtcAudioTrack.TAG, "AudioTrack.write error, Exception: " + e.getMessage());
                ...
                if (iWriteBytes != iCapacity && iWriteBytes < 0) {
                    this.keepAlive = false;
                    RXLogging.m5688e(WebRtcAudioTrack.TAG, "AudioTrack.write played invalid number of bytes: " + iWriteBytes);
                    WebRtcAudioTrack.this.reportWebRtcAudioTrackError("AudioTrack.write failed: " + iWriteBytes);
                }
                WebRtcAudioTrack.this.byteBuffer.rewind();
            }
            if (WebRtcAudioTrack.this.audioTrack != null) {
                RXLogging.m5690i(WebRtcAudioTrack.TAG, "Calling AudioTrack.stop...");
                try {
                    WebRtcAudioTrack.this.audioTrack.stop();
                    RXLogging.m5690i(WebRtcAudioTrack.TAG, "AudioTrack.stop is done.");
                } catch (Exception e2) {
                    RXLogging.m5688e(WebRtcAudioTrack.TAG, "AudioTrack.stop failed: " + e2.getMessage());
                }
            }
        }
        ...
        public void stopThread() {
            synchronized (this) {
                RXLogging.m5690i(WebRtcAudioTrack.TAG, "stopThread");
                this.keepAlive = false;
            }
        }
    }
    ...
    private static AudioTrack createAudioTrackOnLollipopOrHigher(int i, int i2, int i3) {
        RXLogging.m5690i(TAG, "createAudioTrackOnLollipopOrHigher");
        int nativeOutputSampleRate = AudioTrack.getNativeOutputSampleRate(sForbidVoip ? 3 : 0);
        RXLogging.m5690i(TAG, "nativeOutputSampleRate: " + nativeOutputSampleRate);
        if (i != nativeOutputSampleRate) {
            RXLogging.m5692w(TAG, "Unable to use fast mode since requested sample rate is not native");
        }
        if (sForbidVoip) {
            usageAttribute = 1;
        } else {
        ...
        if (usageAttribute != DEFAULT_USAGE) {
            RXLogging.m5692w(TAG, "A non default usage attribute is used: " + usageAttribute);
        }
    ...
    private int getStreamMaxVolume() {
        this.threadChecker.checkIsOnValidThread();
        RXLogging.m5690i(TAG, "getStreamMaxVolume");
    ...
    private int getStreamVolume() {
        this.threadChecker.checkIsOnValidThread();
        RXLogging.m5690i(TAG, "getStreamVolume");
        AudioManager audioManager = this.audioManager;
        if (audioManager == null) {
            return -1;
        }
    ...
    private int initPlayout(int i, int i2, boolean z) {
        ...
        try {
            this.threadChecker.checkIsOnValidThread();
            RXLogging.m5690i(TAG, "initPlayout(sampleRate=" + i + ", channels=" + i2 + ")");
            ...
            RXLogging.m5690i(TAG, "AudioTrack.getMinBufferSize: " + minBufferSize);
            ...
            RXLogging.m5693w(TAG, "initPlayout exception", e2);
    ...
    private void logBufferCapacityInFrames() {
        RXLogging.m5690i(TAG, "AudioTrack: buffer capacity in frames: " + this.audioTrack.getBufferCapacityInFrames());
    }

    private void logBufferSizeInFrames() {
        RXLogging.m5690i(TAG, "AudioTrack: buffer size in frames: " + this.audioTrack.getBufferSizeInFrames());
    }

    private void logMainParameters() {
        RXLogging.m5690i(TAG, "AudioTrack: session ID: " + this.audioTrack.getAudioSessionId() + ", channels: " + this.audioTrack.getChannelCount() + ", sample rate: " + this.audioTrack.getSampleRate() + ", max gain: " + AudioTrack.getMaxVolume());
    }
    ...
    private void logUnderrunCount() {
        if (this.audioTrack != null) {
            RXLogging.m5690i(TAG, "underrun count: " + this.audioTrack.getUnderrunCount());
        }
    }
    ...
    private void releaseAudioResources() {
        RXLogging.m5690i(TAG, "releaseAudioResources");
        AudioTrack audioTrack = this.audioTrack;
        if (audioTrack != null) {
            audioTrack.release();
    ...
    public void reportWebRtcAudioTrackError(String str) {
        RXLogging.m5688e(TAG, "Run-time playback error: " + str);
    ...
    private void reportWebRtcAudioTrackInitError(String str) {
        RXLogging.m5688e(TAG, "Init playout error: " + str);
        WebRtcAudioUtils.logAudioState(TAG);
        WebRtcAudioTrackErrorCallback webRtcAudioTrackErrorCallback = errorCallbackOld;
        if (webRtcAudioTrackErrorCallback != null) {
    ...
    private void reportWebRtcAudioTrackStartError(AudioTrackStartErrorCode audioTrackStartErrorCode, String str) {
        RXLogging.m5688e(TAG, "Start playout error: " + audioTrackStartErrorCode + ". " + str);
        WebRtcAudioUtils.logAudioState(TAG);
        WebRtcAudioTrackErrorCallback webRtcAudioTrackErrorCallback = errorCallbackOld;
        if (webRtcAudioTrackErrorCallback != null) {
    ...
    public static synchronized void setAudioTrackUsageAttribute(int i) {
        RXLogging.m5692w(TAG, "Default usage attribute is changed from: " + DEFAULT_USAGE + " to " + i);
        usageAttribute = i;
    }

    @Deprecated
    public static void setErrorCallback(WebRtcAudioTrackErrorCallback webRtcAudioTrackErrorCallback) {
        RXLogging.m5690i(TAG, "Set error callback (deprecated");
        errorCallbackOld = webRtcAudioTrackErrorCallback;
    }

    public static void setSpeakerMute(boolean z) {
        RXLogging.m5692w(TAG, "setSpeakerMute(" + z + ")");
        speakerMute = z;
    }

    private boolean setStreamVolume(int i) {
        this.threadChecker.checkIsOnValidThread();
        RXLogging.m5690i(TAG, "setStreamVolume(" + i + ")");
        if (this.audioManager == null) {
            return false;
        }
        if (isVolumeFixed()) {
            RXLogging.m5688e(TAG, "The device implements a fixed volume policy.");
            return false;
        }
    ...
    private int startPlayout() {
        this.threadChecker.checkIsOnValidThread();
        RXLogging.m5690i(TAG, "startPlayout");
        AudioTrack audioTrack = this.audioTrack;
        if (audioTrack == null) {
            RXLogging.m5688e(TAG, "null audio track object");
            return -100;
        }
        if (this.audioThread != null) {
            RXLogging.m5688e(TAG, "the previous audio thread leak");
            return -101;
        }
    ...
    private boolean stopPlayout() {
        this.threadChecker.checkIsOnValidThread();
        RXLogging.m5690i(TAG, "stopPlayout");
        this.apiResult = "StopPlayout:";
        logUnderrunCount();
        if (this.audioThread != null) {
            RXLogging.m5690i(TAG, "Stopping the AudioTrackThread...");
            this.audioThread.stopThread();
            RXLogging.m5690i(TAG, "interrupt the AudioTrackThread...");
            this.audioThread.interrupt();
            if (!ThreadUtils.joinUninterruptibly(this.audioThread, 2000L)) {
                RXLogging.m5688e(TAG, "Join of AudioTrackThread timed out.");
                WebRtcAudioUtils.logAudioState(TAG);
                this.apiResult += "Join of AudioTrackThread timed out.";
            }
            RXLogging.m5690i(TAG, "AudioTrackThread has now been stopped.");
            this.audioThread = null;
        }
        releaseAudioResources();
        RXLogging.m5690i(TAG, "stopPlayout release done.");
        return true;
    }

    public static void setErrorCallback(ErrorCallback errorCallback2) {
        RXLogging.m5690i(TAG, "Set extended error callback");
