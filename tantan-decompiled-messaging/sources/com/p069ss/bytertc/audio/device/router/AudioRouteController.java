...
public class AudioRouteController implements IAudioRouteCallback {
    ...
    private int initAudioRouteDeviceManager() {
        ...
        if (iInit != 0) {
            release();
            RXLogging.m5688e(TAG, "init failed. code: " + iInit);
        }
        return iInit;
    }
    ...
    private int release() {
        int iRelease;
        this.wLock.lock();
        RXLogging.m5692w(TAG, "AudioRouteController: release begin");
        try {
            ...
            RXLogging.m5692w(TAG, "AudioRouteController: release end");
            this.wLock.unlock();
        }
    }
    ...
    private int setAudioRoute(@AudioRouteDeviceManager.RoutingDeviceType int i, boolean z) {
        ...
        try {
            ...
            RXLogging.m5692w(TAG, "AudioRouteController: setAudioRoute, end, routing: " + i + ", isMediaMode: " + z);
            this.rLock.unlock();
        }
    }
    ...
    @Override // com.p069ss.bytertc.audio.device.router.IAudioRouteCallback
    public void onDeviceEvent(int i, boolean z, String str) throws Throwable {
        AudioRouteController audioRouteController;
        Lock lock;
        this.rLock.lock();
        RXLogging.m5692w(TAG, "onDeviceEvent, begin, deviceType: " + i + ", connected: " + z + ", deviceInfo:" + str);
        try {
            if (this.nativeAudioRouteControllerPtr == 0) {
                RXLogging.m5692w(TAG, "AudioRouteController: onDeviceEvent, end");
                lock = this.rLock;
            } else {
                RXLogging.m5688e(TAG, "AudioRouteController: onDeviceEvent(Java) deviceType:" + i + ", connected:" + z + ", deviceInfo:" + str);
    ...
    public void onError(int i, String str) {
        ...
        try {
            if (this.nativeAudioRouteControllerPtr != 0) {
                RXLogging.m5688e(TAG, "AudioRouteController: ERROR-" + i + ":" + str);
