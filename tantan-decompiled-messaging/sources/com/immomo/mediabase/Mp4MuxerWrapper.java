...
public class Mp4MuxerWrapper extends MuxerBase {
    ...
    public int addMediaTrack(MediaFormat mediaFormat, int i) {
        ...
        if (mediaFormat == null || i < 1 || i > 2) {
            Log4Cam.m18379e("Mp4MuxerWrapper", "Add media track error ! Invalid parameter ! format=" + mediaFormat + " and track=" + i);
            return -1;
        }
        synchronized (this.mMuxerLock) {
            try {
                ...
                if (mediaMuxer != null) {
                    ...
                    Log4Cam.m18376d("Mp4MuxerWrapper", "Add track info " + getCurStringInfo());
                }
            } catch (Throwable th) {
                throw th;
            }
        ...
        synchronized (this.mMuxerLock) {
            try {
                if (!this.mIsRunning && this.mMediaTrackInfo == this.mAddedMediaTrack) {
                    ...
                    Log4Cam.m18379e("Mp4MuxerWrapper", "Start Media muxing !!" + getCurStringInfo());
                    return;
                }
                Log4Cam.m18386w("Mp4MuxerWrapper", "Meida info not enough , need waitting, already have " + getCurStringInfo());
            } catch (Throwable th) {
                throw th;
            }
        }
    ...
    public void stopMuxing() {
        ...
        Log4Cam.m18376d("Mp4MuxerWrapper", "Stop media muxing !" + this.mMediaTrackInfo);
    }
        ...
        synchronized (this.mMuxerLock) {
            try {
                if (byteBuffer == null || bufferInfo == null) {
                    Log4Cam.m18379e("Mp4MuxerWrapper", "[writeSampleData] Invalid Parameter !! ByteBuffer or BufferInfo is null");
                    return false;
                }
                if (this.mIsRunning) {
                    MediaMuxer mediaMuxer = this.mMediaMuxer;
                ...
                Log4Cam.m18386w("Mp4MuxerWrapper", "Media muxer not started !!, already have media type:" + getCurStringInfo());
                return false;
            } catch (Throwable th) {
                throw th;
            }
