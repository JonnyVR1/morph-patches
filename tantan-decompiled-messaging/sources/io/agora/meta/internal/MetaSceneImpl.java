...
public class MetaSceneImpl extends IMetaScene {
    ...
    public int addEventHandler(IMetaSceneEventHandler iMetaSceneEventHandler) {
        if (this.mIsDestroying.get()) {
            Logging.m87219e(TAG, "MetaScene is destroying(addEventHandler)");
            return -8;
        }
        synchronized (this) {
            try {
                ...
                if (j == 0) {
                    Logging.m87219e(TAG, "MetaScene does not initialize or it may be destroyed (addEventHandler)");
                    return -7;
                }
    ...
    public int addSceneView(TextureView textureView, SceneDisplayConfig sceneDisplayConfig) {
        if (this.mIsDestroying.get()) {
            Logging.m87219e(TAG, "MetaScene is destroying(addSceneView)");
            return -8;
        }
        synchronized (this) {
            try {
                long j = this.mNativeHandle;
                if (j == 0) {
                    Logging.m87219e(TAG, "MetaScene does not initialize or it may be destroyed (addSceneView)");
                    return -7;
                }
    ...
    public int enableFaceCapture(boolean z) {
        if (this.mIsDestroying.get()) {
            Logging.m87219e(TAG, "MetaScene is destroying(enableFaceCapture)");
            return -8;
        }
        synchronized (this) {
            try {
                long j = this.mNativeHandle;
                if (j == 0) {
                    Logging.m87219e(TAG, "MetaScene does not initialize or it may be destroyed (enableFaceCapture)");
                    return -7;
                }
    ...
    public int enableSceneVideoCapture(TextureView textureView, boolean z) {
        if (this.mIsDestroying.get()) {
            Logging.m87219e(TAG, "MetaScene is destroying(enableSceneVideoCapture)");
            return -8;
        }
        synchronized (this) {
            try {
                long j = this.mNativeHandle;
                if (j == 0) {
                    Logging.m87219e(TAG, "MetaScene does not initialize or it may be destroyed (enableSceneVideoCapture)");
                    return -7;
                }
    ...
    public int enableVideoDisplay(String str, boolean z) {
        if (this.mIsDestroying.get()) {
            Logging.m87219e(TAG, "MetaScene is destroying(enableVideoDisplay)");
            return -8;
        }
        synchronized (this) {
            try {
                long j = this.mNativeHandle;
                if (j == 0) {
                    Logging.m87219e(TAG, "MetaScene does not initialize or it may be destroyed (enableVideoDisplay)");
                    return -7;
                }
    ...
    public int enterScene(EnterSceneConfig enterSceneConfig) {
        if (this.mIsDestroying.get()) {
            Logging.m87219e(TAG, "MetaScene is destroying(enterScene)");
            return -8;
        }
        synchronized (this) {
            try {
                long j = this.mNativeHandle;
                if (j == 0) {
                    Logging.m87219e(TAG, "MetaScene does not initialize or it may be destroyed (enterScene)");
                    return -7;
                }
    ...
    public ILocalUserAvatar getLocalUserAvatar() {
        if (this.mIsDestroying.get()) {
            Logging.m87219e(TAG, "MetaScene is destroying(getLocalUserAvatar)");
            return null;
        }
        synchronized (this) {
            try {
                if (this.mNativeHandle == 0) {
                    Logging.m87219e(TAG, "MetaScene does not initialize or it may be destroyed (getLocalUserAvatar)");
                    return null;
                }
    ...
    public int leaveScene() {
        if (this.mIsDestroying.get()) {
            Logging.m87219e(TAG, "MetaScene is destroying(leaveScene)");
            return -8;
        }
        synchronized (this) {
            try {
                long j = this.mNativeHandle;
                if (j == 0) {
                    Logging.m87219e(TAG, "MetaScene does not initialize or it may be destroyed (leaveScene)");
                    return -7;
                }
    ...
    public int pushVideoFrameToDisplay(String str, AgoraVideoFrame agoraVideoFrame) throws Throwable {
        Throwable th;
        if (this.mIsDestroying.get()) {
            Logging.m87219e(TAG, "MetaScene is destroying(pushVideoFrameToDisplay(AgoraVideoFrame))");
            return -8;
        }
        synchronized (this) {
            try {
                try {
                    ...
                    try {
                        Logging.m87219e(TAG, "MetaScene does not initialize or it may be destroyed (pushVideoFrameToDisplay)");
                        return -7;
                    } catch (Throwable th2) {
                        th = th2;
                    }
    ...
    public int removeEventHandler(IMetaSceneEventHandler iMetaSceneEventHandler) {
        if (this.mIsDestroying.get()) {
            Logging.m87219e(TAG, "MetaScene is destroying(removeEventHandler)");
            return -8;
        }
        synchronized (this) {
            try {
                long j = this.mNativeHandle;
                if (j == 0) {
                    Logging.m87219e(TAG, "MetaScene does not initialize or it may be destroyed (removeEventHandler)");
                    return -7;
                }
    ...
    public int removeSceneView(TextureView textureView) {
        if (this.mIsDestroying.get()) {
            Logging.m87219e(TAG, "MetaScene is destroying(removeSceneView)");
            return -8;
        }
        synchronized (this) {
            try {
                long j = this.mNativeHandle;
                if (j == 0) {
                    Logging.m87219e(TAG, "MetaScene does not initialize or it may be destroyed (removeSceneView)");
                    return -7;
                }
    ...
    public int sendSceneMessage(byte[] bArr) {
        if (this.mIsDestroying.get()) {
            Logging.m87219e(TAG, "MetaScene is destroying(sendSceneMessage)");
            return -8;
        }
        synchronized (this) {
            try {
                long j = this.mNativeHandle;
                if (j == 0) {
                    Logging.m87219e(TAG, "MetaScene does not initialize or it may be destroyed (sendSceneMessage)");
                    return -7;
                }
    ...
    public int setSceneParameters(String str) {
        if (this.mIsDestroying.get()) {
            Logging.m87219e(TAG, "MetaScene is destroying(setSceneParameters)");
            return -8;
        }
        synchronized (this) {
            try {
                long j = this.mNativeHandle;
                if (j == 0) {
                    Logging.m87219e(TAG, "MetaScene does not initialize or it may be destroyed (setSceneParameters)");
                    return -7;
                }
    ...
    public int pushVideoFrameToDisplay(String str, VideoFrame videoFrame) {
        if (this.mIsDestroying.get()) {
            Logging.m87219e(TAG, "MetaScene is destroying(pushVideoFrameToDisplay(VideoFrame))");
            return -8;
        }
        synchronized (this) {
            try {
