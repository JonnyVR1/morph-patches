        ...
        if (!handler.post(new Runnable() { // from class: io.agora.base.internal.ThreadUtils.3
            ...
            public void run() {
                try {
                    ...
                    c1CaughtException.f63336e = e2;
                }
                countDownLatch.countDown();
            }
        })) {
            c1CaughtException.f63336e = new Exception("handler post fail at invokeAtFrontUninterruptibly !");
        } else if (j <= 0) {
            awaitUninterruptibly(countDownLatch);
        } else {
            awaitUninterruptibly(countDownLatch, j);
        }
        Exception exc = c1CaughtException.f63336e;
        if (exc == null) {
            return c1Result.value;
        }
        RuntimeException runtimeException = new RuntimeException(exc);
        runtimeException.setStackTrace(concatStackTraces(c1CaughtException.f63336e.getStackTrace(), runtimeException.getStackTrace()));
