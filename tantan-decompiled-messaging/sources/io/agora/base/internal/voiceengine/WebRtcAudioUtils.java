...
public final class WebRtcAudioUtils {
    ...
    public static String getThreadInfo() {
        return "@[name=" + Thread.currentThread().getName() + ", id=" + Thread.currentThread().getId() + Constants.AES_SUFFIX;
    }
    ...
    private static void logAudioDeviceInfo(String str, AudioManager audioManager) {
        if (runningOnMarshmallowOrHigher()) {
            ...
            Logging.m87208d(str, "Audio Devices: ");
    ...
    private static void logAudioStateBasic(String str, AudioManager audioManager) {
        Logging.m87208d(str, "Audio State: audio mode: " + modeToString(audioManager.getMode()) + ", has mic: " + hasMicrophone() + ", mic muted: " + audioManager.isMicrophoneMute() + ", music active: " + audioManager.isMusicActive() + ", speakerphone: " + audioManager.isSpeakerphoneOn() + ", BT SCO: " + audioManager.isBluetoothScoOn());
    }
    ...
    private static void logAudioStateVolume(String str, AudioManager audioManager) {
        ...
        Logging.m87208d(str, "Audio State: ");
        if (runningOnLollipopOrHigher()) {
            zIsVolumeFixed = audioManager.isVolumeFixed();
            Logging.m87208d(str, "  fixed volume=" + zIsVolumeFixed);
        } else {
    ...
    public static void logDeviceInfo(String str) {
        Logging.m87208d(str, "Android SDK: " + Build.VERSION.SDK_INT + ", Release: " + Build.VERSION.RELEASE + ", Brand: " + Build.BRAND + ", Device: " + Build.DEVICE + ", Id: " + Build.ID + ", Hardware: " + Build.HARDWARE + ", Manufacturer: " + Build.MANUFACTURER + ", Model: " + Build.MODEL + ", Product: " + Build.PRODUCT);
    }
    ...
    public static synchronized void setWebRtcBasedAutomaticGainControl(boolean z) {
        Logging.m87213w(TAG, "setWebRtcBasedAutomaticGainControl() is deprecated");
    }
    ...
    public static String sourceToString(int i) {
        ...
        if (i == 2) {
            return "SOURCE_VOICE_UPLINK";
        }
        if (i == 3) {
            return "SOURCE_VOICE_DOWNLINK";
        }
        if (i == 4) {
            return "SOURCE_VOICE_CALL";
        }
        if (i == 6) {
            return "SOURCE_VOICE_RECOGNITION";
        }
        if (i != 7) {
            return i != 9 ? "SOURCE_INVALID" : "SOURCE_VOICE_UNPROCESSED";
        }
        return "SOURCE_VOICE_COMMUNICATION";
    }
    ...
    public static synchronized boolean useWebRtcBasedAcousticEchoCanceler() {
        try {
            if (useWebRtcBasedAcousticEchoCanceler) {
                Logging.m87213w(TAG, "Overriding default behavior; now using WebRTC AEC!");
            }
        } catch (Throwable th) {
            throw th;
        }
    ...
    public static synchronized boolean useWebRtcBasedNoiseSuppressor() {
        try {
            if (useWebRtcBasedNoiseSuppressor) {
                Logging.m87213w(TAG, "Overriding default behavior; now using WebRTC NS!");
            }
        } catch (Throwable th) {
            throw th;
        }
