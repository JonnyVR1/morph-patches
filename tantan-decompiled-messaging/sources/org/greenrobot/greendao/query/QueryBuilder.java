...
public class QueryBuilder<T> {
    ...
    private void appendJoinsAndWheres(StringBuilder sb, String str) {
        ...
        boolean zIsEmpty = this.whereCollector.isEmpty();
        boolean z = !zIsEmpty;
        if (!zIsEmpty) {
            sb.append(" WHERE ");
    ...
    private int checkAddOffset(StringBuilder sb) {
        ...
        if (this.limit == null) {
            qkq0.m175383a("Offset cannot be set without limit");
            return 0;
        }
        sb.append(" OFFSET ?");
    ...
    private void checkLog(String str) {
        if (LOG_SQL) {
            DaoLog.m221099d("GreenDao Built SQL for query: " + str);
        }
        if (LOG_VALUES) {
            DaoLog.m221099d("GreenDao Values for query: " + this.values);
        }
    }
    ...
    private void checkOrderBuilder() {
        ...
        if (sb == null) {
            ...
            this.orderBuilder.append(Constants.SEPARATOR_COMMA);
        }
    }
    ...
    public WhereCondition m221119or(WhereCondition whereCondition, WhereCondition whereCondition2, WhereCondition... whereConditionArr) {
    ...
    public QueryBuilder<T> whereOr(WhereCondition whereCondition, WhereCondition whereCondition2, WhereCondition... whereConditionArr) {
        this.whereCollector.add(m221119or(whereCondition, whereCondition2, whereConditionArr), new WhereCondition[0]);
        return this;
    }
    ...
    public QueryBuilder(AbstractDao<T, ?> abstractDao) {
        this(abstractDao, j6f.GPS_DIRECTION_TRUE);
    }
}
