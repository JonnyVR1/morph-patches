...
public abstract class FiniteFields {
    ...
    public static PolynomialExtensionField getBinaryExtensionField(int[] iArr) {
        if (iArr[0] != 0) {
            ig3.m135964a("Irreducible polynomials in GF(2) must have constant term");
            return null;
        }
        for (int i = 1; i < iArr.length; i++) {
            if (iArr[i] <= iArr[i - 1]) {
                ig3.m135964a("Polynomial exponents must be montonically increasing");
                return null;
            }
        }
    ...
    public static FiniteField getPrimeField(BigInteger bigInteger) {
        ...
        if (bigInteger.signum() <= 0 || iBitLength < 2) {
            ig3.m135964a("'characteristic' must be >= 2");
            return null;
        }
        if (iBitLength < 3) {
