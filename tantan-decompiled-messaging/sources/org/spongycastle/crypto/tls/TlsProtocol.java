...
public abstract class TlsProtocol {
    ...
    private void processAlert() throws IOException {
        while (this.alertQueue.available() >= 2) {
            ...
            if (s == 2) {
                ...
                rhg0.m179353a(TLS_ERROR_MESSAGE);
                return;
            }
            if (s2 == 0) {
                handleClose(false);
    ...
    public int getAvailableInputBytes() {
        ...
        qkq0.m175383a("Cannot use getAvailableInputBytes() in blocking mode! Use getInputStream().available() instead.");
        return 0;
    }
    ...
    public int getAvailableOutputBytes() {
        ...
        qkq0.m175383a("Cannot use getAvailableOutputBytes() in blocking mode! Use getOutputStream() instead.");
        return 0;
    }
    ...
    public InputStream getInputStream() {
        ...
        qkq0.m175383a("Cannot use InputStream in non-blocking mode! Use offerInput() instead.");
        return null;
    }
    ...
    public OutputStream getOutputStream() {
        ...
        qkq0.m175383a("Cannot use OutputStream in non-blocking mode! Use offerOutput() instead.");
        return null;
    }
    ...
    public void offerInput(byte[] bArr) throws IOException {
        if (this.blocking) {
            qkq0.m175383a("Cannot use offerInput() in blocking mode! Use getInputStream() instead.");
            return;
        }
        if (this.closed) {
            rhg0.m179353a("Connection is closed, cannot accept any more input");
            return;
        }
    ...
    public void offerOutput(byte[] bArr, int i, int i2) throws IOException {
        if (this.blocking) {
            qkq0.m175383a("Cannot use offerOutput() in blocking mode! Use getOutputStream() instead.");
        } else if (this.appDataReady) {
            writeData(bArr, i, i2);
        } else {
            rhg0.m179353a("Application data cannot be sent until the handshake is complete!");
        }
    }
    ...
    public int readInput(byte[] bArr, int i, int i2) {
        if (this.blocking) {
            qkq0.m175383a("Cannot use readInput() in blocking mode! Use getInputStream() instead.");
            return 0;
        }
        try {
    ...
    public int readOutput(byte[] bArr, int i, int i2) {
        if (this.blocking) {
            qkq0.m175383a("Cannot use readOutput() in blocking mode! Use getOutputStream() instead.");
            return 0;
        }
    ...
    public void writeData(byte[] bArr, int i, int i2) throws IOException {
        if (this.closed) {
            if (this.failedWithError) {
                ...
                rhg0.m179353a("Sorry, connection has been closed, you cannot write more data");
                return;
            }
        }
        while (i2 > 0) {
