...
public class IESEngine {
    ...
    private byte[] decryptBlock(byte[] bArr, int i, int i2) throws InvalidCipherTextException {
        ...
        int iDoFinal;
        byte[] bArr4;
        if (i2 < this.f206556V.length + this.mac.getMacSize()) {
            dql.m113082a("Length of input must be greater than the MAC and V combined");
            return null;
        }
        if (this.cipher == null) {
            iDoFinal = (i2 - this.f206556V.length) - this.mac.getMacSize();
            ...
            for (int i4 = 0; i4 != iDoFinal; i4++) {
                bArr4[i4] = (byte) (bArr[(this.f206556V.length + i) + i4] ^ bArr5[i4]);
            }
            bArr3 = bArr;
        } else {
            ...
            byte[] bArr9 = this.f206555IV;
            BufferedBlockCipher bufferedBlockCipher = this.cipher;
            if (bArr9 != null) {
                bufferedBlockCipher.init(false, new ParametersWithIV(new KeyParameter(bArr7), this.f206555IV));
            } else {
                bufferedBlockCipher.init(false, new KeyParameter(bArr7));
            }
            byte[] bArr10 = new byte[this.cipher.getOutputSize((i2 - this.f206556V.length) - this.mac.getMacSize())];
            BufferedBlockCipher bufferedBlockCipher2 = this.cipher;
            byte[] bArr11 = this.f206556V;
            bArr3 = bArr;
        ...
        byte[] lengthTag = this.f206556V.length != 0 ? getLengthTag(encodingV) : null;
        int i6 = i + i2;
    ...
    private byte[] encryptBlock(byte[] bArr, int i, int i2) throws InvalidCipherTextException {
        ...
        if (this.cipher == null) {
            ...
            BufferedBlockCipher bufferedBlockCipher = this.cipher;
            if (bArr9 != null) {
                bufferedBlockCipher.init(true, new ParametersWithIV(new KeyParameter(bArr6), this.f206555IV));
            } else {
        ...
        byte[] bArr12 = this.f206556V;
        byte[] bArr13 = new byte[bArr12.length + i2 + macSize];
        System.arraycopy(bArr12, 0, bArr13, 0, bArr12.length);
        System.arraycopy(bArr3, 0, bArr13, this.f206556V.length, i2);
        System.arraycopy(bArr11, 0, bArr13, this.f206556V.length + i2, macSize);
        return bArr13;
    }
    ...
    private void extractParams(CipherParameters cipherParameters) {
        if (!(cipherParameters instanceof ParametersWithIV)) {
            ...
            this.f206555IV = parametersWithIV.getIV();
    ...
    public byte[] processBlock(byte[] bArr, int i, int i2) throws InvalidCipherTextException {
        if (this.forEncryption) {
            ...
            if (ephemeralKeyPairGenerator != null) {
                ...
                this.f206556V = ephemeralKeyPairGenerate.getEncodedPublicKey();
            }
        } else if (this.keyParser != null) {
            ByteArrayInputStream byteArrayInputStream = new ByteArrayInputStream(bArr, i, i2);
            try {
                this.pubParam = this.keyParser.readKey(byteArrayInputStream);
                this.f206556V = Arrays.copyOfRange(bArr, i, (i2 - byteArrayInputStream.available()) + i);
            } catch (IOException e) {
