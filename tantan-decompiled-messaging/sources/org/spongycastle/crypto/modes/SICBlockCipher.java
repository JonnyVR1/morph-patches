...
public class SICBlockCipher extends StreamBlockCipher implements SkippingStreamCipher {
    ...
    public SICBlockCipher(BlockCipher blockCipher) {
        ...
        this.f206672IV = new byte[blockSize];
    ...
    private void checkCounter() {
        if (this.f206672IV.length >= this.blockSize) {
            return;
        }
        int i = 0;
        while (true) {
            byte[] bArr = this.f206672IV;
            if (i == bArr.length) {
                return;
            }
            if (this.counter[i] != bArr[i]) {
                qkq0.m175383a("Counter in CTR/SIC mode out of range.");
                return;
            }
            i++;
        }
    ...
    public void init(boolean z, CipherParameters cipherParameters) throws IllegalArgumentException {
        if (!(cipherParameters instanceof ParametersWithIV)) {
            ig3.m135964a("CTR/SIC mode requires ParametersWithIV");
            return;
        }
        ParametersWithIV parametersWithIV = (ParametersWithIV) cipherParameters;
        byte[] bArrClone = Arrays.clone(parametersWithIV.getIV());
        this.f206672IV = bArrClone;
        int i = this.blockSize;
        if (i < bArrClone.length) {
            pac0.m168085a("CTR/SIC mode requires IV no greater than: ", this.blockSize, " bytes.");
            return;
        }
        int i2 = 8 > i / 2 ? i / 2 : 8;
        if (i - bArrClone.length > i2) {
            pac0.m168085a("CTR/SIC mode requires IV of at least: ", this.blockSize - i2, " bytes.");
            return;
        }
