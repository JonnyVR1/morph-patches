...
public class CTRSP800DRBG implements SP80090DRBG {
    ...
    public CTRSP800DRBG(BlockCipher blockCipher, int i, int i2, EntropySource entropySource, byte[] bArr, byte[] bArr2) {
        ...
        if (getMaxSecurityStrength(blockCipher, i) < i2) {
            ig3.m135964a("Requested security strength is not supported by block cipher and key size");
            throw null;
        }
    ...
    private void CTR_DRBG_Instantiate_algorithm(byte[] bArr, byte[] bArr2, byte[] bArr3) {
        ...
        this._Key = bArr4;
        byte[] bArr5 = new byte[blockSize];
        this.f206751_V = bArr5;
        CTR_DRBG_Update(bArrBlock_Cipher_df, bArr4, bArr5);
        this._reseedCounter = 1L;
    }

    private void CTR_DRBG_Reseed_algorithm(byte[] bArr) {
        CTR_DRBG_Update(Block_Cipher_df(Arrays.concatenate(getEntropy(), bArr), this._seedLength), this._Key, this.f206751_V);
        this._reseedCounter = 1L;
    }
    ...
    public int generate(byte[] bArr, byte[] bArr2, boolean z) {
        ...
        if (bArr2 != null) {
            ...
            CTR_DRBG_Update(bArrBlock_Cipher_df, this._Key, this.f206751_V);
        } else {
            bArrBlock_Cipher_df = new byte[i];
        }
        int length = this.f206751_V.length;
        byte[] bArr3 = new byte[length];
        this._engine.init(true, new KeyParameter(expandKey(this._Key)));
        for (int i2 = 0; i2 <= bArr.length / length; i2++) {
            int i3 = i2 * length;
            int length2 = bArr.length - i3 > length ? length : bArr.length - (this.f206751_V.length * i2);
            if (length2 != 0) {
                addOneTo(this.f206751_V);
                this._engine.processBlock(this.f206751_V, 0, bArr3, 0);
    ...
    public int getBlockSize() {
        return this.f206751_V.length * 8;
    }
