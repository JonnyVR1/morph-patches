...
final class State {
    ...
    final BitReader f205882br = new BitReader();
    ...
    public static void close(State state) throws IOException {
        ...
        if (i == 0) {
            qkq0.m175383a("State MUST be initialized");
        } else {
            if (i == 11) {
                return;
            }
            state.runningState = 11;
            BitReader.close(state.f205882br);
        }
    }
    ...
    public static void setInput(State state, InputStream inputStream) {
        if (state.runningState != 0) {
            qkq0.m175383a("State MUST be uninitialized");
            return;
        }
        BitReader.init(state.f205882br, inputStream);
        int iDecodeWindowBits = decodeWindowBits(state.f205882br);
        if (iDecodeWindowBits == 9) {
            C21876a.m221094a("Invalid 'windowBits' code");
            return;
        }
