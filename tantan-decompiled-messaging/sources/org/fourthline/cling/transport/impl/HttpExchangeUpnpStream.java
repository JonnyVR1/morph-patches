...
public abstract class HttpExchangeUpnpStream extends UpnpStream {
    ...
    public void run() {
        try {
            log.fine("Processing HTTP request: " + getHttpExchange().getRequestMethod() + MinimalPrettyPrinter.DEFAULT_ROOT_VALUE_SEPARATOR + getHttpExchange().getRequestURI());
            ...
            try {
                ...
                try {
                    byte[] bytes = C22151IO.readBytes(requestBody);
                    if (requestBody != null) {
                        requestBody.close();
                    }
                    ...
                    if (streamResponseMessageProcess != null) {
                        ...
                        if (length > 0) {
                            ...
                            try {
                                ...
                                C22151IO.writeBytes(responseBody, bodyBytes);
                                responseBody.flush();
                                responseBody.close();
                            } catch (Throwable th) {
            ...
            if (logger.isLoggable(level)) {
                log.log(level, "Cause: " + Exceptions.unwrap(th4), Exceptions.unwrap(th4));
            }
            try {
