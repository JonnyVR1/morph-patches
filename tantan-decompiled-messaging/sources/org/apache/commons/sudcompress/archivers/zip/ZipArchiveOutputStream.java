...
public class ZipArchiveOutputStream extends Suddo {
    ...
    public ZipArchiveOutputStream(File file) {
        ...
        try {
            ...
            try {
                ...
                p6g0.m167665c(seekableByteChannelNewByteChannel);
    ...
    private void copyFromZipInputStream(InputStream inputStream) throws IOException {
        ...
        if (currentEntry == null) {
            qkq0.m175383a("No current entry");
            return;
        }
    ...
    private byte[] createCentralFileHeader(ZipArchiveEntry zipArchiveEntry, ByteBuffer byteBuffer, EntryMetaData entryMetaData, boolean z) {
        ...
        if (!this.isSplitZip) {
            ...
            ZipShort.putShort(j6f.COLOR_SPACE_UNCALIBRATED, bArr, 34);
        } else {
    ...
    private byte[] createLocalFileHeader(ZipArchiveEntry zipArchiveEntry, ByteBuffer byteBuffer, boolean z, boolean z2, long j) {
        ZipShort zipShort = ResourceAlignmentExtraField.f205864ID;
    ...
    private void preClose() throws IOException {
        if (this.finished) {
            rhg0.m179353a("Stream has already been finished");
            return;
        }
        CurrentEntry currentEntry = this.entry;
        if (currentEntry == null) {
            rhg0.m179353a("No current entry to close");
        } else {
    ...
    public void finish() throws IOException {
        ...
        if (this.entry != null) {
            rhg0.m179353a("This archive contains unclosed entries.");
            return;
        }
        long totalBytesWritten = this.streamCompressor.getTotalBytesWritten();
    ...
    public void setLevel(int i) {
        if (i < -1 || i > 9) {
            ig3.m135964a(vdg0.m197997a(i, "Invalid compression level: "));
        } else {
            if (this.level == i) {
                return;
            }
    ...
    public void writeCentralDirectoryEnd() throws IOException {
        ...
        writeCounted(ZipShort.getBytes(Math.min(iIntValue, j6f.COLOR_SPACE_UNCALIBRATED)));
        writeCounted(ZipShort.getBytes(Math.min(size, j6f.COLOR_SPACE_UNCALIBRATED)));
