...
public class ZipArchiveInputStream extends ArchiveInputStream {
    ...
    private final InputStream f205868in;
    ...
    private static final byte[] f205867DD = ZipLong.DD_SIG.getBytes();
    ...
    public static /* synthetic */ class C218661 {

        /* JADX INFO: renamed from: $SwitchMap$org$apache$commons$sudcompress$archivers$zip$ZipMethod */
        static final /* synthetic */ int[] f205869x101d57ac;

        static {
            int[] iArr = new int[ZipMethod.values().length];
            f205869x101d57ac = iArr;
            try {
                iArr[ZipMethod.UNSHRINKING.ordinal()] = 1;
            } catch (NoSuchFieldError unused) {
            }
            try {
                f205869x101d57ac[ZipMethod.IMPLODING.ordinal()] = 2;
            } catch (NoSuchFieldError unused2) {
            }
            try {
                f205869x101d57ac[ZipMethod.BZIP2.ordinal()] = 3;
            } catch (NoSuchFieldError unused3) {
            }
            try {
                f205869x101d57ac[ZipMethod.ENHANCED_DEFLATED.ordinal()] = 4;
    ...
    public ZipArchiveInputStream(InputStream inputStream, String str, boolean z, boolean z2, boolean z3) {
        ...
        this.f205868in = new PushbackInputStream(inputStream, byteBufferAllocate.capacity());
    ...
    private void closeEntry() throws IOException {
        if (this.closed) {
            rhg0.m179353a("The stream is closed");
            return;
        }
    ...
    private void drainCurrentEntryData() throws EOFException {
        ...
        while (compressedSize > 0) {
            long j = this.f205868in.read(this.buf.array(), 0, (int) Math.min(this.buf.capacity(), compressedSize));
            if (j < 0) {
                throw new EOFException("Truncated ZIP entry: " + kbg0.m145301a(this.current.entry.getName()));
            }
            count(j);
            compressedSize -= j;
        }
    ...
    private int fill() throws IOException {
        ...
        int i = this.f205868in.read(this.buf.array());
        if (i > 0) {
            this.buf.limit(i);
    ...
    private void pushback(byte[] bArr, int i, int i2) throws IOException {
        ((PushbackInputStream) this.f205868in).unread(bArr, i, i2);
        pushedBackBytes(i2);
    }
    ...
    private int readDeflated(byte[] bArr, int i, int i2) throws IOException {
        ...
        if (fromInflater <= 0) {
            ...
            if (fromInflater == -1) {
                rhg0.m179353a("Truncated ZIP file");
                return 0;
            }
        }
        return fromInflater;
    ...
    private void readFully(byte[] bArr, int i) throws IOException {
        ...
        int iM167663a = p6g0.m167663a(this.f205868in, bArr, i, length);
        count(iM167663a);
        if (iM167663a >= length) {
            return;
        }
    ...
    private int readStored(byte[] bArr, int i, int i2) throws IOException {
        ...
        if (this.buf.position() >= this.buf.limit()) {
            this.buf.position(0);
            int i3 = this.f205868in.read(this.buf.array());
    ...
    private void readStoredEntry() throws IOException {
        ...
        while (!zBufferContainsSignature) {
            int i2 = this.f205868in.read(this.buf.array(), iCacheBytesRead, 512 - iCacheBytesRead);
            if (i2 <= 0) {
                rhg0.m179353a("Truncated ZIP file");
                return;
            }
    ...
    private void realSkip(long j) throws IOException {
        ...
        while (j2 < j) {
            long length = j - j2;
            InputStream inputStream = this.f205868in;
    ...
    public long getCompressedCount() {
        ...
        if (method2 == code) {
            return ((UnshrinkingInputStream) currentEntry2.f205871in).getCompressedCount();
        }
        int method3 = currentEntry2.entry.getMethod();
        int code2 = ZipMethod.IMPLODING.getCode();
        CurrentEntry currentEntry3 = this.current;
        if (method3 == code2) {
            return ((ExplodingInputStream) currentEntry3.f205871in).getCompressedCount();
        }
        ...
        if (method4 == code3) {
            return ((abg0) currentEntry4.f205871in).f68665c;
        }
        if (currentEntry4.entry.getMethod() == ZipMethod.BZIP2.getCode()) {
            return ((lig0) this.current.f205871in).f128197g.getBytesRead();
        }
        return -1L;
    }
            ...
            try {
                ...
                if (this.current.entry.getCompressedSize() != -1) {
                    if (ZipUtil.canHandleEntryData(this.current.entry) && methodByCode != ZipMethod.STORED && methodByCode != ZipMethod.DEFLATED) {
                        BoundedInputStream boundedInputStream = new BoundedInputStream(this.f205868in, this.current.entry.getCompressedSize());
                        int i = C218661.f205869x101d57ac[methodByCode.ordinal()];
                        if (i == 1) {
                            this.current.f205871in = new UnshrinkingInputStream(boundedInputStream);
                        } else if (i == 2) {
                            CurrentEntry currentEntry2 = this.current;
                            currentEntry2.f205871in = new ExplodingInputStream(currentEntry2.entry.getGeneralPurposeBit().getSlidingDictionarySize(), this.current.entry.getGeneralPurposeBit().getNumberOfShannonFanoTrees(), boundedInputStream);
                        } else if (i == 3) {
                            this.current.f205871in = new lig0(boundedInputStream);
                        } else if (i == 4) {
                            this.current.f205871in = new abg0(boundedInputStream);
                        }
                    }
                } else if (methodByCode == ZipMethod.ENHANCED_DEFLATED) {
                    this.current.f205871in = new abg0(this.f205868in);
                }
                this.entriesRead++;
    ...
    public int read(byte[] bArr, int i, int i2) throws IOException {
        ...
        if (this.current.entry.getMethod() == 0) {
            ...
            deflated = this.current.f205871in.read(bArr, i, i2);
        }
        if (deflated >= 0) {
    ...
    public class BoundedInputStream extends InputStream {
        ...
        /* JADX INFO: renamed from: in */
        private final InputStream f205870in;
        private final long max;
        private long pos = 0;

        public BoundedInputStream(InputStream inputStream, long j) {
            this.max = j;
            this.f205870in = inputStream;
        }
        ...
        public int available() {
            long j = this.max;
            if (j < 0 || this.pos < j) {
                return this.f205870in.available();
            }
            return 0;
        }
        ...
        public int read(byte[] bArr, int i, int i2) throws IOException {
            ...
            int i3 = this.f205870in.read(bArr, i, (int) (j >= 0 ? Math.min(i2, j - this.pos) : i2));
            if (i3 == -1) {
                return -1;
            }
            long j2 = i3;
        ...
        public long skip(long j) throws IOException {
            ...
            long jM167664b = p6g0.m167664b(this.f205870in, j);
            this.pos += jM167664b;
            return jM167664b;
        }
