...
class UnshrinkingInputStream extends Suddo {
    ...
    public int decompressNextSymbol() throws IOException {
        ...
        if (nextCode2 < 0) {
            rhg0.m179353a("Unexpected EOF;");
            return 0;
        }
        if (nextCode2 == 1) {
            if (getCodeSize() >= 13) {
                rhg0.m179353a("Attempt to increase code size beyond maximum");
                return 0;
            }
            incrementCodeSize();
        } else {
            if (nextCode2 != 2) {
                rhg0.m179353a(vdg0.m197997a(nextCode2, "Invalid clear code subcode "));
                return 0;
            }
            partialClear();
            setTableSize(getClearCode() + 1);
